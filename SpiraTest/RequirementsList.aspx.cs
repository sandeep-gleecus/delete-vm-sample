...
namespace Inflectra.SpiraTest.Web
{
	...
	/// Requirements Sortable List and handling all raised events
	/// </summary>
    [HeaderSettings(GlobalNavigation.NavigationHighlightedLink.Requirements, "SiteMap_Requirements", "Requirements-Management/#requirements-sorted-list")]
	public partial class RequirementsList : PageLayout
	{
		private const string CLASS_NAME = "Inflectra.SpiraTest.Web.RequirementsList::";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            //Update the setting that we're using the list view if it's not already set
            ...
            if (listPage != "Table")
            {
                SaveProjectSetting(GlobalFunctions.PROJECT_SETTINGS_REQUIREMENT_GENERAL_SETTINGS, GlobalFunctions.PROJECT_SETTINGS_KEY_HOME_PAGE, "Table");
            }

            //Populate the user and project id in the grid control
            this.grdRequirementsList.ProjectId = this.ProjectId;
            this.grdRequirementsList.BaseUrl = UrlRewriterModule.RetrieveRewriterURL(UrlRoots.NavigationLinkEnum.Requirements, ProjectId, -2);
            this.grdRequirementsList.ArtifactPrefix = GlobalFunctions.ARTIFACT_PREFIX_REQUIREMENT;

            //Add any event handlers
            //grdRequirementsList_loaded
            Dictionary<string, string> handlers = new Dictionary<string, string>();
            handlers.Add("loaded", "grdRequirementsList_loaded");
            this.grdRequirementsList.SetClientEventHandlers(handlers);
