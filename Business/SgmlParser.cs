...
namespace Inflectra.SpiraTest.Business {
    ...
    public class SgmlParseException : Exception
    {
        ...
        public SgmlParseException(string message, DtdEntity e)
            : base(message)
        {
            if (e != null)
    ...
    /// </summary>
    public class DtdEntity : IDisposable
    {
        /// <summary>
        ...
        public DtdEntity(string name, string pubid, string uri, string proxy)
        {
            m_name = name;
            m_publicId = pubid;
            m_uri = uri;
        ...
        public DtdEntity(string name, string literal)
        {
            m_name = name;
            m_literal = literal;
            m_isInternal = true;
        ...
        /// <param name="proxy">The proxy server to use when retrieving any web content.</param>
        public DtdEntity(string name, Uri baseUri, TextReader stm, string proxy)
        {
            m_name = name;
            m_isInternal = true;
            m_stm = stm;
        ...
        public void Open(DtdEntity parent, Uri baseUri)
        {
        ...
        {
            char ch = m_lastchar;
            while (ch != DtdEntity.EOF && (ch == ' ' || ch == '\r' || ch == '\n' || ch == '\t'))
            {
                ch = ReadChar();
            }
            return ch;
        ...
        public string ScanToken(StringBuilder sb, string term, bool nmtoken)
        {
            ...
            while (ch != DtdEntity.EOF && term.IndexOf(ch) < 0)
            {
        ...
        public string ScanLiteral(StringBuilder sb, char quote)
        {
            ...
            sb.Length = 0;
            char ch = ReadChar();
            while (ch != DtdEntity.EOF && ch != quote)
            {
                if (ch == '&')
                {
                    ch = ReadChar();
        ...
        public string ExpandCharEntity()
        {
            ...
            {
                ch = ReadChar();
                for (; ch != DtdEntity.EOF && ch != ';'; ch = ReadChar())
                {
                    int p = 0;
                    if (ch >= '0' && ch <= '9')
                    {
            ...
            // IE and Netscape map the unicode characters 
        ...
        {
            string str = (ch == DtdEntity.EOF) ? "EOF" : Char.ToString(ch);
    ...
    public class ElementDecl
    {
        ...
        /// <summary>
        /// The Sgml.ContentModel of the element declaration.
        /// </summary>
...
*/
    ...
    public class SgmlDtd
    {
        ...
        private Dictionary<string, DtdEntity> m_pentities;
        private Dictionary<string, DtdEntity> m_entities;
        private StringBuilder m_sb;
        private DtdEntity m_current;
        ...
        public SgmlDtd(string name, XmlNameTable nt)
        {
            ...
            this.m_pentities = new Dictionary<string, DtdEntity>();
            this.m_entities = new Dictionary<string, DtdEntity>();
        ...
        public static SgmlDtd Parse(Uri baseUri, string name, string pubid, string url, string subset, string proxy, XmlNameTable nt)
        {
            ...
            {
                dtd.PushEntity(baseUri, new DtdEntity(dtd.Name, pubid, url, proxy));
            }

            if (!string.IsNullOrEmpty(subset))
            {
                dtd.PushEntity(baseUri, new DtdEntity(name, subset));
            }
        ...
        {
            SgmlDtd dtd = new SgmlDtd(name, nt);
            dtd.PushEntity(baseUri, new DtdEntity(dtd.Name, baseUri, input, proxy));
            if (!string.IsNullOrEmpty(subset))
            {
                dtd.PushEntity(baseUri, new DtdEntity(name, subset));
            }
        ...
        /// </summary>
        /// <param name="name">The name of the <see cref="DtdEntity"/> to find.</param>
        ...
        //-------------------------------- Parser -------------------------
        private void PushEntity(Uri baseUri, DtdEntity e)
        {
            ...
            while (true) 
            {
                switch (ch) 
                {
                    ...
                    case '<':
                        ...
                        break;
                    case '%':
                        DtdEntity e = ParseParameterEntity(SgmlDtd.WhiteSpace);
                        try 
                        {
        ...
        string ScanName(string term)
        {
            ...
            if (ch == '%') 
            {
                DtdEntity e = ParseParameterEntity(term);
                ch = this.m_current.Lastchar;
        ...
        private DtdEntity ParseParameterEntity(string term)
        {
            ...
            if (this.m_current.Lastchar == ';') 
                this.m_current.ReadChar();
            DtdEntity e = GetParameterEntity(name);
            return e;
        }

        private DtdEntity GetParameterEntity(string name)
        {
            DtdEntity e = null;
        ...
        /// <summary>
        /// Returns a dictionary for looking up entities by their <see cref="DtdEntity.Literal"/> value.
        /// </summary>
        /// <returns>A dictionary for looking up entities by their <see cref="DtdEntity.Literal"/> value.</returns>
        ...
        public Dictionary<string, DtdEntity> GetEntitiesLiteralNameLookup()
        {
            Dictionary<string, DtdEntity> hashtable = new Dictionary<string, DtdEntity>();
            foreach (DtdEntity entity in this.m_entities.Values)
        ...
        private void ParseEntity()
        {
            ...
            else 
            {
                ...
                if (DtdEntity.IsLiteralType(tok))
                {
                    ch = this.m_current.SkipWhitespace();
                    string literal = this.m_current.ScanLiteral(this.m_sb, ch);
                    e = new DtdEntity(name, literal);
                ...
                else 
                {
                    ...
                    e = new DtdEntity(name, pubid, uri, this.m_current.Proxy);
                }
            }
            ch = this.m_current.SkipWhitespace();
            if (ch == '-') 
        ...
        string[] ParseNameGroup(char ch, bool nmtokens)
        {
            ...
            if (ch == '(') 
            {
                ...
                while (ch != ')') 
                {
                    ...
                    if (ch == '%') 
                    {
                        DtdEntity e = ParseParameterEntity(SgmlDtd.ngterm);
                        PushEntity(this.m_current.ResolvedUri, e);
        ...
        private ContentModel ParseContentModel(char ch)
        {
            ...
            {
                DtdEntity e = ParseParameterEntity(SgmlDtd.dcterm);
                PushEntity(this.m_current.ResolvedUri, e);
        ...
        void ParseModel(char cmt, ContentModel cm)
        {
            ...
            while (ch != cmt || cm.CurrentDepth > depth) // the entity must terminate while inside the content model.
            {
                ...
                if (ch == '%') 
                {
                    DtdEntity e = ParseParameterEntity(SgmlDtd.cmterm);
                    PushEntity(this.m_current.ResolvedUri, e);
                    ParseModel(DtdEntity.EOF, cm);
                    PopEntity();                    
                    ch = this.m_current.SkipWhitespace();
                } 
                else if (ch == '(') 
        ...
        void ParseAttList(Dictionary<string, AttDef> list, char term)
        {
            ...
            while (ch != term) 
            {
                if (ch == '%') 
                {
                    DtdEntity e = ParseParameterEntity(SgmlDtd.peterm);
                    PushEntity(this.m_current.ResolvedUri, e);
                    ParseAttList(list, DtdEntity.EOF);
                    PopEntity();                    
                    ch = this.m_current.SkipWhitespace();
                } 
                else if (ch == '-') 
