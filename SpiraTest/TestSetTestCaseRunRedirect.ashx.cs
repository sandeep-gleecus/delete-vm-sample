...
{
    /// <summary>
    /// Accepts a TestSetTestCase instance ID and redirects to the latest test run for that instance
    /// </summary>
    public class TestSetTestCaseRunRedirect : LocalizedHttpHandler
    {
        private const string CLASS_NAME = "Inflectra.SpiraTest.Web.TestSetTestCaseRunRedirect::";
                ...
                if (!String.IsNullOrEmpty(context.Request.QueryString[GlobalFunctions.PARAMETER_TEST_SET_TEST_CASE_ID]))
                {
                    ...
                    if (Int32.TryParse(context.Request.QueryString[GlobalFunctions.PARAMETER_TEST_SET_TEST_CASE_ID], out testSetTestCaseId))
                    {
                        ...
                        try
                        {
                            ...
                            {
                                //Now find the most recent test run for that test set test case
                                TestRunManager testRunManager = new TestRunManager();
                                Hashtable filters = new Hashtable();
                                filters.Add("TestSetTestCaseId", testSetTestCase.TestSetTestCaseId);
                                List<TestRunView> testRuns = testRunManager.Retrieve(ProjectId, "EndDate", false, 1, 1, filters, GlobalFunctions.GetCurrentTimezoneUtcOffset());

                                if (testRuns.Count > 0)
                                {
                                    int testRunId = testRuns[0].TestRunId;
                                    context.Response.Redirect(UrlRewriterModule.RetrieveRewriterURL(Common.UrlRoots.NavigationLinkEnum.TestRuns, ProjectId, testRunId));
                                }
                                else
                                {
                                    //Do nothing
                                    Logger.LogWarningEvent(CLASS_NAME + METHOD_NAME, "Unable to redirect to test run that has TXTC = " + testSetTestCaseId);
                                }
                            }
                            else
                            {
