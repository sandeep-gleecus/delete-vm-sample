...
namespace Inflectra.SpiraTest.Web.Administration.ProjectTemplate
{
	...
	/// Administration Task Workflow Details Page and handling all raised events
	/// </summary>
    [
    HeaderSettings(GlobalNavigation.NavigationHighlightedLink.Administration, "Admin_WorkflowDetails_Title", "Template-Tasks/#edit-workflow-details", "Admin_WorkflowDetails_Title"),
    ...
    public partial class TaskWorkflowDetails : AdministrationBase
	{
        private const string CLASS_NAME = "Inflectra.SpiraTest.Web.Administration.ProjectTemplate.TaskWorkflowDetails::";

        protected int workflowId;
        protected List<TaskStatus> taskStatuses;
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            if (!Int32.TryParse(Request.QueryString[GlobalFunctions.PARAMETER_WORKFLOW_ID], out workflowId))
            {
                ...
                Response.Redirect(UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "TaskWorkflows"));
                return;
            }
            ...
            //Now get the list of possible workflow steps (i.e. task statuses)
            TaskManager taskManager = new TaskManager();
            this.taskStatuses = taskManager.RetrieveStatuses();

            //Set the the return URL
            this.lnkWorkflowList.NavigateUrl = UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "TaskWorkflows");
		...
		protected void LoadAndBindData()
		{
            ...
            TaskWorkflow workflow = taskWorkflowManager.Workflow_RetrieveById(this.workflowId);
			...
			//Databind the form
            this.grdWorkflowSteps.DataSource = taskStatuses;
            this.DataBind();
		...
		private void grdWorkflowSteps_RowCreated(object sender, GridViewRowEventArgs e)
		{
            ...
            {
                int statusId = ((TaskStatus)(e.Row.DataItem)).TaskStatusId;
                List<DataModel.TaskWorkflowTransition> transitions = new TaskWorkflowManager().WorkflowTransition_RetrieveByInputStatus(this.workflowId, statusId);
		...
		protected void rptWorkflowStepTransitions_ItemCommand(object source, RepeaterCommandEventArgs e)
		{
			...
			if (e.CommandName.ToLower(System.Globalization.CultureInfo.InvariantCulture) == "delete")
			{
                ...
                new TaskWorkflowManager().WorkflowTransition_Delete(workflowTransitionId);
		...
		private void btnAdd_Click(object sender, EventArgs e)
		{
			...
			int sourceTaskStatusId = Int32.Parse (this.hdnSourceStep.Value);
			int destinationTaskStatusId = Int32.Parse(this.ddlDestinationStep.RawSelectedValue);
			...
			//but when developers do the same thing they need to be the owner of the task
            TaskWorkflowManager taskWorkflowManager = new Business.TaskWorkflowManager();
            taskWorkflowManager.WorkflowTransition_Insert(this.workflowId, sourceTaskStatusId, destinationTaskStatusId, this.txtTransitionName.Text);
        ...
        /// </summary>
        /// <returns></returns>
        protected string GetTaskStatusesAsJson()
        {
            const string METHOD_NAME = "GetTaskStatusesAsJson";

            Logger.LogEnteringEvent(CLASS_NAME + METHOD_NAME);

            Dictionary<string, string> statusDic = new Dictionary<string, string>();
            foreach (TaskStatus taskStatus in this.taskStatuses)
            {
                if (!statusDic.ContainsKey(taskStatus.TaskStatusId.ToString()))
                {
                    statusDic.Add(taskStatus.TaskStatusId.ToString(), taskStatus.Name);
                }
            }
