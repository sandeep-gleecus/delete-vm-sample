...
{
	public partial class AssignedIncidents : WebPartBase, IWebPartReloadable
	{
		private const string CLASS_NAME = "Inflectra.SpiraTest.Web.UserControls.WebParts.MyPage.AssignedIncidents::";
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			try
			{
                ...
                {
                    string rssUrl = this.ResolveUrl("~/Feeds/" + UserId + "/" + UserRssToken + "/AssignedIncidents.aspx");
                    this.Subtitle = GlobalFunctions.WEBPART_SUBTITLE_RSS + rssUrl;
                }

				//Register event handlers
				grdOwnedIncidents.RowDataBound += new GridViewRowEventHandler(grdOwnedIncidents_RowDataBound);
		...
		public void LoadAndBindData()
		{
			...
			if (GetUserSetting(GlobalFunctions.USER_SETTINGS_MY_PAGE_SETTINGS, GlobalFunctions.USER_SETTINGS_KEY_FILTER_BY_PROJECT, false) && ProjectId > 0)
            {
                ...
                this.grdOwnedIncidents.Columns[2].Visible = false;
                this.grdOwnedIncidents.Columns[3].Visible = true;
            }
            else
            {
                //Display the project field
                this.grdOwnedIncidents.Columns[2].Visible = true;
                this.grdOwnedIncidents.Columns[3].Visible = false;
            }

			//Now get the list of open incidents owned by the user in order of incident type
			IncidentManager incidentManager = new IncidentManager();
			List<IncidentView> ownedIncidents = incidentManager.RetrieveOpenByOwnerId(UserId, filterProjectId, null, this.rowsToDisplay);
            ...
            ((NameDescriptionFieldEx)this.grdOwnedIncidents.Columns[1]).NavigateUrlFormat = "{0}";

            this.grdOwnedIncidents.DataSource = ownedIncidents;
			this.grdOwnedIncidents.DataBind();
		}
		...
		private void grdOwnedIncidents_RowDataBound(object sender, GridViewRowEventArgs e)
		{
			...
			if (e.Row.RowType == DataControlRowType.DataRow)
			{
                ...
                if (hyperlink != null)
                {
                    hyperlink.NavigateUrl = UrlRewriterModule.RetrieveRewriterURL(UrlRoots.NavigationLinkEnum.Incidents, incident.ProjectId, incident.IncidentId) + "?" + GlobalFunctions.PARAMETER_REFERER_PROJECT_LIST + "=" + GlobalFunctions.PARAMETER_VALUE_TRUE;
                }
			}
		}
	}
