...
using Inflectra.SpiraTest.ApiTestSuite.SpiraSoapService60;
...
namespace Inflectra.SpiraTest.ApiTestSuite.API_Tests
{
	...
	public class V6_0_ApiTest
	{
		protected static SpiraSoapService60.SoapServiceClient spiraSoapClient;
        protected static SpiraSoapService60.RemoteCredentials credentials;
		...
		public void Init()
		{
			...
			spiraSoapClient = new SpiraSoapService60.SoapServiceClient();
			spiraSoapClient.Endpoint.Address = new EndpointAddress(Properties.Settings.Default.WebServiceUrl + "v6_0/SoapService.svc");

			//Configure the HTTP Binding (cookies not needed for the v6.0 API)
		...
		public void CleanUp()
		{
            ...
            credentials = spiraSoapClient.Connection_Authenticate1("administrator", "Welcome123$", PLUGIN_NAME);
		...
		{
			//First Authenticate as Administrator (should succeed)
			credentials = spiraSoapClient.Connection_Authenticate1("administrator", "Welcome123$", "MyPlugIn");
			Assert.IsNotNull(credentials, "Authentication 1 Failed");

            //Next Authenticate as Fred Bloggs (should succeed), specifying the API Key
            credentials = spiraSoapClient.Connection_Authenticate2("fredbloggs", "{7A05FD06-83C3-4436-B37F-51BCF0060483}", "MyPlugIn");
			Assert.IsNotNull(credentials, "Authentication 2 Failed");

			//Test that we can fallback to using API key even with the first method (needed for OAuth users)
			credentials = spiraSoapClient.Connection_Authenticate1("fredbloggs", "{7A05FD06-83C3-4436-B37F-51BCF0060483}", "MyPlugIn");
			Assert.IsNotNull(credentials, "Authentication 1 Failed using Api Key backup");

			//Finally Make sure that an invalid authentication fails in both cases
			credentials = spiraSoapClient.Connection_Authenticate1("testtest", "wrongwrong", "MyPlugIn");
			Assert.IsNull(credentials, "Authentication 3 Should Have Failed");

            //Finally Make sure that an invalid authentication fails in both cases
            credentials = spiraSoapClient.Connection_Authenticate2("testtest", "{00000000-0000-0000-0000-000000000000}", "MyPlugIn");
            Assert.IsNull(credentials, "Authentication 4 Should Have Failed");
		}
		...
		public void _02_RetrieveProjectList()
		{
			...
			credentials = spiraSoapClient.Connection_Authenticate1("fredbloggs", "PleaseChange", PLUGIN_NAME);
			...
			catch (FaultException<ExceptionDetail> exception)
			{
				...
				if (detail.Type == "System.ArgumentNullException")
				{
					errorCaught = true;
				}
				else
			...
			credentials = spiraSoapClient.Connection_Authenticate1("wrong", "wrong", PLUGIN_NAME);
			errorCaught = false;
			try
			{
				remoteProjects = spiraSoapClient.Project_Retrieve(credentials);
		...
		public void _03_CreateProject()
		{
			...
			remoteProject = spiraSoapClient.Project_Create(credentials, remoteProject, null);
			...
			remoteProject = spiraSoapClient.Project_Create(credentials, remoteProject, projectId1);
            ...
            remoteProject.Active = true;
            spiraSoapClient.Project_Update(credentials, remoteProject);
		...
		public void _04_ImportUsers()
		{
			...
			remoteUser.LdapDn = "";
			remoteUser = spiraSoapClient.User_Create(credentials, remoteUser, "aant123456789", "What is 2+3?", "5", projectId1, InternalRoutines.PROJECT_ROLE_MANAGER);
			...
			remoteUser = spiraSoapClient.User_Create(credentials, remoteUser, null, null, null, projectId1, InternalRoutines.PROJECT_ROLE_TESTER);
			...
			remoteUser.Active = true;
			remoteUser.LdapDn = "";
			remoteUser = spiraSoapClient.User_Create(credentials, remoteUser, "aant123456789", "What is 1+1?", "2", projectId1, InternalRoutines.PROJECT_ROLE_MANAGER);
			...
			spiraSoapClient.Project_AddUserMembership(credentials, remoteProjectUser);
			...
			remoteUser.Active = true;
			remoteUser.Approved = true;
			remoteUser = spiraSoapClient.User_Create(credentials, remoteUser, "mypassword123456", "What is 1+1?", "2", projectId1, InternalRoutines.PROJECT_ROLE_TESTER);
			...
			spiraSoapClient.User_Update(credentials, remoteUser);
            ...
            spiraSoapClient.Project_UpdateUserMembership(credentials, remoteProjectUser);
            ...
            spiraSoapClient.Project_RemoveUserMembership(credentials, remoteProjectUser);
		...
		public void _05_ImportCustomProperties()
		{
			...
			RemoteCustomProperty[] remoteCustomProperties = spiraSoapClient.CustomProperty_RetrieveForArtifactType(credentials, projectTemplateId1, (int)DataModel.Artifact.ArtifactTypeEnum.Requirement, false);
			...
			customListId1 = spiraSoapClient.CustomProperty_AddCustomList(credentials, remoteCustomList).CustomPropertyListId.Value;
			...
			customValueId2 = spiraSoapClient.CustomProperty_AddCustomListValue(credentials, projectTemplateId1, remoteCustomListValue).CustomPropertyValueId.Value;
			...
			remoteCustomList.Active = true;
			remoteCustomList.Values = new RemoteCustomListValue[] { remoteCustomListValue };
			customListId2 = spiraSoapClient.CustomProperty_AddCustomList(credentials, remoteCustomList).CustomPropertyListId.Value;
			...
			spiraSoapClient.CustomProperty_UpdateCustomList(credentials, remoteCustomList);
			...
			spiraSoapClient.CustomProperty_AddDefinition(credentials, remoteCustomProperty, null);
			...
			spiraSoapClient.CustomProperty_AddDefinition(credentials, remoteCustomProperty, remoteCustomList.CustomPropertyListId);

			//Verify the additions
			remoteCustomProperties = spiraSoapClient.CustomProperty_RetrieveForArtifactType(credentials, projectTemplateId1, (int)DataModel.Artifact.ArtifactTypeEnum.Requirement, false);
			...
			//Verify the additions
			remoteCustomProperties = spiraSoapClient.CustomProperty_RetrieveForArtifactType(credentials, projectTemplateId1, (int)DataModel.Artifact.ArtifactTypeEnum.Incident, false);
			Assert.AreEqual(2, remoteCustomProperties.Length);
			Assert.AreEqual("Custom_01", remoteCustomProperties.FirstOrDefault(c => c.Name == "Source").CustomPropertyFieldName);
			Assert.AreEqual("Custom_02", remoteCustomProperties.FirstOrDefault(c => c.Name == "Req Type").CustomPropertyFieldName);
			Assert.AreEqual(1, remoteCustomProperties.FirstOrDefault(c => c.Name == "Source").CustomPropertyTypeId);
			...
			remoteCustomProperties = spiraSoapClient.CustomProperty_RetrieveForArtifactType(credentials, projectTemplateId1, (int)DataModel.Artifact.ArtifactTypeEnum.Task, false);
			...
			spiraSoapClient.CustomProperty_AddDefinition(credentials, remoteCustomProperty, customListId2);
			remoteCustomProperty = new RemoteCustomProperty();
			remoteCustomProperty.CustomPropertyTypeId = (int)DataModel.CustomProperty.CustomPropertyTypeEnum.List;
			...
			remoteCustomProperties = spiraSoapClient.CustomProperty_RetrieveForArtifactType(credentials, projectTemplateId1, (int)DataModel.Artifact.ArtifactTypeEnum.TestSet, false);
			Assert.AreEqual(1, remoteCustomProperties.Length);
			remoteCustomProperties = spiraSoapClient.CustomProperty_RetrieveForArtifactType(credentials, projectTemplateId1, (int)DataModel.Artifact.ArtifactTypeEnum.TestRun, false);
			Assert.AreEqual(1, remoteCustomProperties.Length);
		}
		...
		{
            //First lets authenticate and connect to the new project as an administrator
            credentials = credentials = spiraSoapClient.Connection_Authenticate1("administrator", "Welcome123$", PLUGIN_NAME);
			...
			componentId1 = spiraSoapClient.Component_Create(credentials, new RemoteComponent() { Name = "Component 1", IsActive = true, ProjectId = projectId1 }).ComponentId.Value;
			componentId2 = spiraSoapClient.Component_Create(credentials, new RemoteComponent() { Name = "Component 2", IsActive = false, ProjectId = projectId1 }).ComponentId.Value;
			componentId3 = spiraSoapClient.Component_Create(credentials, new RemoteComponent() { Name = "Component 3", IsActive = true, ProjectId = projectId1 }).ComponentId.Value;
			...
			spiraSoapClient.Component_Update(credentials, component);
		...
		public void _07_ImportReleases()
		{
            ...
            credentials = spiraSoapClient.Connection_Authenticate1("aant", "aant123456789", PLUGIN_NAME);
			...
			releaseId1 = spiraSoapClient.Release_Create(credentials, remoteRelease, null).ReleaseId.Value;
			//Iteration #1
			...
			remoteRelease.ResourceCount = 5;
			iterationId1 = spiraSoapClient.Release_Create(credentials, remoteRelease, releaseId1).ReleaseId.Value;
			//Iteration #2
			remoteRelease = new RemoteRelease();
            remoteRelease.ProjectId = projectId1;
			...
			remoteRelease.ResourceCount = 5;
			iterationId2 = spiraSoapClient.Release_Create(credentials, remoteRelease, releaseId1).ReleaseId.Value;
			...
			spiraSoapClient.Release_Update(credentials, remoteRelease);
			...
			remoteRelease = spiraSoapClient.Release_RetrieveById(credentials, projectId1, iterationId1);
			Assert.AreEqual((int)Release.ReleaseStatusEnum.Planned, remoteRelease.ReleaseStatusId, "IncidentStatusId");
			...
			remoteRelease.ResourceCount = 5;
			remoteRelease.ReleaseStatusId = (int)Release.ReleaseStatusEnum.Completed;
			int iterationId3 = spiraSoapClient.Release_Create(credentials, remoteRelease, null).ReleaseId.Value;
		...
		public void _08_ImportRequirements()
		{
			...
			remoteRequirement = spiraSoapClient.Requirement_Create1(credentials, remoteRequirement, 0);
			...
			remoteRequirement = spiraSoapClient.Requirement_Create1(credentials, remoteRequirement, 1);
			...
			spiraSoapClient.Requirement_AddStep(credentials, projectId1, remoteRequirementStep, null, null);
			remoteRequirementStep = new RemoteRequirementStep();
			remoteRequirementStep.RequirementId = requirementId5;
			remoteRequirementStep.Description = "Requirement 4 Step 3";
			remoteRequirementStep.CreationDate = DateTime.UtcNow;
			remoteRequirementStep = spiraSoapClient.Requirement_AddStep(credentials, projectId1, remoteRequirementStep, null, USER_ID_FRED_BLOGGS);
			...
			remoteRequirementStep.CreationDate = DateTime.UtcNow;
			spiraSoapClient.Requirement_AddStep(credentials, projectId1, remoteRequirementStep, requirementStepId, null);
			...
			remoteRequirement.Description = String.Empty;
			remoteRequirement.AuthorId = userId1;
			remoteRequirement = spiraSoapClient.Requirement_Create2(credentials, remoteRequirement, requirementId5);
			...
			spiraSoapClient.Requirement_Update(credentials, remoteRequirement);
			...
			remoteRequirementStep = spiraSoapClient.Requirement_RetrieveStepById(credentials, projectId1, requirementStepId);
			remoteRequirementStep.Description = "Requirement 4 Step 1A";
			spiraSoapClient.Requirement_UpdateStep(credentials, projectId1, remoteRequirementStep);
		...
		public void _09_ImportTestCases()
		{
			...
			remoteTestCaseFolder = spiraSoapClient.TestCase_CreateFolder(credentials, remoteTestCaseFolder);
			...
			remoteTestCase = spiraSoapClient.TestCase_Create(credentials, remoteTestCase);
			...
			remoteTestCase.AuthorId = userId2;
			remoteTestCase.OwnerId = userId1;
			spiraSoapClient.TestCase_Update(credentials, remoteTestCase);
		...
		public void _10_ImportTestSteps()
		{
			...
			remoteTestStep = spiraSoapClient.TestCase_AddStep(credentials, remoteTestStep, testCaseId3);
			...
			remoteTestStep.ExpectedResult = "It should work";
			remoteTestStep.SampleData = "test 123";
			remoteTestStep = spiraSoapClient.TestCase_AddStep(credentials, remoteTestStep, testCaseId4);
			...
			testStepId5 = spiraSoapClient.TestCase_AddLink(credentials, projectId1, testCaseId1, 1, testCaseId3, null);
			...
			testStepId6 = spiraSoapClient.TestCase_AddLink(credentials, projectId1, testCaseId1, 2, testCaseId2, parameterArray);
		...
		public void _11_ImportCoverage()
		{
			...
			spiraSoapClient.Release_AddTestMapping(credentials, projectId1, remoteReleaseTestCaseMapping);
			...
			remoteReleaseTestCaseMapping.ReleaseId = releaseId1;
			remoteReleaseTestCaseMapping.TestCaseId = testCaseId1;
			spiraSoapClient.Release_RemoveTestMapping(credentials, projectId1, remoteReleaseTestCaseMapping);
			//Iteration 1
			remoteReleaseTestCaseMapping = new RemoteReleaseTestCaseMapping();
			remoteReleaseTestCaseMapping.ReleaseId = iterationId1;
			remoteReleaseTestCaseMapping.TestCaseId = testCaseId1;
			...
			remoteReleaseTestCaseMapping.TestCaseId = testCaseId1;
			remoteReleaseTestCaseMappings.Add(remoteReleaseTestCaseMapping);
			spiraSoapClient.Release_AddTestMapping2(credentials, projectId1, remoteReleaseTestCaseMappings.ToArray());
		...
		public void _12_ImportTestSets()
		{
			...
			testSetFolderId1 = spiraSoapClient.TestSet_CreateFolder(credentials, remoteTestSetFolder).TestSetFolderId.Value;
			...
			testSetId1 = spiraSoapClient.TestSet_Create(credentials, remoteTestSet).TestSetId.Value;
			...
			testSetId2 = spiraSoapClient.TestSet_Create(credentials, remoteTestSet).TestSetId.Value;
			//Verify that it inserted correctly
			...
			RemoteTestSetTestCaseMapping[] remoteTestSetTestCaseMappings = spiraSoapClient.TestSet_AddTestMapping(credentials, projectId1, remoteTestSetTestCaseMapping, null, null);
			...
			remoteTestSetTestCaseMappings = spiraSoapClient.TestSet_AddTestMapping(credentials, projectId1, remoteTestSetTestCaseMapping, null, null);
			Assert.AreEqual(1, remoteTestSetTestCaseMappings.Length);
			Assert.AreEqual(testSetId1, remoteTestSetTestCaseMappings[0].TestSetId);
			...
			remoteTestSetTestCaseMapping.OwnerId = USER_ID_FRED_BLOGGS;
			spiraSoapClient.TestSet_AddTestMapping(credentials, projectId1, remoteTestSetTestCaseMapping, existingTestSetTestCaseId, parameters.ToArray());
		...
		public void _13_ImportTestRun()
		{
			...
			RemoteManualTestRun[] remoteManualTestRuns = spiraSoapClient.TestRun_CreateFromTestCases(credentials, projectId1, new int[] { testCaseId3, testCaseId4 }, iterationId1);
			...
			remoteManualTestRuns = spiraSoapClient.TestRun_Save(credentials, projectId1, remoteManualTestRuns, DateTime.UtcNow);
			...
			remoteManualTestRuns = spiraSoapClient.TestRun_Save(credentials, projectId1, remoteManualTestRuns, DateTime.UtcNow.AddMinutes(1));
			testRunId1 = remoteManualTestRuns[0].TestRunId.Value;
			testRunId2 = remoteManualTestRuns[1].TestRunId.Value;
			testRunStepId1 = remoteManualTestRuns[0].TestRunSteps[0].TestRunStepId.Value;
			testRunStepId2 = remoteManualTestRuns[0].TestRunSteps[1].TestRunStepId.Value;
			...
			testRunId3 = spiraSoapClient.TestRun_RecordAutomated1(credentials, remoteAutomatedTestRun).TestRunId.Value;
			...
			remoteAutomatedTestRun.TestRunFormatId = 1; //Plain Text
			testRunId4 = spiraSoapClient.TestRun_RecordAutomated1(credentials, remoteAutomatedTestRun).TestRunId.Value;
			...
			remoteAutomatedTestRuns.Add(remoteAutomatedTestRun);
			RemoteAutomatedTestRun[] remoteAutomatedTestRuns2 = spiraSoapClient.TestRun_RecordAutomated3(credentials, projectId1, remoteAutomatedTestRuns.ToArray());
			...
			credentials = spiraSoapClient.Connection_Authenticate1("administrator", "Welcome123$", PLUGIN_NAME);
			spiraSoapClient.Project_RefreshProgressExecutionStatusCaches(credentials, projectId1, null, false);
            credentials = spiraSoapClient.Connection_Authenticate1("aant", "aant123456789", PLUGIN_NAME);
			...
			testRunId6 = spiraSoapClient.TestRun_RecordAutomated1(credentials, remoteAutomatedTestRun).TestRunId.Value;
			...
			remoteDocument = spiraSoapClient.Document_AddFile(credentials, remoteDocument, dummyImage);
		...
		public void _14_ImportIncidents()
		{
			...
			incidentId1 = spiraSoapClient.Incident_Create(credentials, remoteIncident).IncidentId.Value;
			//Incident #2
			...
			incidentId2 = spiraSoapClient.Incident_Create(credentials, remoteIncident).IncidentId.Value;
			//Incident #3
			remoteIncident = new RemoteIncident();
            remoteIncident.ProjectId = projectId1;
            remoteIncident.IncidentTypeId = incidentTypeId;
			...
			incidentId3 = spiraSoapClient.Incident_Create(credentials, remoteIncident).IncidentId.Value;
			...
			spiraSoapClient.Incident_AddComments(credentials, projectId1, remoteComments);
			...
			spiraSoapClient.Incident_Update(credentials, remoteIncident);
			...
			remoteIncident.OpenerId = userId1;
			remoteIncident.OwnerId = userId2;
			int incidentId4 = spiraSoapClient.Incident_Create(credentials, remoteIncident).IncidentId.Value;
		...
		public void _15_ImportAttachments()
		{
			...
			attachmentId1 = spiraSoapClient.Document_AddFile(credentials, remoteDocument, attachmentData).AttachmentId.Value;
			...
			remoteDocument.AuthorId = userId2;
			remoteDocument.AttachedArtifacts = new RemoteLinkedArtifact[] { remoteLinkedArtifact };
			attachmentId2 = spiraSoapClient.Document_AddFile(credentials, remoteDocument, attachmentData).AttachmentId.Value;

			//Now lets get the attachment meta-data and verify
			remoteDocument = spiraSoapClient.Document_RetrieveById(credentials, projectId1, attachmentId1);
			...
			remoteLinkedArtifact.ArtifactTypeId = (int)DataModel.Artifact.ArtifactTypeEnum.TestCase;
			remoteDocument.AttachedArtifacts = new RemoteLinkedArtifact[] { remoteLinkedArtifact };
			attachmentId3 = spiraSoapClient.Document_AddUrl(credentials, remoteDocument).AttachmentId.Value;
			...
			remoteDocument = new RemoteDocument();
			remoteDocument.ProjectId = projectId1;
			remoteDocument.FilenameOrUrl = "test_data3.xls";
			remoteDocument.Description = "This is a test to set a non default status when that is not supported";
			remoteDocument.AuthorId = userId2;
			remoteDocument.DocumentStatusId = documentStatuses[0].DocumentStatusId;
			remoteLinkedArtifact = new RemoteLinkedArtifact();
			remoteLinkedArtifact.ArtifactId = testCaseId1;
			remoteLinkedArtifact.ArtifactTypeId = (int)DataModel.Artifact.ArtifactTypeEnum.TestCase;
			remoteDocument.AttachedArtifacts = new RemoteLinkedArtifact[] { remoteLinkedArtifact };
			int attachmentId4 = spiraSoapClient.Document_AddFile(credentials, remoteDocument, attachmentData).AttachmentId.Value; ;
		...
		public void _16_ImportTasks()
		{
			...
			remoteTaskFolder = spiraSoapClient.Task_CreateFolder(credentials, remoteTaskFolder);
			...
			taskId1 = spiraSoapClient.Task_Create(credentials, remoteTask).TaskId.Value;
			...
			taskId2 = spiraSoapClient.Task_Create(credentials, remoteTask).TaskId.Value;
			...
			spiraSoapClient.Task_Update(credentials, remoteTask);
			...
			spiraSoapClient.Task_UpdateFolder(credentials, remoteTaskFolder);
			...
			remoteTask.TaskFolderId = taskFolderId1;
			remoteTask.OwnerId = userId2;
			int taskId3 = spiraSoapClient.Task_Create(credentials, remoteTask).TaskId.Value;
		...
		public void _17_RetrieveIncidents()
		{
			...
			RemoteIncident[] remoteIncidents = spiraSoapClient.Incident_RetrieveNew(credentials, projectId1, DateTime.Parse("1/1/2000"), 1, 100);
			...
			remoteIncidents = spiraSoapClient.Incident_Retrieve(credentials, 1, remoteFilters.ToArray(), remoteSort, 1, 999999);
		...
		public void _18_RetrieveTasks()
		{
			...
			remoteSort.SortAscending = true;
			remoteTasks = spiraSoapClient.Task_Retrieve(credentials, 1, remoteFilters.ToArray(), remoteSort, 1, 999999);
		...
		public void _19_RetrieveRequirements()
		{
			...
			remoteFilters.Add(remoteFilter);
			RemoteRequirement[] remoteRequirements = spiraSoapClient.Requirement_Retrieve(credentials, 1, remoteFilters.ToArray(), 1, 999999);
			...
			RemoteRequirementStatus[] requirementStatuses = spiraSoapClient.Requirement_RetrieveStatuses(credentials);
		...
		public void _20_RetrieveTestCases()
		{
			...
			remoteFilter.DateRangeValue.EndDate = DateTime.UtcNow;
			remoteFilters.Add(remoteFilter);
			RemoteTestCase[] remoteTestCases = spiraSoapClient.TestCase_Retrieve(credentials, 1, remoteFilters.ToArray(), remoteSort, 1, 999999, null);
			...
			remoteTestCases = spiraSoapClient.TestCase_Retrieve(credentials, 1, remoteFilters.ToArray(), remoteSort, 1, 999999, 1);
			Assert.AreEqual(5, remoteTestCases.Length);
			Assert.AreEqual("Ability to create new author", remoteTestCases[0].Name);
			...
			RemoteTestRun[] remoteTestRuns = spiraSoapClient.TestRun_Retrieve(credentials, 1, remoteFilters.ToArray(), remoteSort, 1, 999999);
			...
			remoteSort.PropertyName = "Name";
			remoteSort.SortAscending = true;
			remoteTestCases = spiraSoapClient.TestCase_RetrieveByFolder(credentials, 1, 1, null, remoteSort, 1, 999999, null);
			...
			RemoteTestCaseType[] testCaseTypes = spiraSoapClient.TestCase_RetrieveTypes(credentials, 1);
			...
			RemoteTestCaseStatus[] testCaseStatuses = spiraSoapClient.TestCase_RetrieveStatuses(credentials);
		...
		public void _21_RetrieveReleases()
		{
			...
			remoteFilters.Add(remoteFilter);
			RemoteRelease[] remoteReleases = spiraSoapClient.Release_Retrieve2(credentials, 1, remoteFilters.ToArray(), 1, 999999);
			...
			RemoteReleaseStatus[] releaseStatuses = spiraSoapClient.Release_RetrieveStatuses(credentials);
		...
		public void _22_RetrieveTestSets()
		{
			...
			remoteFilters.Add(remoteFilter);
			RemoteTestSet[] remoteTestSets = spiraSoapClient.TestSet_Retrieve(credentials, 1, remoteFilters.ToArray(), remoteSort, 1, 999999, null);
			...
			remoteFilter.DateRangeValue.EndDate = DateTime.UtcNow.AddDays(-8);
			remoteFilters.Add(remoteFilter);
			remoteTestSets = spiraSoapClient.TestSet_Retrieve(credentials, 1, remoteFilters.ToArray(), remoteSort, 1, 999999, 1);
		...
		public void _24_RetrieveSettings()
		{
			...
			spiraSoapClient.System_ProcessNotifications();
		}
		...
		public void _25_RetrieveProjectRoles()
		{
			...
			Assert.AreEqual(52, remoteProjectRoles[0].Permissions.Length);
		...
		public void _26_Concurrency_Handling()
		{
			...
			int taskId = spiraSoapClient.Task_Create(credentials, remoteTask).TaskId.Value;
			...
			spiraSoapClient.Task_Update(credentials, remoteTask1);
			...
			try
			{
				...
				spiraSoapClient.Task_Update(credentials, remoteTask2);
			}
			...
			int incidentId = spiraSoapClient.Incident_Create(credentials, remoteIncident).IncidentId.Value;
			...
			spiraSoapClient.Incident_Update(credentials, remoteIncident1);
			...
			try
			{
				...
				spiraSoapClient.Incident_Update(credentials, remoteIncident2);
			}
			catch (FaultException<ServiceFaultMessage> soapException)
			{
				ServiceFaultMessage detail = soapException.Detail;
			...
			spiraSoapClient.Requirement_Update(credentials, remoteRequirement1);
			...
			try
			{
				...
				spiraSoapClient.Requirement_Update(credentials, remoteRequirement2);
			}
			catch (FaultException<ServiceFaultMessage> soapException)
			{
				ServiceFaultMessage detail = soapException.Detail;
			...
			remoteRelease.EndDate = DateTime.UtcNow.AddMonths(3);
			remoteRelease.ResourceCount = 5;
			int releaseId = spiraSoapClient.Release_Create(credentials, remoteRelease, null).ReleaseId.Value;
			...
			spiraSoapClient.Release_Update(credentials, remoteRelease1);
			...
			try
			{
				...
				spiraSoapClient.Release_Update(credentials, remoteRelease2);
			}
			catch (FaultException<ServiceFaultMessage> soapException)
			{
				ServiceFaultMessage detail = soapException.Detail;
			...
			remoteTestSet.TestSetStatusId = (int)TestSet.TestSetStatusEnum.NotStarted;
			remoteTestSet.TestRunTypeId = (int)TestRun.TestRunTypeEnum.Manual;
			int testSetId = spiraSoapClient.TestSet_Create(credentials, remoteTestSet).TestSetId.Value;
			...
			spiraSoapClient.TestSet_Update(credentials, remoteTestSet1);
			...
			try
			{
				...
				spiraSoapClient.TestSet_Update(credentials, remoteTestSet2);
			}
			catch (FaultException<ServiceFaultMessage> soapException)
			{
				ServiceFaultMessage detail = soapException.Detail;
			...
			spiraSoapClient.TestCase_Update(credentials, remoteTestCase1);
			...
			try
			{
				...
				spiraSoapClient.TestCase_Update(credentials, remoteTestCase2);
			}
			catch (FaultException<ServiceFaultMessage> soapException)
			{
				ServiceFaultMessage detail = soapException.Detail;
			...
			remoteTestStep.Position = 1;
			spiraSoapClient.TestCase_AddStep(credentials, remoteTestStep, testCaseId);
		...
		{
			//First lets authenticate and connect to the project
			spiraSoapClient.Connection_Authenticate1("aant", "aant123456789", PLUGIN_NAME);
			...
			remoteSort.SortAscending = true;
			RemoteDocument[] remoteDocuments = spiraSoapClient.Document_RetrieveForFolder(credentials, projectId1, projectAttachmentFolderId, null, remoteSort, 1, 999);
			...
			remoteSort.SortAscending = true;
			remoteDocuments = spiraSoapClient.Document_RetrieveForFolder(credentials, projectId1, projectAttachmentFolderId, new RemoteFilter[] { remoteFilter }, remoteSort, 1, 999);
			...
			remoteDocuments = spiraSoapClient.Document_Retrieve(credentials, projectId1, new RemoteFilter[] { remoteFilter }, remoteSort, 1, 999);
			...
			remoteSort.PropertyName = "UploadDate";
			remoteSort.SortAscending = false;
			remoteDocuments = spiraSoapClient.Document_RetrieveForArtifact(credentials, projectId1, (int)DataModel.Artifact.ArtifactTypeEnum.TestCase, testCaseId1, null, remoteSort);
			...
			remoteSort.SortAscending = true;
			remoteDocuments = spiraSoapClient.Document_RetrieveForArtifact(credentials, projectId1, (int)DataModel.Artifact.ArtifactTypeEnum.TestCase, testCaseId1, new RemoteFilter[] { remoteFilter }, remoteSort);
			...
			spiraSoapClient.Document_DeleteFromArtifact(credentials, projectId1, attachmentId2, (int)DataModel.Artifact.ArtifactTypeEnum.TestCase, testCaseId1);
		...
		public void _30_CreateRetrieveUpdateAssociations()
		{
			...
			remoteSort.SortAscending = true;
			RemoteAssociation[] remoteAssociations = spiraSoapClient.Association_RetrieveForArtifact(credentials, projectId1, (int)DataModel.Artifact.ArtifactTypeEnum.Incident, incidentId1, null, remoteSort);
			...
			remoteAssociations = spiraSoapClient.Association_RetrieveForArtifact(credentials, projectId1, (int)DataModel.Artifact.ArtifactTypeEnum.Incident, incidentId1, new RemoteFilter[] { remoteFilter }, remoteSort);
			...
			spiraSoapClient.Association_Delete(credentials, projectId1, (int)remoteAssociation.ArtifactLinkId);
			//Verify it no longer exists
			remoteAssociations = spiraSoapClient.Association_RetrieveForArtifact(credentials, projectId1, (int)DataModel.Artifact.ArtifactTypeEnum.Incident, incidentId1, new RemoteFilter[] { remoteFilter }, remoteSort); Assert.AreEqual(0, remoteAssociations.Length);
		}
		...
		public void _31_AutomatedTestLauncher()
		{
			...
			spiraSoapClient.TestCase_AddUpdateAutomationScript(credentials, projectId1, testCaseId1, AUTOMATION_ENGINE_ID_QTP, "file:///c:/mytests/test1.qtp", "", null, "1.0", null, null);
			spiraSoapClient.TestCase_AddUpdateAutomationScript(credentials, projectId1, testCaseId2, AUTOMATION_ENGINE_ID_QTP, "test2.qtp", "", binaryData, "1.0", null, null);
			spiraSoapClient.TestCase_AddUpdateAutomationScript(credentials, projectId1, testCaseId3, AUTOMATION_ENGINE_ID_QTP, "file:///c:/mytests/test3.qtp", "", null, "1.0", null, null);
			...
			int automationHostId = spiraSoapClient.AutomationHost_Create(credentials, remoteAutomationHost).AutomationHostId.Value;
			...
			spiraSoapClient.TestSet_Update(credentials, remoteTestSet);
			...
			//Verify that testCaseId1 has the default parameter value specified (new to the 4.0 and later APIs)
            ...
            spiraSoapClient.TestSet_SetInUseStatus(credentials, projectId1, remoteAutomatedTestRun.TestSetId.Value, remoteAutomatedTestRun.TestSetTestCaseId.Value, true);
            ...
            spiraSoapClient.TestSet_SetInUseStatus(credentials, projectId1, remoteAutomatedTestRun.TestSetId.Value, remoteAutomatedTestRun.TestSetTestCaseId.Value, false);
            isInUse = spiraSoapClient.TestSet_CheckInUseStatus(credentials, projectId1, remoteAutomatedTestRun.TestSetId.Value, remoteAutomatedTestRun.TestSetTestCaseId.Value);
            Assert.IsFalse(isInUse);
			...
			remoteAutomatedTestRun = spiraSoapClient.TestRun_RecordAutomated1(credentials, remoteAutomatedTestRun);
			...
			Assert.IsNull(spiraSoapClient.AutomationHost_RetrieveById(credentials, projectId1, automationHostId));
		}
		...
		public void _32_ImportRetrieveArtifactComments()
		{
			...
			spiraSoapClient.Requirement_CreateComment(credentials, projectId1, remoteComment);
			//Verify the data
			...
			remoteComment.Text = "This is a comment";
			spiraSoapClient.Release_CreateComment(credentials, projectId1, remoteComment);
			//Verify the data
			...
			remoteComment.Text = "This is a comment";
			spiraSoapClient.Task_CreateComment(credentials, projectId1, remoteComment);
			//Verify the data
			...
			remoteComment.Text = "This is a comment";
			spiraSoapClient.TestSet_CreateComment(credentials, projectId1, remoteComment);
			//Verify the data
			...
			remoteComment.Text = "This is a comment";
			spiraSoapClient.TestCase_CreateComment(credentials, projectId1, remoteComment);
			//Verify the data
		...
		public void _33_DataMappingChanges()
		{
			...
			int userId = spiraSoapClient.User_Create(credentials, remoteUser, "test123", "What is 3+4?", "7", projectId1, 1).UserId.Value;
		...
		public void _34_RetrieveAutomationHostsAndEngines()
		{
			...
			remoteSort.PropertyName = "Name";
			remoteSort.SortAscending = true;
			RemoteAutomationHost[] remoteAutomationHosts = spiraSoapClient.AutomationHost_Retrieve(credentials, projectId1, null, remoteSort, 1, 9999);
			...
			remoteAutomationHost.Active = true;
			int automationHostId1 = spiraSoapClient.AutomationHost_Create(credentials, remoteAutomationHost).AutomationHostId.Value;
			...
			remoteAutomationHost.Active = true;
			int automationHostId2 = spiraSoapClient.AutomationHost_Create(credentials, remoteAutomationHost).AutomationHostId.Value;

			//Verify that we can retrieve them
			remoteAutomationHosts = spiraSoapClient.AutomationHost_Retrieve(credentials, projectId1, null, remoteSort, 1, 9999);
			...
			remoteAutomationHosts = spiraSoapClient.AutomationHost_Retrieve(credentials, projectId1, new RemoteFilter[] { remoteFilter }, remoteSort, 1, 9999);
			...
			spiraSoapClient.AutomationHost_Update(credentials, remoteAutomationHost);
		...
		public void _35_RetrieveDocumentFoldersTypesAndVersions()
		{
			...
			int folderId1 = spiraSoapClient.Document_AddFolder(credentials, remoteDocumentFolder).ProjectAttachmentFolderId.Value;
			...
			int folderId2 = spiraSoapClient.Document_AddFolder(credentials, remoteDocumentFolder).ProjectAttachmentFolderId.Value;
			remoteDocumentFolder = new RemoteDocumentFolder();
            remoteDocumentFolder.ProjectId = projectId1;
            remoteDocumentFolder.Name = "Documents";
			remoteDocumentFolder.ParentProjectAttachmentFolderId = folderId1;
			int folderId3 = spiraSoapClient.Document_AddFolder(credentials, remoteDocumentFolder).ProjectAttachmentFolderId.Value;
			...
			remoteDocumentFolder.ParentProjectAttachmentFolderId = oldRootFolder;
			spiraSoapClient.Document_UpdateFolder(credentials, remoteDocumentFolder);
			...
			int attachmentId1 = spiraSoapClient.Document_AddFile(credentials, remoteDocument, attachmentData).AttachmentId.Value;
			...
			spiraSoapClient.Document_AddFileVersion(credentials, projectId1, remoteDocumentVersion, attachmentData2, true);
			...
			remoteDocument.ProjectAttachmentFolderId = folderId2;
			remoteDocument.CurrentVersion = "1.0";
			int attachmentId2 = spiraSoapClient.Document_AddUrl(credentials, remoteDocument).AttachmentId.Value;
			...
			remoteDocumentVersion.VersionNumber = "2.0";
			remoteDocumentVersion.AuthorId = USER_ID_JOE_SMITH;
			spiraSoapClient.Document_AddUrlVersion(credentials, projectId1, remoteDocumentVersion, true);
			...
			remoteDocument = spiraSoapClient.Document_RetrieveById(credentials, projectId1, attachmentId2);
			Assert.AreEqual(1, remoteDocument.Versions.Length);
		...
		public void _36b_RequirementTestStepAssociations()
		{
			...
			RemoteTestStep testStep = spiraSoapClient.TestCase_RetrieveStepById(credentials, projectId1, testStepId1);
		...
		public void _37_DeleteItems()
		{
            ...
            Assert.IsFalse(spiraSoapClient.User_RetrieveById(credentials, remoteUser.UserId.Value).Active);
		...
		public void _38_BuildManagement()
		{
			...
			remoteBuild1 = spiraSoapClient.Build_Create(credentials, remoteBuild1);
			//Build 2
			...
			remoteBuild2 = spiraSoapClient.Build_Create(credentials, remoteBuild2);
			...
			RemoteBuild[] remoteBuilds = spiraSoapClient.Build_RetrieveByReleaseId(credentials, projectId1, iterationId2, null, null, 1, 15);
			...
			remoteBuilds = spiraSoapClient.Build_RetrieveByReleaseId_NoDescription(credentials, projectId1, iterationId2, null, null, 1, 15).OrderBy(b => b.Name).ToArray();
			Assert.AreEqual(2, remoteBuilds.Length);
			...
			remoteBuilds = spiraSoapClient.Build_RetrieveByReleaseId(credentials, projectId1, iterationId2, remoteFilters.ToArray(), remoteSort, 1, 15);
			...
			RemoteTestRun[] remoteTestRuns = spiraSoapClient.TestRun_Retrieve(credentials, projectId1, remoteFilters.ToArray(), remoteSort, 1, 15);
			...
			remoteSort.SortAscending = true;
			RemoteIncident[] remoteIncidents = spiraSoapClient.Incident_Retrieve(credentials, projectId1, remoteFilters.ToArray(), remoteSort, 1, 15);
			Assert.AreEqual(1, remoteTestRuns.Length);
			Assert.AreEqual(remoteBuild1.BuildId, remoteTestRuns[0].BuildId.Value);
		}
		...
		public void _39_MoveArtifacts()
		{
			...
			remoteRequirement.RequirementTypeId = rq_typeFeatureId;
			remoteRequirement = spiraSoapClient.Requirement_Create2(credentials, remoteRequirement, null);
			...
			remoteTestStep.ExpectedResult = "It should work";
			remoteTestStep.SampleData = "test 123";
			remoteTestStep = spiraSoapClient.TestCase_AddStep(credentials, remoteTestStep, testCaseId);
			...
			Assert.IsTrue(spiraSoapClient.TestCase_RetrieveByFolder(credentials, projectId1, testFolderId1, null, remoteSort, 1, 999999, null).Any(t => t.TestCaseId == testCaseId));
			//Now move back to the end of the root
			spiraSoapClient.TestCase_Move(credentials, projectId1, testCaseId, null);
			//Verify
			Assert.IsFalse(spiraSoapClient.TestCase_RetrieveByFolder(credentials, projectId1, testFolderId1, null, remoteSort, 1, 999999, null).Any(t => t.TestCaseId == testCaseId));

			//Now move the test set under a folder
			spiraSoapClient.TestSet_Move(credentials, projectId1, testSetId, testSetFolderId1);
			//Verify
			RemoteTestSet[] testSets = spiraSoapClient.TestSet_RetrieveByFolder(credentials, projectId1, testSetFolderId1, null, remoteSort, 1, 999999, null);
			Assert.AreEqual(2, testSets.Length);
			//Now move back to the end of the list
			spiraSoapClient.TestSet_Move(credentials, projectId1, testSetId, null);
			//Verify
			testSets = spiraSoapClient.TestSet_RetrieveByFolder(credentials, projectId1, testSetFolderId1, null, remoteSort, 1, 999999, null);
		...
		public void _40_CheckCounts()
		{
			...
			long numInc = spiraSoapClient.Incident_Count(credentials, projectId1, null);
			long numReq = spiraSoapClient.Requirement_Count(credentials, projectId1, null);
			long numRel = spiraSoapClient.Release_Count(credentials, projectId1, null);
			long numTsk = spiraSoapClient.Task_Count(credentials, projectId1, null);
			long numTC = spiraSoapClient.TestCase_Count(credentials, projectId1, null, null);
			long numTS = spiraSoapClient.TestSet_Count(credentials, projectId1, null, null);
			long numTR = spiraSoapClient.TestRun_Count(credentials, projectId1, null);
			...
			Assert.AreEqual(8, numTR);
		}

		/// <summary>Tests the various functions in the DataSync API</summary>
		[Test, SpiraTestCase(1886)]
		public void _41_DataSyncApi()
		{
			//First authenticate the DataSync API
			RemoteCredentials credentials = spiraSoapClient.Connection_Authenticate1("administrator", "Welcome123$", PLUGIN_NAME);
			Assert.IsNotNull(credentials);
			...
			credentials = spiraSoapClient.Connection_Authenticate1("administrator", "Welcome123$", "UnitTest");
			Assert.IsNotNull(credentials);
		...
		public void _42_SourceCodeQueries()
		{
			try
			{
				...
				remoteSort.PropertyName = "Name";
				remoteSort.SortAscending = true;
				revisions = spiraSoapClient.SourceCode_RetrieveRevisions(credentials, PROJECT_ID, branchId, 1, 15, remoteSort, null);
				Assert.IsTrue(revisions.Length > 0);
				Assert.IsNotNullOrEmpty(revisions[0].Id);
				Assert.IsNotNullOrEmpty(revisions[0].Name);
				Assert.IsNotNullOrEmpty(revisions[0].Message);
		...
		public void _43_InstantMessenger()
		{
			...
			remoteUser = spiraSoapClient.User_Create(credentials, remoteUser, "PleaseChange", "What is 1+1?", "2", projectId1, 1);
			...
			credentials = spiraSoapClient.Connection_Authenticate2("imuser1", "{C9C569C6-2BBF-47C2-B498-1238F8A7A0AF}", "NUnit");
			Assert.IsNotNull(credentials);
            ...
            credentials = spiraSoapClient.Connection_Authenticate2("imuser2", "{78197F7D-2166-49B0-BE7D-AD13FEBB4858}", "NUnit");
            Assert.IsNotNull(credentials);
            messageInfo = spiraSoapClient.Message_GetInfo(credentials);
			Assert.AreEqual(0, messageInfo.UnreadMessages);
			Assert.IsTrue(messageInfo.OnlineUsers.Length > 0);
			Assert.IsTrue(messageInfo.OnlineUsers.Any(u => u == imUser2));
			spiraSoapClient.Message_PostNew(credentials, imUser1, "This is a message 1 from user 2");
		...
		public void _44_RetrieveArtifactHistory()
		{
			...
			remoteSort.SortAscending = false;
			RemoteHistoryChange[] changes = spiraSoapClient.History_RetrieveForArtifact(credentials, 1, /*Requirement*/1, 4, 1, Int32.MaxValue, remoteSort, null);
			...
			changes = spiraSoapClient.History_RetrieveForArtifact(credentials, 1, /*Requirement*/1, 4, 1, Int32.MaxValue, remoteSort, new RemoteFilter[] { remoteFilter });
        ...
        public void _47_ProjectTemplates()
        {
            ...
            projectTemplate = spiraSoapClient.ProjectTemplate_Create(credentials, projectTemplate, null);
            ...
            projectTemplate.IsActive = true;
            projectTemplate = spiraSoapClient.ProjectTemplate_Create(credentials, projectTemplate, projectTemplateId4);
            ...
            spiraSoapClient.ProjectTemplate_Update(credentials, projectTemplate);
            ...
            RemoteTaskStatus[] taskStatuses = spiraSoapClient.Task_RetrieveStatuses(credentials);
...
group by R.EXECUTION_STATUS_NAME";
            ...
            Assert.AreEqual(4, tableData.Rows.Length);
            ...
            Guid reportRequestGuid = spiraSoapClient.Reports_GenerateSavedReport(credentials, PROJECT_ID, reportSavedId);
		...
		public void _49_Import_Retrieve_Risks()
		{
			...
			int riskId1 = spiraSoapClient.Risk_Create(credentials, remoteRisk).RiskId.Value;
			...
			remoteRisk.CreatorId = USER_ID_FRED_BLOGGS;
			remoteRisk.CreationDate = DateTime.UtcNow;
			int riskId2 = spiraSoapClient.Risk_Create(credentials, remoteRisk).RiskId.Value;
			...
			//Get all the risks
			RemoteSort remoteSort = new RemoteSort();
			remoteSort.PropertyName = "RiskId";
			remoteSort.SortAscending = true;
			RemoteRisk[] risks = spiraSoapClient.Risk_Retrieve(credentials, projectId1, null, remoteSort, 1, Int32.MaxValue);
			Assert.AreEqual(2, risks.Length);
			long count = spiraSoapClient.Risk_Count(credentials, projectId1, null);
			...
			remoteSort = new RemoteSort();
			remoteSort.PropertyName = "RiskStatusName";
			remoteSort.SortAscending = true;
			risks = spiraSoapClient.Risk_Retrieve(credentials, projectId1, remoteFilters.ToArray(), remoteSort, 1, Int32.MaxValue);
			...
			remoteRisk.ReleaseId = releaseId1;
			remoteRisk.ComponentId = componentId1;
			spiraSoapClient.Risk_Update(credentials, remoteRisk);
			...
			remoteSort.PropertyName = "RiskId";
			remoteSort.SortAscending = true;
			risks = spiraSoapClient.Risk_Retrieve(credentials, projectId1, null, remoteSort, 1, Int32.MaxValue);
			Assert.AreEqual(1, risks.Length);
			...
			int mitigationId1 = spiraSoapClient.Risk_AddMitigation(credentials, projectId1, remoteRiskMitigation, null, USER_ID_FRED_BLOGGS).RiskMitigationId.Value;
			...
			remoteRiskMitigation.CreationDate = DateTime.UtcNow;
			remoteRiskMitigation.IsActive = true;
			int mitigationId2 = spiraSoapClient.Risk_AddMitigation(credentials, projectId1, remoteRiskMitigation, null, USER_ID_FRED_BLOGGS).RiskMitigationId.Value;
			...
			remoteRiskMitigation = spiraSoapClient.Risk_RetrieveMitigationById(credentials, projectId1, mitigationId1);
			...
			spiraSoapClient.Risk_UpdateMitigation(credentials, projectId1, remoteRiskMitigation);
			...
			spiraSoapClient.Risk_CreateComment(credentials, projectId1, remoteComment);
			...
			remoteRisk.RiskStatusId = rk_statusClosedId;
			remoteRisk.CreationDate = DateTime.UtcNow;
			int riskId3 = spiraSoapClient.Risk_Create(credentials, remoteRisk).RiskId.Value;
		...
		public void _50_Event_Logs()
		{
			...
			int count;
			RemoteSort remoteSort = new RemoteSort();
			remoteSort.PropertyName = "EventTimeUtc";
			remoteSort.SortAscending = false;
			List<RemoteFilter> remoteFilters = new List<RemoteFilter>();

			//Verify can retrieve log without filters
			RemoteEvent2[] remoteEvents = spiraSoapClient.System_RetrieveEvents(credentials, pageIndex + 1, pageSize, remoteSort, remoteFilters.ToArray());
			...
			remoteEvents = spiraSoapClient.System_RetrieveEvents(credentials, pageIndex + 1, pageSize, remoteSort, remoteFilters.ToArray());
			...
			pageIndex = 0;
			remoteSort.PropertyName = "Message";
