<%@ Page Language="c#" CodeBehind="ReleaseWorkflowDetails.aspx.cs" AutoEventWireup="True"
    Inherits="Inflectra.SpiraTest.Web.Administration.ProjectTemplate.ReleaseWorkflowDetails" MasterPageFile="~/MasterPages/Administration.master" %>
...
<asp:Content ContentPlaceHolderID="cplAdministrationContent" runat="server" ID="Content2">
	<div class="container-fluid">
        <div class="row">
            <div class="col-lg-10">
                ...
                <p>
                    <asp:Localize runat="server" Text="<%$Resources:Main,Admin_ReleaseWorkflowDetails_Intro %>" />
                </p>
                <tstsc:GridViewEx ID="grdWorkflowSteps" runat="server" AutoGenerateColumns="False" Width="100%" CssClass="DataGrid WorkflowDiagram workflow-list" ShowFooter="False" ShowHeader="True">
                    ...
                    <Columns>
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,workflow_step_name %>" HeaderStyle-CssClass="priority1" ItemStyle-CssClass="priority1 v-mid">
                            <ItemTemplate>
                                <div class="Step display-inline-block ml4" data-workflowStep="<%# ((Inflectra.SpiraTest.DataModel.ReleaseStatus) Container.DataItem).Name%>">
                                    <tstsc:HyperLinkEx runat="server" CssClass="large" NavigateUrl='<%# UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "ReleaseWorkflowStep") + "?" + GlobalFunctions.PARAMETER_WORKFLOW_ID + "=" + workflowId.ToString() + "&" + GlobalFunctions.PARAMETER_RELEASE_STATUS_ID + "=" + ((Inflectra.SpiraTest.DataModel.ReleaseStatus) Container.DataItem).ReleaseStatusId%>'><%# ((Inflectra.SpiraTest.DataModel.ReleaseStatus) Container.DataItem).Name%></tstsc:HyperLinkEx>
                                    <asp:Label runat="server" Visible='<%# ((Inflectra.SpiraTest.DataModel.ReleaseStatus) Container.DataItem).ReleaseStatusId == (int)Release.ReleaseStatusEnum.Planned%>'
                        ...
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,workflow_transitions %>" HeaderStyle-CssClass="priority1" ItemStyle-CssClass="priority1">
                            <ItemTemplate>
                                ...
                                <asp:Repeater runat="server" OnItemCommand="rptWorkflowStepTransitions_ItemCommand" ID="rptWorkflowStepTransitions">
                                    <ItemTemplate>
                                        <div class="btn-group">
                                            <tstsc:HyperLinkEx Font-Italic="true" SkinID="ButtonDefault" runat="server" NavigateUrl='<%# UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "ReleaseWorkflowTransition") + "?" + GlobalFunctions.PARAMETER_WORKFLOW_ID + "=" + workflowId.ToString() + "&" + GlobalFunctions.PARAMETER_WORKFLOW_TRANSITION_ID + "=" + ((Inflectra.SpiraTest.DataModel.ReleaseWorkflowTransition) Container.DataItem).WorkflowTransitionId%>'><%#: ((Inflectra.SpiraTest.DataModel.ReleaseWorkflowTransition) Container.DataItem).Name%></tstsc:HyperLinkEx>
                                            <tstsc:LinkButtonEx runat="server" CommandName="Delete" CommandArgument='<%# ((Inflectra.SpiraTest.DataModel.ReleaseWorkflowTransition) Container.DataItem).WorkflowTransitionId%>' CausesValidation="false"
                                                ID="Linkbutton2" Tooltip="<%$Resources:Main,Admin_WorkflowDetails_Delete %>">
                                                <span class="fas fa-trash-alt"></span>
                                            </tstsc:LinkButtonEx>
                                        </div>
                                        <span class="fas fa-arrow-right mx4 "></span>
                                        <div class="Step display-inline-block muted" data-workflowStep="<%# ((Inflectra.SpiraTest.DataModel.ReleaseWorkflowTransition)Container.DataItem).OutputReleaseStatus.Name%>">
                                            <tstsc:LabelEx runat="server">
                                                </span> <%#: ((Inflectra.SpiraTest.DataModel.ReleaseWorkflowTransition)Container.DataItem).OutputReleaseStatus.Name%>
                                            </tstsc:LabelEx>
                                        </div>
                                        <div class="Spacer">
                                        </div>
                        ...
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,workflow_status_id %>" HeaderStyle-CssClass="priority4" ItemStyle-CssClass="priority4">
                            <ItemTemplate>
                                <%# String.Format(GlobalFunctions.FORMAT_ID, ((Inflectra.SpiraTest.DataModel.ReleaseStatus)Container.DataItem).ReleaseStatusId)%>
                            </ItemTemplate>
                        </tstsc:TemplateFieldEx>
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Main,Global_Operations %>" HeaderStyle-CssClass="priority1" ItemStyle-CssClass="priority1">
                            <ItemTemplate>
                                <tstsc:HyperLinkEx runat="server" NavigateUrl="javascript:void(0)" ClientScriptMethod='<%# "pnlAddTransition_display(event," + ((Inflectra.SpiraTest.DataModel.ReleaseStatus) Container.DataItem).ReleaseStatusId + ")"%>'
            ...
            <tr>
                ...
                <td>
                    <tstsc:DropDownListEx ID="ddlDestinationStep" runat="server" CssClass="DropDownList"
                        DataTextField="Name" DataValueField="ReleaseStatusId" Width="250px" />
                </td>
            </tr>
            <tr>
                <td>&nbsp;
    ...
    <script type="text/javascript">
        ...
        var incidentStatuses = <%=GetReleaseStatusesAsJson() %>;
