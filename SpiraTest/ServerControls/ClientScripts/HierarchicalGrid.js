...
Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridColumn = function ()
{
	this.$header = null; //Start with $ prefix so that it does not includes in the Serialization used by the JavaScriptSerializer.
}
Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridColumn.registerClass('Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridColumn');

Inflectra.SpiraTest.Web.ServerControls.HierarchicalGrid = function (element)
{
	...
	this._totalCount = 0;
	this._rowIndex = 1;
	this._pageCountLabel = null;
	this._nextLink = null;
	this._lastLink = null;

	//Load in all the passed parameters from the server-control
	Inflectra.SpiraTest.Web.ServerControls.HierarchicalGrid.initializeBase(this, [element]);
}

Inflectra.SpiraTest.Web.ServerControls.HierarchicalGrid.prototype =
{
    ...
    //Refreshes the data rows array (and handlers)
    _refreshTableRows: function ()
    {
        //Clear the handlers and then the rows
        this._clearTableRowHandlers();
        var table = this.get_element();
        delete this._tableRows;
        this._tableRows = new Array();
        var i;
        //Ignore the header, subheader and footer
        for (i = 2; i < table.rows.length - 1; i++)
        {
            ...
            if (primaryKey)
            {
                this._addTableRowHandlers(tr, primaryKey);
            }
    ...
    expandNode: function (e)
    {
        ...
        var img = e.img;
        img.className = 'fas w3 mr2 fa-caret-right ti-0 pointer rotate45 gray js-Expanded';
        img.setAttribute('Tooltip', resx.HierarchicalGrid_CollapseNode);
        img.title = resx.HierarchicalGrid_CollapseNode;

        //Also some artifacts have an alternate expanded icon
        var artifactId = e.primaryKey;
        if (e.thisRef.get_expandedItemImage() != '')
        {
            var prefix = e.thisRef.get_elementId() + '_icon_';
            var icon = document.getElementById(prefix + artifactId);
            if (icon)
            {
                icon.src = e.thisRef._themeFolder + 'Images/' + e.thisRef.get_expandedItemImage();
            }
        }

        //Next call the method to expand the node
        var context = {};
        context.tr = e.tr;
        var webServiceClass = e.webServiceClass;
        e.thisRef.display_spinner();
        webServiceClass.Expand(e.thisRef.get_projectId(), artifactId, Function.createDelegate(e.thisRef, e.thisRef.expandNode_success), Function.createDelegate(e.thisRef, e.thisRef.operation_failure), context);
    },
    expandNode_success: function (dataSource, context)
    {
        //Next we need to add the new rows under the item being expanded
        var selected_tr = context.tr;
        var startIndex = selected_tr.rowIndex - 1;
        var itemCount = -1;    //Retrieve all since we don't know how many exist
        context.startIndex = startIndex;
        var webServiceClass = this.get_webServiceClass();
        webServiceClass.HierarchicalList_RetrieveSelection(this.get_projectId(), this._standardFilters, startIndex, itemCount, Function.createDelegate(this, this.expandNode_success2), Function.createDelegate(this, this.operation_failure), context);
    },
    expandNode_success2: function (dataSource, context)
    {
        //Databind the additional rows (ignore the header row) until we get to the same indent level
        var i;
        var selected_tr = context.tr;
        var start_rowIndex = selected_tr.rowIndex;
        var selected_indent = selected_tr.getAttribute('tst:indent');
        var table = this.get_element();
        var columns = this.get_columns();
        var dataItems = dataSource.items;
        for (i = 1; i < dataItems.length; i++)
        {
            //If we have the same indent level as the selected row, stop
            var dataRow = dataItems[i];
            if (dataRow.indent.length > selected_indent.length && dataRow.indent.substr(0, selected_indent.length) == selected_indent)
            {
                var tr = table.insertRow(start_rowIndex + i);
                this._dataBindRow(dataRow, columns, table, tr, true);
            }
            else
            {
                i = dataItems.length;
        ...
        this._visibleCount = dataSource.visibleCount;
        this._totalCount = dataSource.totalCount;

        //Now we need to remove any additional rows at the end        
        var rows_in_page = dataItems.length + context.startIndex - 1;
        var data_rows_count = table.rows.length - 3;
        var rows_to_delete = data_rows_count - rows_in_page;
        for (i = 0; i < rows_to_delete; i++)
        {
            var index = rows_in_page + 2;
            //Never delete the header or subheader
            if (index >= 2 && index < table.rows.length - 1)
            {
                table.deleteRow(index);
            }
        }
        this._refreshTableRows();
        this._update_display_counts();

        //See if we need to update the pagination count information
        if (dataSource.pageCount != this._pageCount)
        {
            this._updatePageCounts(dataSource.pageCount);
        }
        this.hide_spinner();
    },
    collapseNode: function (e)
    {
        //First we need to change the expand icon
        var img = e.img;
        img.className = 'fas w3 mr2 fa-caret-right pointer ti-0 dark-gray js-Collapsed';
        ...
        if (e.thisRef.get_expandedItemImage() != '')
        {
            ...
            if (icon)
            {
                icon.src = e.thisRef._themeFolder + 'Images/' + e.thisRef.get_summaryItemImage();
            }
        }

        //Call the method to collapse the node
        var context = {};
        context.tr = e.tr;
        var webServiceClass = e.webServiceClass;
        e.thisRef.display_spinner();
        webServiceClass.Collapse(e.thisRef.get_projectId(), artifactId, Function.createDelegate(e.thisRef, e.thisRef.collapseNode_success), Function.createDelegate(e.thisRef, e.thisRef.operation_failure), context);
    },
    collapseNode_success: function (dataSource, context)
    {
        //Next we need to delete the rows under the item being collapsed
        var selected_tr = context.tr;
        var selected_indent = selected_tr.getAttribute('tst:indent');
        var index = selected_tr.rowIndex + 1;
        var table = this.get_element();
        var continueLoop = true;
        var deleteCount = 0;
        var dataRowCount = table.rows.length - 3;   //minus header, subheader and footer
        while (continueLoop)
        {
            if (index < table.rows.length)
            {
                var tr = table.rows[index];
                //Get the indent level attribute
                var indent = tr.getAttribute('tst:indent');
                if (indent && indent.length > selected_indent.length && indent.substr(0, selected_indent.length) == selected_indent)
                {
                    //If we are a child item, delete row
                    table.deleteRow(index);
                    deleteCount++;
                }
                else
        ...
        //Now we need to populate the additional rows at the end to keep the page size constant
        var startIndex = dataRowCount - deleteCount;
        var webServiceClass = this.get_webServiceClass();
        webServiceClass.HierarchicalList_RetrieveSelection(this.get_projectId(), this._standardFilters, startIndex, deleteCount, Function.createDelegate(this, this.collapseNode_success2), Function.createDelegate(this, this.operation_failure));
    },
    collapseNode_success2: function (dataSource)
    {
        //Databind the additional rows
        var i;
        var table = this.get_element();
        var columns = this.get_columns();
        var dataItems = dataSource.items;
    ...
    _updatePageCounts: function (pageCount)
    {
        //We need to change the number of pages label and make the buttons active/inactive
        this._pageCount = pageCount;
        this._pageCountLabel.nodeValue = pageCount;
        if (this._currPage < pageCount)
        {
            this._nextLink.style.cursor = 'pointer';
            this._nextLink.style.filter = '';
            this._nextLink.style.opacity = '';
            this._nextLink.style.mozOpacity = '';
            this._lastLink.style.cursor = 'pointer';
            this._lastLink.style.filter = '';
            this._lastLink.style.opacity = '';
            this._lastLink.style.mozOpacity = '';
        }
        else
        {
            this._nextLink.style.cursor = 'default';
            this._nextLink.style.filter = 'alpha(opacity=50)'; // IE Only
            this._nextLink.style.opacity = '0.5';  //others
            this._nextLink.style.mozOpacity = '0.5'; //FireFox only
            this._lastLink.style.cursor = 'default';
            this._lastLink.style.filter = 'alpha(opacity=50)'; // IE Only
            this._lastLink.style.opacity = '0.5';  //others
            this._lastLink.style.mozOpacity = '0.5'; //FireFox only
        }
    },
    ...
    _load_data: function ()
    {
        ...
        this.get_webServiceClass().HierarchicalList_Retrieve(this.get_projectId(), this._standardFilters, false, Function.createDelegate(this, this.retrieve_success), Function.createDelegate(this, this.operation_failure));
    },
    ...
    operation_failure: function (exception)
    {
        ...
        var messageBox = $get(this.get_errorMessageControlId());
    ...
    set_visibleCountControlId: function (value)
    {
        ...
        if (this._visibleCountControlId != value)
        {
            this._visibleCountControlId = value;
            this.raisePropertyChanged('visibleCountControlId');
        }
    },
    ...
    get_forceTwoLevelIndent: function ()
    {
        return this._forceTwoLevelIndent;
    },
    set_forceTwoLevelIndent: function (value)
    {
        this._forceTwoLevelIndent = value;
    },
    ...
    set_expandedItemImage: function (value)
    {
        ...
        if (this._expandedItemImage != value)
        {
            this._expandedItemImage = value;
            this.raisePropertyChanged('expandedItemImage');
        }
    },
    ...
    {
        var e = Function._validateParams(arguments, [{ name: 'value', type: Array, elementType: Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridColumn, elementMayBeNull: false}]);
        if (e) throw e;
    ...
    initialize: function ()
    {
        Inflectra.SpiraTest.Web.ServerControls.HierarchicalGrid.callBaseMethod(this, 'initialize');
    ...
    dispose: function ()
    {
        ...
        delete this._dragDownHandlers;
        delete this._dragUpHandlers;
        delete this._touchHandlers;
        ...
        Inflectra.SpiraTest.Web.ServerControls.HierarchicalGrid.callBaseMethod(this, 'dispose');
    },
    ...
    dataBind: function (callback)
    {
        ...
        //The first row contains the filter values and columns (not real data)
        dataRow = dataItems[0];
        for (var property in dataRow.Fields)
        {
            if (!property.startsWith('__')) // Excluding the Ajax Framework Properties
            {
                column = new Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridColumn();
        ...
        for (i = 0; i < columns.length; i++)
        {
            ...
            if (column.allowDragAndDrop && this._allowColumnPositioning)
            {
                ...
                var dragSource = $create(Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridColumnBehavior, { columnIndex: i, dataField: column.dataField, 'parent': this }, null, null, th);
            ...
            //Columns are not sortable so render as text
            thDiv.appendChild(document.createTextNode(column.headerText));
        ...
        this._editColumnHeader.appendChild(document.createTextNode(resx.Global_Edit));
        ...
        checkbox.name = this.get_elementId() + '_chkHead';
        ...
        for (i = 0; i < columns.length; i++)
        {
            ...
            if (this._allowColumnPositioning)
            {
                ...
                var dragSource = $create(Inflectra.SpiraTest.Web.ServerControls.HierarchicalResizableColumnBehavior, { parent: this, fieldName: column.dataField }, null, null, tdDiv);
            ...
            if (column.fieldType == globalFunctions._fieldType_text)
            {
                ...
                textbox.name = this.get_elementId() + '_filter_' + column.dataField;
            ...
            {
                var div = d.ce('div');
                div.id = this.get_elementId() + '_filter_' + column.dataField + '_ddl';
                ...
                dropdown = $create(Inflectra.SpiraTest.Web.ServerControls.DropDownList, { multiSelectable: true }, null, null, div);
                dropdown.set_name(this.get_elementId() + '_filter_' + column.dataField);
            ...
            {
                var div = d.ce('div');
                div.id = this.get_elementId() + '_filter_' + column.dataField + '_dat';
                div.className = 'DatePicker';
                div.style.width = '90px';
                var datePicker = $create(Inflectra.SpiraTest.Web.ServerControls.DateRangeFilter, null, null, null, div);
                datePicker.set_name(this.get_elementId() + '_filter_' + column.dataField);
            ...
            if (column.fieldType == globalFunctions._fieldType_identifier)
            {
                ...
                tdDiv.appendChild(document.createTextNode(this.get_artifactPrefix() + ' '));
                textbox = d.ce('input');
        ...
        else
        {
            ...
            btn.style.filter = 'alpha(opacity=50)'; // IE Only
            btn.style.opacity = '0.5';  //others
            btn.style.mozOpacity = '0.5'; //FireFox only
        }
    ...
    _dataBind_Process: function (dataSource, columns, table, tBody, thead, callback, thisRef)
    {
        ...
        while (thisRef._rowIndex < dataItems.length)
        {
            ...
            thisRef._dataBindRow(dataRow, columns, table, tr, true);
            internalCount++;
            thisRef._rowIndex++;
            if (internalCount == 10)
            {
        ...
        else
        {
            ...
            thisRef._dataBind_footer(dataSource, columns, table, tBody, thead, callback);
        }
    },
    _dataBind_footer: function (dataSource, columns, table, tBody, thead, callback)
    {
        var tr = d.ce('tr');
        ...
        //The footer can have items dragged, means move the item to the end of the list
        $create(Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridRowBehavior, { 'parent': this }, null, null, tr);
        ...
        div.appendChild(document.createTextNode(resx.Global_Show + '\u00a0'));
        div = d.ce('div');
        this._ddlPaginationOptions = $create(Inflectra.SpiraTest.Web.ServerControls.DropDownList, null, null, null, div);
        this._ddlPaginationOptions.set_name('ddlPaginationOptions');
        //Add event handler
        ...
        div = d.ce('div');
        divDisplay.appendChild(div);
        div.appendChild(document.createTextNode('\u00a0' + resx.Global_RowsPerPage));
    ...
    _onTableRowKeyDown: function (evt, e)
    {
        ...
        if (e.thisRef._isInEdit)
        {
            if (keynum == 13)
            {
                //Unfocus the text (needed in IE10)
                window.focus();
    ...
    _update_display_counts: function ()
    {
        //Display the counts if the display controls exist
        if (document.getElementById(this.get_totalCountControlId()))
        {
            var countSpan = document.getElementById(this.get_totalCountControlId());
            countSpan.innerHTML = this._totalCount + ' ';
        }
        if (document.getElementById(this.get_visibleCountControlId()))
        {
            var countSpan = document.getElementById(this.get_visibleCountControlId());
            countSpan.innerHTML = this._visibleCount + ' ';
        }
    },
    _dataBindRow: function (dataRow, columns, table, tr, addDragHandlers)
    {
        var td;
        var a;
        var img;
        ...
        {
            //Need to hook the mousedown event for drag n drop
            if (this._allowEdit && !dataRow.readOnly)
            {
                var dragSource = $create(Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridRowBehavior, { 'primaryKey': dataRow.primaryKey, 'parent': this }, null, null, tr);
        ...
        tr.setAttribute('tst:indent', dataRow.indent);
        ...
        checkbox = d.ce('input');
        checkbox.type = 'checkbox';
        checkbox.name = this.get_elementId() + '_chkItem' + dataRow.primaryKey;
        checkbox.setAttribute('tst:controlid', this.get_elementId());
        checkbox.setAttribute('tst:primarykey', dataRow.primaryKey);
        td.appendChild(checkbox);

        //Add the checkbox handler
        if (!dataRow.notSelectable)
        {
            var _checkboxChangedHandler = Function.createCallback(this._onCheckboxChanged, { thisRef: this, isSummary: isSummary, tr: tr, checkbox: checkbox });
        ...
        for (var j = 0; j < columns.length; j++)
        {
            ...
            //If we have a name/description column we need to add the indent and the various images and buttons
            if (column.fieldType == globalFunctions._fieldType_nameDescription)
            {
                //Set the primarykey of the row, used for drag and drop editing
                td.className = 'priority1';
                td.setAttribute('tst:primarykey', dataRow.primaryKey);
                //First the indent level
                var indentPosition = dataRow.indent.length / 3;
                if (this._forceTwoLevelIndent && indentPosition > 1)
                {
                    if (dataRow.summary)
                    {
                        indentPosition = 1;
                ...
                tdDiv.style.paddingLeft = ((indentPosition - 1) * 10) + 'px';

                //Next the various icons and buttons
                img = d.ce('span');
                if (dataRow.summary)
                {
                    if (dataRow.expanded)
                    ...
                    {
                        img.className = 'fas w3 mr2 fa-caret-right ti-0 pointer dark-gray js-Collapsed';
                        img.setAttribute('Tooltip', resx.HierarchicalGrid_ExpandNode);
                        img.title = resx.HierarchicalGrid_ExpandNode;
                    }
                    //Add the expand/collapse node handler
                    var _onExpandCollapseNodeHandler = Function.createCallback(this._onExpandCollapseNode, { thisRef: this, primaryKey: dataRow.primaryKey, img: img, tr: tr, webServiceClass: this.get_webServiceClass() });
                ...
                {
                    img.alt = dataRow.Fields[column.dataField].textValue;
                }

                if (dataRow.summary)
                {
                    if (dataRow.expanded && this.get_expandedItemImage() != '')
                    {
                        img.src = this._themeFolder + 'Images/' + this.get_expandedItemImage();
                    }
                    else
                    {
                        img.src = this._themeFolder + 'Images/' + this.get_summaryItemImage();
                    }
                }
                else
                {
                    ...
                    {
                        img.src = this._themeFolder + 'Images/' + this.get_alternateItemImage();
                    }
                    else
                    {
                        img.src = this._themeFolder + 'Images/' + this.get_itemImage();
                    }
                }
                tdDiv.appendChild(img);
                tdDiv.appendChild(document.createTextNode(' '));
            ...
            if ((column.dataField != null) && (column.dataField.length > 0))
            {
                //If summary make bold
                td.style.fontWeight = (isSummary) ? 'bold' : 'normal';
                ...
                else
                {
                    ...
                    if (column.dataField.indexOf('Yn') != -1)
                    {
                        if (value == 'Y')
                        {
                            value = resx.Global_Yes;
                        }
                        if (value == 'N')
                        {
                            value = resx.Global_No;
                        }
                    }
                    ...
                    if (value != null)
                    {
                        ...
                        if (column.fieldType == globalFunctions._fieldType_nameDescription)
                        {
                            ...
                            {
                                a.href = this.get_baseUrl().replace(globalFunctions.artifactIdToken, '' + dataRow.primaryKey);
                            }
                            tdDiv.appendChild(a);
                        ...
                        {
                            tdDiv.appendChild(document.createTextNode(value));
                        }
                    }
                }
    ...
    _clearTableRowHandlers: function ()
    {
        if (this._allowEdit)
        {
            ...
            {
                $removeHandler(this._tableRows[i], 'dblclick', this._tableRowDblClickHandlers[i]);
                $removeHandler(this._tableRows[i], 'keydown', this._tableRowKeyDownHandlers[i]);
                $removeHandler(this._tableRows[i], 'contextmenu', this._tableRowContextMenuHandlers[i]);
    ...
    _onCheckboxChanged: function (evt, e)
    {
        ...
        var prefix = e.thisRef.get_elementId() + '_chkItem';
        for (i = 0; i < frm.length; i++)
        {
            element = frm.elements[i];
            if (element.type == 'checkbox' && element.name.indexOf(prefix) != -1)
            {
                if (!element.checked)
                {
        ...
        {
            element = frm.elements[i];
            if (element.type == 'checkbox' && element.name == (e.thisRef.get_elementId() + '_chkHead'))
            {
    ...
    _onCheckboxSelectAllChanged: function (evt, e)
    {
        ...
        {
            element = frm.elements[i];
            if (element.type == 'checkbox' && element.name.indexOf(prefix) != -1 && !element.disabled)
            {
            ...
            for (var j = 0; j < this._tableRows.length; j++)
            {
                ...
                if (primaryKey)
                {
                    ...
                    if (artifactId != -1)
                    {
                        var webServiceClass = this.get_webServiceClass();
                        this.display_spinner();
                        webServiceClass.Refresh(this.get_projectId(), artifactId, Function.createDelegate(this, this.edit_success), Function.createDelegate(this, this.operation_failure), tr);
                    }
                }
            }
        }
    ...
    set_filter: function (newFilterName, newFilterValue, keepExisting)
    {
        ...
        this.display_spinner();
        var webServiceClass = this.get_webServiceClass();
        webServiceClass.UpdateFilters(this.get_projectId(), filterValues, null, Function.createDelegate(this, this.operation_success), Function.createDelegate(this, this.operation_failure));
    },
    ...
    _get_selected_items: function ()
    {
        ...
        var i;
        var element;
        var prefix = this.get_elementId() + '_chkItem';
        for (i = 0; i < frm.length; i++)
        {
            element = frm.elements[i];
            if (element.type == 'checkbox' && element.name.indexOf(prefix) != -1)
            {
                if (element.checked)
                {
                    ...
                    var primaryKey = element.name.substring(prefix.length, element.name.length);
    ...
    //Opens up a new item
    open_item: function (target, primaryKey)
    {
    ...
    insert_item: function (artifact)
    {
        //Make sure that no more than one is selected
        this._lastItemType = artifact;
		var items = this._get_selected_items();
		//If no item was selected on the grid, check if we are storing a selected id in state, and if so, add this to the (empty) items array
		//This is important for when the users inserted an item, then hit Save and New - so the 2nd item is added in the correct place (see [IN:4878])
		if (items.length == 0 && this._lastSelectedArtifactId > 0) {
			items.push(this._lastSelectedArtifactId);
		}
        if (items.length <= 1)
        {
            ...
            { 
                ...
                {
					artifactId = items[0];
					//Update the select artifact id with the item (this could be the existing _lastSelectedArtifactId (useless) or the single selected item on the grid (useful))
					this._lastSelectedArtifactId = artifactId;
                }
                //Call the appropriate webservice to insert the items
                var webServiceClass = this.get_webServiceClass();
                this.display_spinner();
                webServiceClass.HierarchicalList_Insert(this.get_projectId(), this._standardFilters, artifactId, artifact, Function.createDelegate(this, this.insert_success), Function.createDelegate(this, this.operation_failure));

            }
        }
        else
        {
            alert(resx.Global_SelectOneCheckBoxForInsert);
        }
    },
    insert_success: function (artifactId)
    {
        ...
        if (this._allowEdit)
        {
            //Now we need to get an updated set of indent levels so that we can correctly add the new item
            var webServiceClass = this.get_webServiceClass();
            webServiceClass.RetrieveHierarchy(this.get_projectId(), this._standardFilters, Function.createDelegate(this, this.insert_success2), Function.createDelegate(this, this.operation_failure), artifactId);
        }
        else
        {
			this.hide_spinner();
			//If we are no longer editing, then we need to clear out the last selected artifact id
			this._lastSelectedArtifactId = -1;
            var url = this._baseUrl.replace(globalFunctions.artifactIdToken, '' + artifactId);
            window.open(url, '_self');
        }
    },
    insert_success2: function (hierarchyList, artifactId)
    {
        //First get the hierarchy indent of the new item to add
        var table = this.get_element();
        var new_indent = hierarchyList[globalFunctions.keyPrefix + artifactId];
        var new_index = -1;

        //If we are adding beyond the current page limit, don't display
        if (new_indent)
        {
            var i;
            ...
            for (i = 2; i < table.rows.length - 1; i++)
            {
                ...
                if (primaryKey)
                {
                    var update_indent = hierarchyList[globalFunctions.keyPrefix + primaryKey];
                    if (update_indent)
                    {
                        //Update the indent level of this row
                        if (tr.cells.length > 0)
                        {
                            tr.setAttribute('tst:indent', update_indent);
                        }
                        //See if we have passed the indent of the new item
                        if (update_indent > new_indent && !match_found)
                        {
                            new_index = i;
                            match_found = true;
                        }
            ...
            //Add the new row 0 if no match, just add at the end
            if (new_index == -1)
            {
                new_index = table.rows.length - 1;
            }
            var tr = table.insertRow(new_index);
            tr.setAttribute('tst:primarykey', artifactId);
            this._refreshTableRows();
            this._lastEditWasInsert = true;
            //Now need to get a refreshed editable copy of the particular row's data using a web service call
            var webServiceClass = this.get_webServiceClass();
            webServiceClass.Refresh(this.get_projectId(), artifactId, Function.createDelegate(this, this.edit_success), Function.createDelegate(this, this.operation_failure), tr);
        }
        else
        {
			this.hide_spinner();
			//Because this item is on the next page, there is no save and new, so reset the last selected artifact to avoid confusion
			this._lastSelectedArtifactId = -1;
        }
    },
    ...
    delete_items: function ()
    {
        ...
        if (items.length > 0)
        {
            ...
            this.display_spinner();
            var webServiceClass = this.get_webServiceClass();
            webServiceClass.Delete(this.get_projectId(), items, Function.createDelegate(this, this.operation_success), Function.createDelegate(this, this.operation_failure));
        }
        else
        {
    ...
    expand_to_level: function (select)
    {
        if (select.get_value() != '')
        {
            var level = -1;
            if (select.get_value() > 0)
            {
                level = select.get_value();
            }
            //Call the appropriate webservice to set the indent levelf
            var webServiceClass = this.get_webServiceClass();
            this.display_spinner();
            webServiceClass.ExpandToLevel(this.get_projectId(), level, this._standardFilters, Function.createDelegate(this, this.operation_success), Function.createDelegate(this, this.operation_failure));

            //Now reset the dropdown
            select.get_parent().set_selectedItem('');
        }
    },
    indent_items: function ()
    {
        //Make sure at least one item is selected
        var items = this._get_selected_items();
        if (items.length > 0)
        {
            //Call the appropriate webservice to indent the items
            this.display_spinner();
            var webServiceClass = this.get_webServiceClass();
            webServiceClass.Indent(this.get_projectId(), items, Function.createDelegate(this, this.operation_success), Function.createDelegate(this, this.operation_failure));
        }
        else
        {
            alert(resx.HierarchicalGrid_SelectOneCheckBoxForIndent);
        }
    },
    outdent_items: function ()
    {
        //Make sure at least one item is selected
        var items = this._get_selected_items();
        if (items.length > 0)
        {
            ...
            this.display_spinner();
            var webServiceClass = this.get_webServiceClass();
            webServiceClass.Outdent(this.get_projectId(), items, Function.createDelegate(this, this.operation_success), Function.createDelegate(this, this.operation_failure));
        }
        else
        {
            alert(resx.HierarchicalGrid_SelectOneCheckBoxForOutdent);
        }
    },
    move_items: function (sourceItems, destId)
    {
        //Make sure that the source and destination are not the same and are both valid
        if (sourceItems.length == 0)
        {
            alert(resx.Global_ItemDraggedNotValid);
        }
        else if (destId == -1)
        {
            alert(resx.Global_DestinationItemNotValid);
        }
        else
        {
            //Make sure the the destination isn't in the source list
            for (var i = 0; i < sourceItems.length; i++)
            {
                if (sourceItems[i] == destId)
                {
                    //alert(resx.Global_CannotMoveItemOntoItself);
                    return;
                }
            }

            //Call the appropriate webservice to move the items
            this.display_spinner();
            var webServiceClass = this.get_webServiceClass();
            webServiceClass.Move(this.get_projectId(), sourceItems, destId, Function.createDelegate(this, this.operation_success), Function.createDelegate(this, this.operation_failure));
        }
    },
    copy_items: function ()
    {
    ...
    {
        //Make sure no more than one item is selected (or none to paste at the end)
        var items = this._get_selected_items();
        if (items.length > 1)
        {
            alert(resx.Global_SelectOneCheckBoxForPaste);
        }
        else
        {
            ...
            else
            {
                ...
                {
                    //See if we have a destination item selected
                    var destId = -1;
                    if (items.length > 0)
                    {
                        destId = items[0];
                    ...
                    {
                        this.display_spinner();
                        webServiceClass.Copy(this.get_projectId(), this._clipboard_items, destId, Function.createDelegate(this, this.operation_success), Function.createDelegate(this, this.operation_failure));
                    }
                    else if (this._clipboard_type == 'Move')
                    {
                        this.display_spinner();
                        webServiceClass.Move(this.get_projectId(), this._clipboard_items, destId, Function.createDelegate(this, this.operation_success), Function.createDelegate(this, this.operation_failure));
                    }
                }
            }
        }
    },
    export_items: function (operation, dialogTitle, dialogText, dialogButton)
    {
        //Make sure at least one item is selected
        var items = this._get_selected_items();
        var context = {};
        context.operation = operation;
        context.title = dialogTitle;
        context.text = dialogText;
    ...
    export_items_success: function (projectList, context)
    {
        ...
        var text = document.createTextNode(context.title);
        h1.appendChild(text);
        text = document.createTextNode(context.text);
        ...
        a.appendChild(document.createTextNode(context.button));
        div.appendChild(a);
        a = d.ce('a');
        a.href = 'javascript:void(0)';
        a.className = 'btn btn-default';
        ...
        a.appendChild(document.createTextNode(resx.Global_Cancel));
        div.appendChild(a);
    },
    ...
    _onExportConfirmClick: function (sender, e)
    {
        ...
        else
        {
            ...
            var webServiceClass = e.thisRef.get_webServiceClass();
            e.thisRef.display_spinner();
            webServiceClass.Export(e.thisRef.get_projectId(), destProjectId, e.thisRef._get_selected_items(), Function.createDelegate(e.thisRef, e.thisRef.export_confirm_success), Function.createDelegate(e.thisRef, e.thisRef.operation_failure));
        }
    },
    ...
    retrieve_filter_success: function (filterList)
    {
        ...
        var h1 = d.ce('h1');
        panel.appendChild(h1);
        var text = document.createTextNode(resx.Global_RetrieveSavedFilter);
        h1.appendChild(text);
        text = document.createTextNode(resx.Global_PleaseSelectFilter);
        panel.appendChild(text);
        //The project dropdown
        var para = d.ce('p');
        panel.appendChild(para);
        ...
        a.appendChild(document.createTextNode(resx.Global_ApplyFilter));
        div.appendChild(a);
        a = d.ce('a');
        a.href = 'javascript:void(0)';
        a.className = 'btn btn-default';
    ...
    //Handles page-specific operations where we want to act on the selected items in the grid
    custom_list_operation: function (operation, value, successMessage)
    {
        ...
        if (items.length > 0)
        {
            //Call the appropriate webservice to perform the operation
            var webServiceClass = this.get_webServiceClass();
            this.display_spinner();
            webServiceClass.CustomListOperation(operation, this.get_projectId(), -1, this._get_selected_items(), Function.createDelegate(this, this.custom_list_op_success2), Function.createDelegate(this, this.operation_failure), successMessage);
        }
        else
        {
            alert(resx.Global_SelectOneCheckBoxForCommand);
    ...
    custom_list_op_success2: function (result, message)
    {
        //See if we have a validation error or not
        if (!this._operation_error_check(result))
        {
    ...
    _onUpdateClick: function (sender, e)
    {
        ...
        for (i = 0; i < frm.length; i++)
        {
            ...
            var prefix = thisRef.get_elementId() + '_edit_';
            if ((element.type == 'text' || element.type == 'hidden') && element.name.indexOf(prefix) != -1)
            {
                //Extract the field name and primary key
                fieldName = element.name.substring(element.name.indexOf('--') + 2, element.name.length);
                primaryKey = element.name.substring(prefix.length, element.name.indexOf('--'));
                ...
                if (dataItem.Fields[fieldName].fieldType == globalFunctions._fieldType_integer)
                {
                    ...
                    else
                    {
                        ...
                        {
                            alert(resx.Global_IntegerValidation.replace('{0}', fieldName));
                            return;
                        }
                    }
                }
        ...
        if (e && e.mode != 'Insert')
        {
			...
			thisRef._lastSelectedArtifactId = -1;
		}

        //Pass to webservice and refresh page
        var webServiceClass = thisRef.get_webServiceClass();
        thisRef.display_spinner();
        webServiceClass.HierarchicalList_Update(thisRef.get_projectId(), dataItems, Function.createDelegate(thisRef, thisRef.update_success), Function.createDelegate(thisRef, thisRef.operation_failure));
    },
    update_success: function (messages)
    {
        this.hide_spinner();
        //See if we have a validation error or not
        ...
        {
            //Refresh the data
            this._retrieve_updated_data();
        }
    },
    _onCancelClick: function (sender, e)
    {
        //Clear any messages
        e.thisRef._clear_messages();

        //Reload the data from the Retrieve method, but only update the rows in the edit state
        e.thisRef.display_spinner();
        e.thisRef.get_webServiceClass().HierarchicalList_Retrieve(e.thisRef.get_projectId(), e.thisRef._standardFilters, false, Function.createDelegate(e.thisRef, e.thisRef.retrieve_cancel_success), Function.createDelegate(e.thisRef, e.thisRef.operation_failure));
    },
    retrieve_cancel_success: function (dataSource)
    {
        //We need to iterate through the rows marked as in the edit state and update if necessary
        var dataItems = dataSource.items;
        this._clearEdits();
        this._isInDrag = false;
        this._isInEdit = false;
        var table = this.get_element();
        var rows = this.get_dataRows();
        var columns = this.get_columns();
        var i, j;
        var editCssClass = this.get_editRowCssClass();
        var selectedCssClass = this.get_selectedRowCssClass();
        for (i = 0; i < rows.length; i++)
        {
            ...
            if (primaryKey)
            {
                //See if this row is in the edit state and therefore needs updating
                if (tr.className == editCssClass || tr.className == selectedCssClass)
                {
                    //Find the matching row in the datasource and reload row                
                    var matchFound = false;
                    for (j = 1; j < dataItems.length && !matchFound; j++)
                    {
                        var dataRow = dataItems[j];
                        if (primaryKey == dataRow.primaryKey)
                        {
                            //Delete the existing content and databind the row
                            while (tr.hasChildNodes())
                            {
                                tr.removeChild(tr.firstChild);
                            }
                            this._dataBindRow(dataRow, columns, table, tr, false);
                            matchFound = true;
                        }
                    }
                }
    ...
    //Retrieves just the rows that have changed
    _retrieve_updated_data: function ()
    {
        //Clear any messages
        this._clear_messages();

        //Get the updated data only from the Retrieve web method
        this.display_spinner();
        this.get_webServiceClass().HierarchicalList_Retrieve(this.get_projectId(), this._standardFilters, true, Function.createDelegate(this, this.retrieve_updated_success), Function.createDelegate(this, this.operation_failure));
    },
    retrieve_updated_success: function (dataSource)
    {
        //Store the items that are selected
        var selectedItems = this._get_selected_items();

        //Get the items from the data
        var dataItems = dataSource.items;

        //We need to iterate through the existing rows and update if necessary
        this._clearEdits();
        this._isInDrag = false;
        this._isInEdit = false;
        var table = this.get_element();
        ...
        for (i = 0; i < rows.length; i++)
        {
            ...
            if (primaryKey)
            {
                //See if we have to update this row
                var matchFound = false;
                for (j = 1; j < dataItems.length && !matchFound; j++)
                {
                    var dataRow = dataItems[j];
                ...
                //Set a flag if we've left it in the edit state
                if (tr.className == editCssClass && !matchFound)
                {
                    rowLeftInEditState = true;
                }
            }
        }
        ...
        //See if a row was left in the edit state - can happen if you change an item so that it no longer matches a filter
        //just force a complete reload in that case - will be slower, but only happens on occasion
        if (rowLeftInEditState)
        {
            this.load_data();
        }
        else
    ...
    _onFillClick: function (sender, e)
    {
        ...
        for (i = 0; i < frm.length; i++)
        {
            element = frm.elements[i];
            //Handle text-boxes
            if (element.type == 'text' && element.name.indexOf(e.thisRef.get_elementId() + '_edit_') != -1)
            {
                fieldName = element.name.substring(element.name.indexOf('--') + 2, element.name.length);
            ...
            //Handle dropdowns
            if (element.type == 'hidden' && element.name.indexOf(e.thisRef.get_elementId() + '_edit_') != -1)
            {
                fieldName = element.name.substring(element.name.indexOf('--') + 2, element.name.length);
                if (fieldName == e.fieldName)
                {
    ...
    _onEditClick: function (sender, e)
    {
        //We need to get a refreshed copy of the particular row's data using a web service call
        var artifactId = e.primaryKey;
        var webServiceClass = e.thisRef.get_webServiceClass();
        e.thisRef.display_spinner();
        webServiceClass.Refresh(e.thisRef.get_projectId(), artifactId, Function.createDelegate(e.thisRef, e.thisRef.edit_success), Function.createDelegate(e.thisRef, e.thisRef.operation_failure), e.tr);
    },
    edit_success: function (dataItem, tr)
    {
        this.hide_spinner();

        //We only want to put the row into edit mode if it's not already..
        if (tr.className != this.get_editRowCssClass())
        {
            //Store the concurrency value if necessary
            ...
            var columns = this.get_columns();
            var i, j;
            //If we have no cells in the row, databind the row first
            if (tr.cells.length == 0)
            {
                var table = this.get_element();
                this._dataBindRow(dataItem, columns, table, tr, true);
            }
            for (i = 0; i < columns.length; i++)
            {
                var editable = false;
                ...
                if (dataItem.Fields[fieldName] != null)
                {
                    if (dataItem.Fields[fieldName].editable)
                    {
                        ...
                        outerDiv.className = this._allowColumnPositioning ? 'fixed-width priority1' : 'priority1';
                        ...
                        {
                            //Add the icons and indent
                            var indentPosition = dataItem.indent.length / 3;
                            td1.style.paddingLeft = ((indentPosition - 1) * 10) + 'px';
                            td1.classList.add("grow-1");
                            var img = d.ce('span');
                            img.className = "w3";
                            td1.appendChild(img);
                            img = d.ce('img');
                            img.className = "w4 h4 mr2"
                            if (dataItem.summary)
                            {
                                if (dataItem.expanded && this.get_expandedItemImage() != '')
                                {
                                    img.src = this._themeFolder + 'Images/' + this.get_expandedItemImage();
                                }
                                else
                            ...
                            {
                                if (dataItem.alternate && this.get_alternateItemImage() != '')
                                {
                                    img.src = this._themeFolder + 'Images/' + this.get_alternateItemImage();
                                }
                                else
                            ...
                            //Add a textbox
                            var textbox = d.ce('input');
                            textbox.name = this.get_elementId() + '_edit_' + dataItem.primaryKey + '--' + fieldName;
                            textbox.type = 'text';
                            textbox.className = 'text-box fill';
                        ...
                        columns[i].fieldType == globalFunctions._fieldType_multiList)
                        {
                            //Add a dropdown
                            var div = d.ce('div');
                            div.id = this.get_elementId() + '_edit_' + dataItem.primaryKey + '--' + fieldName + '_ddl';
                            ...
                            this._clearEdit(div.id);
                            dropdown = $create(Inflectra.SpiraTest.Web.ServerControls.DropDownList, null, null, null, div);
                            dropdown.set_name(this.get_elementId() + '_edit_' + dataItem.primaryKey + '--' + fieldName);
                        ...
                        if (columns[i].fieldType == globalFunctions._fieldType_dateTime || columns[i].fieldType == globalFunctions._fieldType_customPropertyDate)
                        {
                            //Add a date control
                            var div = d.ce('div');
                            div.id = this.get_elementId() + '_edit_' + dataItem.primaryKey + '--' + fieldName + '_dat';
                            div.className = 'DatePicker';
                            div.style.width = '90px';
                            this._clearEdit(div.id);
                            var datePicker = $create(Inflectra.SpiraTest.Web.ServerControls.DatePicker, null, null, null, div);
                            datePicker.set_name(this.get_elementId() + '_edit_' + dataItem.primaryKey + '--' + fieldName);
                            datePicker.set_dateFormat(this.get_dateFormat());
    ...
    _onNameDescMouseOver: function (sender, e)
    {
        ...
        var artifactId = e.primaryKey;
        var webServiceClass = e.webServiceClass;
        webServiceClass.RetrieveNameDesc(e.thisRef._projectId, artifactId, null, Function.createDelegate(e.thisRef, e.thisRef.retrieveNameDesc_success), Function.createDelegate(e.thisRef, e.thisRef.operation_failure_quiet));
    },
    ...
    _startDrag: function (evt, tr, dragSource)
    {
		...
		Sys.UI.DomElement.setLocation(this._visual, location.x, (location.y - 160)); // hierarchical
...
Inflectra.SpiraTest.Web.ServerControls.HierarchicalGrid.registerClass('Inflectra.SpiraTest.Web.ServerControls.HierarchicalGrid', Sys.UI.Control);

/* HierarchicalGridRow */
Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridRowBehavior = function (element)
{
    Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridRowBehavior.initializeBase(this, [element]);
    this._element = element;
    this._parent = null;
    this._primaryKey = null;
}
Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridRowBehavior.prototype =
{
    /* Constructors */
    initialize: function ()
    {
        Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridRowBehavior.callBaseMethod(this, 'initialize');
    ...
    {
        return (dataType == "HierarchicalGridRow" && data);
    },

    // drop this draggable item into this drop target
    drop: function (dragMode, dataType, data)
    {
        if (dataType == "HierarchicalGridRow" && data)
        {
            ...
            if (this._parent.get_selected_items() && this._parent.get_selected_items().length > 0)
            {
                //Actually execute the move operation
                var sourcePrimaryKeys = this._parent.get_selected_items();
                if (sourcePrimaryKeys.indexOf(sourcePrimaryKey) == -1)
                {
                    sourcePrimaryKeys.push(sourcePrimaryKey);
                }
                this._parent.move_items(sourcePrimaryKeys, destPrimaryKey);
            }
            else
            {
                //Actually execute the move operation
                this._parent.move_items([sourcePrimaryKey], destPrimaryKey);
            }
        }
    },
...
Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridRowBehavior.registerClass('Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridRowBehavior', Sys.UI.Behavior, Inflectra.SpiraTest.Web.ServerControls.IDropTarget);

/* HierarchicalResizableColumn */
Inflectra.SpiraTest.Web.ServerControls.HierarchicalResizableColumnBehavior = function (element)
{
    Inflectra.SpiraTest.Web.ServerControls.HierarchicalResizableColumnBehavior.initializeBase(this, [element]);
    this._element = element;
    this._parent = null;
...
Inflectra.SpiraTest.Web.ServerControls.HierarchicalResizableColumnBehavior.prototype =
{
    /* Constructors */
    initialize: function ()
    {
        Inflectra.SpiraTest.Web.ServerControls.HierarchicalResizableColumnBehavior.callBaseMethod(this, 'initialize');
    },
    dispose: function ()
    {
        Inflectra.SpiraTest.Web.ServerControls.HierarchicalResizableColumnBehavior.callBaseMethod(this, 'initialize');
    ...
    get_dragDataType: function ()
    {
        return 'HierarchicalResizableColumn';
    },
...
Inflectra.SpiraTest.Web.ServerControls.HierarchicalResizableColumnBehavior.registerClass('Inflectra.SpiraTest.Web.ServerControls.HierarchicalResizableColumnBehavior', Sys.UI.Behavior, Inflectra.SpiraTest.Web.ServerControls.IDropTarget);

/* HierarchicalGridColumn */
Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridColumnBehavior = function (element)
{
    Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridColumnBehavior.initializeBase(this, [element]);
    this._element = element;
    this._parent = null;
    this._primaryKey = null;
}
Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridColumnBehavior.prototype =
{
    /* Constructors */
    initialize: function ()
    {
        Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridColumnBehavior.callBaseMethod(this, 'initialize');
    ...
    canDrop: function (dragMode, dataType, data)
    {
        return (dataType == "HierarchicalGridColumn" && data);
    },

    // drop this draggable item into this drop target
    drop: function (dragMode, dataType, data)
    {
        if (dataType == "HierarchicalGridColumn" && data)
        {
            $(this.get_element()).removeClass('drag-target');
...
Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridColumnBehavior.registerClass('Inflectra.SpiraTest.Web.ServerControls.HierarchicalGridColumnBehavior', Sys.UI.Behavior, Inflectra.SpiraTest.Web.ServerControls.IDropTarget);
