--Update Field Name
UPDATE [TST_ARTIFACT_FIELD] SET [CAPTION] = 'Planned Release' WHERE [ARTIFACT_FIELD_ID] = 8
GO

-- Add Project Collection properties.
IF (SELECT COUNT(*) FROM [TST_PROJECT_COLLECTION] WHERE [NAME] = 'Baseline.Filters') = 0
	INSERT INTO [TST_PROJECT_COLLECTION] VALUES
		('Baseline.Filters', 'Y'),
		('Baseline.SortExpression', 'Y'),
		('Baseline.Pagination', 'Y');
GO

-- Fix Foreign Key to Cascade.
IF (SELECT COUNT(*) FROM INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_NAME = 'FK_TST_HISTORY_CHANGESET_TST_PROJECT_BASELINE' AND DELETE_RULE = 'CASCADE') = 0
...
IF (SELECT COUNT(*) FROM INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_NAME = 'FK_TST_HISTORY_CHANGESET_TST_PROJECT_BASELINE' AND DELETE_RULE = 'CASCADE') = 0
	ALTER TABLE [TST_PROJECT_BASELINE]
		ADD CONSTRAINT [FK_TST_HISTORY_CHANGESET_TST_PROJECT_BASELINE] 
			FOREIGN KEY ([CHANGESET_ID]) REFERENCES [TST_HISTORY_CHANGESET] ([CHANGESET_ID]) 
			ON DELETE CASCADE
GO

-- Add the Baselines Reporting View.
