...
{
    /// <summary>
    /// This class encapsulates the NumberRangeFilter client component for use as a server-side control with decimal ranges
    /// </summary>
    [
    ToolboxData("<{0}:DecimalRangeFilter runat=server></{0}:DecimalRangeFilter>"),
    DefaultProperty("Value"),
    ValidationProperty("Value")
    ]
    public class DecimalRangeFilter : WebControl, IScriptControl, IPostBackDataHandler
    {
        ...
        public DecimalRange Value
        {
            get
            {
                return (DecimalRange)ViewState["Value"];
            }
            ...
            if (postColl != null && !String.IsNullOrEmpty(postDataKey) && postColl[postDataKey] != null)
            {
                ...
                if (oldValue != postedValue)
                {
                    ...
                    try
                    {
                        ...
                        if (DecimalRange.TryParse(postedValue, out decimalRange))
                        {
