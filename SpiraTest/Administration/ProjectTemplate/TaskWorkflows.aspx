<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPages/Administration.master"
    AutoEventWireup="true" CodeBehind="TaskWorkflows.aspx.cs" Inherits="Inflectra.SpiraTest.Web.Administration.ProjectTemplate.TaskWorkflows" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cplAdministrationContent" runat="server">
	<div class="container-fluid">
        <div class="row">
            <div class="col-lg-9">
                <h2>
                    <asp:Localize ID="Localize1" Text="<%$Resources:Main,Admin_EditTaskWorkflows_Title %>"
                        runat="server" />
                    <small>
                        <tstsc:HyperLinkEx 
                            ID="lnkAdminHome" 
        ...
        <div class="row">
            <div class="col-lg-9">
                ...
                <p>
                    <asp:Localize ID="Localize3" runat="server" Text="<%$ Resources:Main,Admin_TaskWorkflows_Intro %>" />
                </p>
                <tstsc:GridViewEx ID="grdEditWorkflows" runat="server" ShowFooter="true" ShowSubHeader="False"
                    SkinID="DataGrid" Width="100%" DataKeyNames="TaskWorkflowId">
                    ...
                    <Columns>
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,workflow_name %>" FooterColumnSpan="5" FooterStyle-HorizontalAlign="left" ItemStyle-CssClass="priority1" HeaderStyle-CssClass="priority1" FooterStyle-CssClass="priority1">
                            <ItemTemplate>
                                <tstsc:TextBoxEx runat="server" Text='<%# ((TaskWorkflow) Container.DataItem).Name %>'
                        ...
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,general_isdefault %>" FooterColumnSpan="-1" HeaderStyle-CssClass="Centered priority2" ItemStyle-CssClass="Centered priority2" >
                            ...
                            <ItemTemplate>
                                <tstsc:RadioButtonEx ID="radWorkflowDefault" runat="server" Checked='<%# ((TaskWorkflow) Container.DataItem).IsDefault %>'
                        ...
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,general_isactive %>" FooterColumnSpan="-1" ItemStyle-CssClass="priority2" HeaderStyle-CssClass="priority2">
                            ...
                            <ItemTemplate>
                                <tstsc:CheckBoxYnEx runat="server" ID="chkActiveFlag" NoValueItem="false" Checked='<%# (((TaskWorkflow) Container.DataItem).IsActive) ? true: false %>' />
                            </ItemTemplate>
                        </tstsc:TemplateFieldEx>
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,workflow_id %>" FooterColumnSpan="-1">
                            <ItemTemplate>
                                <asp:Literal runat="server" Text='<%# String.Format(Inflectra.SpiraTest.Web.GlobalFunctions.ARTIFACT_PREFIX_WORKFLOW + Inflectra.SpiraTest.Web.GlobalFunctions.FORMAT_ID, ((TaskWorkflow) Container.DataItem).TaskWorkflowId) %>'
                                    ID="Label9" />
                            </ItemTemplate>
                        </tstsc:TemplateFieldEx>
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,general_operations %>" FooterColumnSpan="-1" ItemStyle-CssClass="priority1" HeaderStyle-CssClass="priority1" >
                            <ItemTemplate>
                                <div class="btn-group">
                                    <tstsc:HyperLinkEx ID="btnWorkflowSteps" runat="server" NavigateUrl='<%#UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "TaskWorkflowDetails") + "?" + GlobalFunctions.PARAMETER_WORKFLOW_ID + "=" + ((TaskWorkflow) Container.DataItem).TaskWorkflowId %>'
                                        ConfirmationMessage="<%$Resources:Messages,Admin_Workflows_Default_EditActiveWarning %>" SkinID="ButtonPrimary"
                                        Confirmation='<%# ((TaskWorkflow) Container.DataItem).IsActive %>' >
                                    ...
                                    <tstsc:LinkButtonEx ID="btnWorkflowCopy" runat="server" CommandName="WorkflowCopy"
                                        CommandArgument='<%# ((TaskWorkflow) Container.DataItem).TaskWorkflowId%>'>
