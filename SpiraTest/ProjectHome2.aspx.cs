...
namespace Inflectra.SpiraTest.Web
{
	...
	public partial class ProjectHome2 : PageLayout
	{
		private const string CLASS_NAME = "Inflectra.SpiraTest.Web.ProjectHome2::";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            if (preferredProjectHome != "Test")
            {
                SaveProjectSetting(GlobalFunctions.PROJECT_SETTINGS_PROJECT_HOME_SETTINGS, GlobalFunctions.PROJECT_SETTINGS_KEY_HOME_PAGE, "Test");
            }
		...
		void prtManager_WebPartDrillDown(object sender, WebPartEventArgs e)
		{
			...
			if (e.WebPart != null && e.WebPart is GenericWebPart)
			{
				...
				if (genericWebPart.ChildControl != null && genericWebPart.ChildControl is WebPartBase)
				{
					...
					if (userControl is UserControls.WebParts.ProjectHome.TasksLateFinishing)
					{
						ucTasksLateFinishing_DrillDown();
					}
                    if (userControl is UserControls.WebParts.ProjectHome.OpenIssues)
					{
						ucIssueList_DrillDown((UserControls.WebParts.ProjectHome.OpenIssues)userControl);
					}
                    if (userControl is UserControls.WebParts.ProjectHome.OpenRisks)
					{
						ucRiskList_DrillDown();
					}
					if (userControl == this.ucTestExecutionStatus)
					{
						ucTestExecutionStatus_DrillDown();
					}
					if (userControl == this.ucReleaseTestSummary)
					{
                    ...
                    if (userControl is UserControls.WebParts.ProjectHome.TaskGraphs)
                    {
                        ucTaskReports_DrillDown();
                    }
                    if (userControl is UserControls.WebParts.ProjectHome.ActivityFeed)
                    {
