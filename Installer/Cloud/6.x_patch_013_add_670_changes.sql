-- Tables used for Source Code changes.
IF OBJECT_ID('TST_SOURCE_CODE_COMMIT', 'U') IS NULL
BEGIN
...
IF OBJECT_ID('TST_SOURCE_CODE_COMMIT_ARTIFACT', 'U') IS NULL
BEGIN
...
IF OBJECT_ID('TST_SOURCE_CODE_COMMIT_BRANCH', 'U') IS NULL
BEGIN
...
IF OBJECT_ID('TST_SOURCE_CODE_FILE_ENTRY', 'U') IS NULL
BEGIN
...
-- Add the BRANCH_ID field to the RELEASE table.
IF COL_LENGTH('TST_RELEASE', 'BRANCH_ID') IS NULL
BEGIN
	ALTER TABLE [TST_RELEASE] ADD [BRANCH_ID] INTEGER NULL
...
-- Add the new File Extensions.
IF (SELECT COUNT(*) FROM [TST_GLOBAL_FILETYPES] WHERE [FILETYPE_MIME] = 'text/x-yaml') = 0
...
--Changes to the TST_HISTORY_POSITION table.
IF (SELECT COUNT(*) FROM INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_NAME = 'FK_TST_ARTIFACT_TYPE_TST_HISTORY_POSITION_PARENT') > 0
	ALTER TABLE [TST_HISTORY_POSITION] DROP CONSTRAINT [FK_TST_ARTIFACT_TYPE_TST_HISTORY_POSITION_PARENT]
GO

IF COL_LENGTH('TST_HISTORY_POSITION', 'CHILD_ARTIFACT_ID') IS NULL
BEGIN
	EXEC sp_rename N'TST_HISTORY_POSITION.PARENT_ARTIFACT_TYPE_ID', N'CHILD_ARTIFACT_ID', 'COLUMN'
END
GO

IF EXISTS (SELECT * FROM sys.indexes WHERE [NAME] = 'IDX_TST_HISTORY_POSITION_2_FK')
BEGIN
	DROP INDEX [TST_HISTORY_POSITION].[IDX_TST_HISTORY_POSITION_2_FK]
END
GO

IF (SELECT columnproperty(object_id('TST_HISTORY_POSITION'), 'HISTORY_POSITION_ID','IsIdentity')) = 1
BEGIN
	...
	ALTER TABLE [TST_HISTORY_POSITION] ADD [HISTORY_POSITION_ID] [bigint] IDENTITY(1,1) NOT NULL
	ALTER TABLE [TST_HISTORY_POSITION] ADD CONSTRAINT [PK_TST_HISTORY_POSITION] PRIMARY KEY CLUSTERED ([HISTORY_POSITION_ID] ASC)
END
GO

--Update Requirement Details report, adding section.
IF (SELECT COUNT(*) FROM [TST_REPORT_SECTION_ELEMENT] WHERE [REPORT_SECTION_ID]=3 AND [REPORT_ELEMENT_ID]=6) = 0
BEGIN
	INSERT INTO [TST_REPORT_SECTION_ELEMENT] ([REPORT_SECTION_ID], [REPORT_ELEMENT_ID]) VALUES (3, 6)
END
GO

UPDATE [TST_REPORT_SECTION] SET [DEFAULT_TEMPLATE] = '<?xml version="1.0" encoding="utf-8"?>
