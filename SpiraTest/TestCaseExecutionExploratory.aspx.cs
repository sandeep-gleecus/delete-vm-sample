...
namespace Inflectra.SpiraTest.Web
{
    ...
    /// </summary>
    [HeaderSettings(GlobalNavigation.NavigationHighlightedLink.TestCases, "TestCaseExecution_Title", "Test-Execution/#exploratory-test-execution", "TestCaseExecution_Title")]
    public partial class TestCaseExecutionExploratory : PageLayout
    {
        private const string CLASS_NAME = "Inflectra.SpiraTest.Web.TestCaseExecutionExploratory::";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.endTestJustEnd.Click += new EventHandler(endTestJustEnd_Click);
            this.endTestUpdateCase.Click += endTestUpdateCase_Click;
            ...
            try
            {
                ...
                else if (testRunsPending.TestRuns.Count != 1)
                {
                    Response.Redirect(UrlRewriterModule.RetrieveRewriterURL(UrlRoots.NavigationLinkEnum.ProjectHome, this.ProjectId, 0) + "?" + GlobalFunctions.PARAMETER_ERROR_MESSAGE + "=" + "Exploratory testing is only available when running a single test case.", true);
                }
                ...
                bool isOfTypeExploratory = testCaseManager.RetrieveById2(ProjectId, testRunsPending.TestRuns[0].TestCaseId, true, true).Type.IsExploratory;

                //If current user not owner, or test case type is not exploratory then bounce redirect
                if (!isAnOwner || !isOfTypeExploratory || !canExecuteExploratory)
                {
        ...
        /// <summary>
        /// Completes the pending test run entry and DOES update the corresponding test case
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void endTestUpdateCase_Click(object sender, EventArgs e)
        {
            const string METHOD_NAME = "endTestUpdateCase_Click";
            ...
            //Actually update the test case itself
            TestRunsPending testRunsPending = testRunsPending = testRunManager.RetrievePendingById(testRunsPendingId, true);
            testRunManager.UpdateExploratoryTestCaseFromRun(ProjectId, testRunsPending.TestRuns[0].TestRunId, UserId, false);
        ...
        /// <summary>
        /// Completes the pending test run entry and does NOT update the corresponding test case
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void endTestJustEnd_Click(object sender, EventArgs e)
        {
            const string METHOD_NAME = "endTestJustEnd_Click";
            ...
            //Actually update the test case itself (status only)
            TestRunsPending testRunsPending = testRunsPending = testRunManager.RetrievePendingById(testRunsPendingId, true);
            testRunManager.UpdateExploratoryTestCaseFromRun(ProjectId, testRunsPending.TestRuns[0].TestRunId, UserId, true);
