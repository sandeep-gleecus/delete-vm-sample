<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="RecentBuilds.ascx.cs" Inherits="Inflectra.SpiraTest.Web.UserControls.WebParts.EnterpriseHome.RecentBuilds" %>
        ...
        <!-- ko foreach: $data -->
            <tr>
                <td class="priority1" colspan="4">
                    <tstsc:ImageEx ImageUrl="Images/org-Portfolio.svg" AlternateText="<%$Resources:Fields,Portfolio %>" ToolTip="<%$Resources:Fields,Portfolio %>" runat="server" CssClass="w4 h4" />
                    <!-- ko if: workspaceId() > 0 -->
                        <a data-bind="attr: { href: urlTemplate_portfolioHome.replace(globalFunctions.projectIdToken, workspaceId()) }">
                            <span data-bind="text: workspaceName"></span>
                        </a>
                    <!-- /ko -->
                    <!-- ko ifnot: workspaceId() > 0 -->
                        <span data-bind="text: workspaceName"></span>
                    <!-- /ko -->
                </td>
            </tr>
...
<script type="text/javascript">
    ...
    var urlTemplate_portfolioHome = '<%=GlobalFunctions.JSEncode(UrlRewriterModule.ResolveUrl(UrlRewriterModule.RetrievePortfolioRewriterURL(UrlRoots.NavigationLinkEnum.PortfolioHome, -2)))%>';
    ...
    function load_recentBuilds() {
        ...
        Inflectra.SpiraTest.Web.Services.Ajax.EnterpriseService.Enterprise_RetrieveBuilds(rowsToDisplay, function (data)
        {
            //Success
