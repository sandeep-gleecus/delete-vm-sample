...
namespace Inflectra.SpiraTest.Web.Administration.ProjectTemplate
{
    ...
    /// Administration Edit Tasks Workflows page and handling all raised events
    /// </summary>
    [
    HeaderSettings(GlobalNavigation.NavigationHighlightedLink.Administration, "Admin_EditTaskWorkflows_Title", "Template-Tasks/#task-workflows", "Admin_EditTaskWorkflows_Title"),
    ...
    public partial class TaskWorkflows : AdministrationBase
    {
        private const string CLASS_NAME = "Inflectra.SpiraTest.Web.Administration.ProjectTemplate.TaskWorkflows";
        ...
        void grdEditWorkflows_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            try
            {
                ...
                if (e.CommandName.ToLowerInvariant() == "workflowadd")
                {
                    ...
                    new TaskWorkflowManager().Workflow_Insert(ProjectTemplateId, Resources.Main.Admin_Workflows_NewWorkflowName, false);
                ...
                if (e.CommandName.ToLowerInvariant() == "workflowcopy")
                {
                    ...
                    new TaskWorkflowManager().Workflow_Copy(workflowId);
                ...
                if (e.CommandName.ToLowerInvariant() == "workflowdelete")
                {
                    ...
                    try
                    {
                        new TaskWorkflowManager().Workflow_Delete(workflowId);
        ...
        /// <summary>
        /// Loads the task types and task workflows configured for the current project
        /// </summary>
        protected void LoadAndBindData()
        {
            ...
            try
            {
                ...
                TaskManager taskManager = new Business.TaskManager();
                TaskWorkflowManager taskWorkflowManager = new TaskWorkflowManager();

                //Get the yes/no flag list
                this.flagList = taskWorkflowManager.RetrieveFlagLookup();

                //Get the list of active workflows for this project (used as a lookup)
                this.workflows = taskWorkflowManager.Workflow_Retrieve(this.ProjectTemplateId, true);

                //Now get the list of all workflows and bind the workflow grid to that
                List<TaskWorkflow> taskWorkflows = taskWorkflowManager.Workflow_Retrieve(ProjectTemplateId, false);
                this.grdEditWorkflows.DataSource = taskWorkflows;
            ...
            try
            {
                ...
                for (int i = 0; i < this.grdEditWorkflows.Rows.Count; i++)
                {
                    ...
                    if (gvr.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        TaskWorkflow taskWorkflow = taskWorkflows.Find(w => w.TaskWorkflowId == workflowId);
                        ...
                        if (taskWorkflow != null)
                        {
                            ...
                            taskWorkflow.StartTracking();
                            taskWorkflow.Name = txtName.Text;
                            taskWorkflow.IsDefault = radWorkflowDefault.Checked;
                            taskWorkflow.IsActive = chkActiveFlag.Checked;
                        }
                    }
                }
                ...
                try
                {
                    ...
                    {
                        taskWorkflowManager.Workflow_Update(taskWorkflow);
                    }
                }
                catch (WorkflowInUseException exception)
                {
