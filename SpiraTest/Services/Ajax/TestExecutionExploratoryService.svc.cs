...
namespace Inflectra.SpiraTest.Web.Services.Ajax
{
    ...
    public class TestExecutionExploratoryService : ListServiceBase, ITestExecutionExploratoryService
    {
        private const string CLASS_NAME = "Inflectra.SpiraTest.Web.Services.Ajax.TestExecutionExploratoryService::";
        ...
        #region ITestExecutionExploratoryService methods
        ...
        /// <summary>States whether a user can execute the specific test run as exploratory and therefore have access to enhanced exploratory functionality</summary>
        /// <param name="projectId">The id of the current project</param>
        /// <param name="userId">The id of the current user</param>
        /// <param name="testRunId">The id of the current test run</param>
        /// <returns>true only if the user can execute exploratory AND the test case associated with the test run is of the right type</returns>
        private bool ExploratoryIsAuthorized (int projectId, int userId, int testRunId)
        {
            ...
            TestRunManager testRunManager = new TestRunManager();
            int TestCaseIdToFind = testRunManager.RetrieveById(testRunId).TestCaseId;
            bool isOfTypeExploratory = testCaseManager.RetrieveById2(projectId, TestCaseIdToFind, true,true).Type.IsExploratory;


            return canExecuteExploratory && isOfTypeExploratory;
        }
        ...
        /// <param name="expectedResult">The description</param>
        /// <param name="sampleData">The description</param>
        /// <param name="actualResult">The actual result</param>
        /// <param name="startDate">start date of the step to calculate the time taken to run the test run</param>
        /// <param name="endDate">end date of the step to calculate the time taken to run the test run</param>
        /// <param name="actualDuration">duration of the test run step</param>
        ...
        public JsonDictionaryOfStrings PassTestRunStep(
            int projectId, 
            ...
            )
        {
            ...
            try
            {
                ...
                //Make sure the test run can be run by this user as exploratory
                if ( !ExploratoryIsAuthorized(projectId, userId, testRunId) )
                {
                    throw new ArtifactAuthorizationException(Resources.Messages.TestExecutionService_ExploratoryNotAuthorized);
                }


                //Now set the execution status and timing statuses
                ...
                //Update the other description fields
                if (String.IsNullOrEmpty(description))
                {
                    testRunStepRow.Description = null;
                }
                else
                {
                    testRunStepRow.Description = GlobalFunctions.HtmlScrubInput(description.Trim());
                }

                if (String.IsNullOrEmpty(expectedResult))
                {
                    testRunStepRow.ExpectedResult = null;
                }
                else
                {
                    testRunStepRow.ExpectedResult = GlobalFunctions.HtmlScrubInput(expectedResult.Trim());
                }

                if (String.IsNullOrEmpty(sampleData))
                {
                    testRunStepRow.SampleData = null;
                }
                else
                {
                    testRunStepRow.SampleData = GlobalFunctions.HtmlScrubInput(sampleData.Trim());
                }

                //Now persist the executation status and timing changes
                testRunManager.UpdateExecutionStatus(projectId, userId, testRunsPending, testRunIndex, DateTime.UtcNow, true, true, false);

                //Check to see if we have to add a linked incident, checking permissions first
                int incidentId = -1;
        ...
        /// <param name="incidentDataItem">The associated incident record</param>
        /// <returns>A list of the updated execution status so that the treeview can be updated</returns>
        public JsonDictionaryOfStrings FailTestRunStep(
            int projectId,
            int testRunsPendingId,
            int testRunId,
            int testRunStepId,
            ...
            )
        {
            ...
            try
            {
                ...
                //Set the execution status and timing fields
                testRunStepRow.StartTracking();
        ...
        /// <param name="incidentDataItem">The associated incident record</param>
        /// <returns>A list of the updated execution status so that the treeview can be updated</returns>
        public JsonDictionaryOfStrings BlockTestRunStep(
            int projectId,
            int testRunsPendingId,
            int testRunId,
            int testRunStepId,
            ...
            )
        {
            ...
            try
            {
                ...
                //Set the execution status and timing
                testRunStepRow.StartTracking();
                ...
                //Check to see if we have to add a linked incident, checking permissions
                int incidentId = -1;
                if (incidentDataItem != null && incidentDataItem.PrimaryKey < 1 && incidentDataItem.Fields.Count > 0 && IsAuthorized(projectId, Project.PermissionEnum.Create, DataModel.Artifact.ArtifactTypeEnum.Incident) == Project.AuthorizationState.Authorized)
                {
                    incidentId = SaveLinkedIncident(userId, projectId, projectTemplateId, incidentDataItem, testRunStepRow);
		...
		/// <param name="incidentDataItem">The associated incident record</param>
		/// <returns>A list of the updated execution status so that the treeview can be updated</returns>
        public JsonDictionaryOfStrings NotApplicableTestRunStep(
            int projectId,
            int testRunsPendingId,
            int testRunId,
            int testRunStepId,
        ...
        /// <param name="incidentDataItem">The associated incident record</param>
        /// <returns>A list of the updated execution status so that the treeview can be updated</returns>
        public JsonDictionaryOfStrings CautionTestRunStep(
            int projectId,
            int testRunsPendingId,
            int testRunId,
            int testRunStepId,
            ...
            )
        {
            ...
            try
            {
                ...
                //Set the execution status and timings
                testRunStepRow.StartTracking();
        ...
        /// <summary>Updates the main text fields of a test run step</summary>
        /// <param name="userId">The id of the current user</param>
        /// <param name="projectId">The id of the current project</param>
        /// <param name="testRunStepId">The id of the current test run step</param>
        /// <param name="testRunId">The id of the current test run</param>
        /// <param name="testRunsPendingId">The id of the pending test runs entry</param>
        /// <param name="fieldToUpdate">specifies which field to update - based on an enum list</param>
        /// <param name="textField">the actual text of whichever test run step field is being returned</param>
        /// <returns>Nothings needs to be returned</returns>
        public void UpdateTestRunSingleField(
            int projectId,
            int testRunsPendingId,
            int testRunId,
            int? testRunStepId,
        ...
        {
            const string METHOD_NAME = "UpdateTestRunSingleField()";
            ...
            try
            {
                ...
                //Update the relevant test run field - where the "is field" bool returns true
                if (fieldToUpdate == (int)FieldToUpdateEnum.TestRunName)
                {
                    //Set the run name
                    testRunRow.Name = String.IsNullOrEmpty(textField) ? null : GlobalFunctions.HtmlScrubInput(textField.Trim());
                }
                else if (fieldToUpdate == (int)FieldToUpdateEnum.TestRunDescription)
                {
                    //Set the description
                    testRunRow.Description = String.IsNullOrEmpty(textField) ? null : GlobalFunctions.HtmlScrubInput(textField.Trim());
                }
                else if (testRunStepId > 0)
                {
                    TestRunStep testRunStepRow = testRunRow.TestRunSteps.FirstOrDefault(t => t.TestRunStepId == testRunStepId);
                    if (testRunStepRow == null)
                    {
                    ...
                    //Update the relevant test run step field - where the "is field" bool returns true
                    if (fieldToUpdate == (int)FieldToUpdateEnum.TestStepDescription)
                    {
                        //Set the description
                        testRunStepRow.Description = String.IsNullOrEmpty(textField) ? null : GlobalFunctions.HtmlScrubInput(textField.Trim());
                    }

                    else if (fieldToUpdate == (int)FieldToUpdateEnum.TestStepExpectedResult)
                    {
                        //Set the expected result
                        testRunStepRow.ExpectedResult = String.IsNullOrEmpty(textField) ? null : GlobalFunctions.HtmlScrubInput(textField.Trim());
                    }

                    else if (fieldToUpdate == (int)FieldToUpdateEnum.TestStepSampleData)
                    {
                        //Set the sample data
                        testRunStepRow.SampleData = String.IsNullOrEmpty(textField) ? null : GlobalFunctions.HtmlScrubInput(textField.Trim());
                    }

                    else if (fieldToUpdate == (int)FieldToUpdateEnum.TestStepActualResult)
                    {
                        //Set the actual result
        ...
        /// <summary>Updates the position of all the test runs steps</summary>
        /// <param name="userId">The id of the current user</param>
        /// <param name="projectId">The id of the current project</param>
        /// <param name="testRunId">The id of the current test run</param>
        /// <param name="testRunsPendingId">The id of the pending test runs entry</param>
        /// <param name="testRunSteps">an array of step IDs and their respective positions</param>
        /// <returns>Nothings needs to be returned</returns>
        public void UpdateTestRunStepPositions(
            int projectId,
            int testRunsPendingId,
            int testRunId,
        ...
        {
            const string METHOD_NAME = "UpdateTestRunStepPositions()";
            ...
            try
            {
                ...
                //Update the database based on the information sent
                testRunManager.UpdateExploratoryTestRunStepPositions(testRunId, testRunStepPositions);
        ...
        /// <summary>Deletes a single test run step</summary>
        /// <param name="userId">The id of the current user</param>
        /// <param name="projectId">The id of the current project</param>
        /// <param name="testRunId">The id of the current test run</param>
        /// <param name="testRunsStepId">The id of the test run step being marked for deletion</param>
        /// <returns>Nothings needs to be returned</returns>
        public int? DeleteExploratoryTestRunStep(
            int projectId,
            int testRunsPendingId,
            int testRunId,
            int testRunStepId
            )
        {
            ...
            try
            {
                ...
                //Make sure we have at least two test steps - otherwise they are left with 0 test run steps after deletion, which will stop them getting back to the page should they leave
                if (!testRunsPending.TestRuns.Any(t => t.TestRunSteps.Count > 1))
                {
                    throw new DataValidationExceptionEx(new List<ValidationMessage>() { new ValidationMessage() { Message = Resources.Messages.TestExecutionService_ExploratoryDoNotDeleteLastStep } });
                }
                ...
                //Update the database based on the information sent to delete the relevant test run step
                testRunManager.DeleteExploratoryTestRunStep(testRunId, testRunStepId);

                //update the status of the test run 
                testRunsPending = testRunManager.RetrievePendingById(testRunsPendingId, true);
                int newExecutionStatusId = testRunManager.UpdateExecutionStatus(
                    projectId,
                    userId,
                    testRunsPending,
                    0, // we only have a single test case in exploratory mode, so the test run index will be always be 0
                    null,
                    true,
                    true,
                    false // don't update the test case as we are still testing
                    );

                //return the new execution status id to the client
                return newExecutionStatusId;
            }
            catch (DataValidationExceptionEx)
            {
                //This is handled by the calling javascript function so we just throw
        ...
        /// <summary>Creates a new test run step in the test run</summary>
        /// <param name="userId">The id of the current user</param>
        /// <param name="projectId">The id of the current project</param>
        /// <param name="testRunId">The id of the current test run</param>
        /// <param name="testCaseId">The id of the relevant test case</param>
        /// <returns>the ID of the new test run step</returns>
        public int? CreateNewExploratoryTestRunStep(
            int projectId,
            int testRunsPendingId,
            int testRunId,
            int testCaseId
            )
        {
            ...
            try
            {
                //Get the pending test record and find the current test run
                TestRunManager testRunManager = new TestRunManager();
                TestRunsPending testRunsPending = testRunManager.RetrievePendingById(testRunsPendingId, true);

                //Make sure we have at least one test steps
                if (!testRunsPending.TestRuns.Any(t => t.TestRunSteps.Count > 0))
                {
                    throw new DataValidationExceptionEx(new List<ValidationMessage>() { new ValidationMessage() { Message = Resources.Messages.TestExecutionService_ExploratoryDoNotDeleteLastStep } });
                }
                ...
                //Add a new test run step to the test run and get the testrunstep Id back
                int newTestRunStepId = testRunManager.CreateNewExploratoryTestRunStep(testRunId, testCaseId);
                if (newTestRunStepId != 0)
                {
                    //update the status of the test run 
                    testRunsPending = testRunManager.RetrievePendingById(testRunsPendingId, true);
                    testRunManager.UpdateExecutionStatus(
                        projectId,
                        userId,
                        testRunsPending,
                        0, // we only have a single test case in exploratory mode, so the test run index will be always be 0
                    ...
                    return newTestRunStepId;
                }
                else
                {
                    throw new ArtifactAuthorizationException(Resources.Messages.Exception_Unknown);
                }
        ...
        /// <summary>Creates a new test run step in the test run that is a rough clone of another</summary>
        /// <param name="userId">The id of the current user</param>
        /// <param name="projectId">The id of the current project</param>
        /// <param name="testRunId">The id of the current test run step to be cloned</param>
        /// <param name="testCaseId">The id of the relevant test run step to be cloned</param>
        ...
        /// <returns>the ID of the new test run step</returns>
        public int? CloneExploratoryTestRunStep(
            int projectId,
            int testRunsPendingId,
            int testRunId,
            int testCaseId,
            ...
            )
        {
            ...
            try
            {
                ...
                //Add a new test run step to the test run and get the testrunstep Id back
                int newTestRunStepId = testRunManager.CloneExploratoryTestRunStep(testRunId, testCaseId, description, expectedResult, sampleData);
                if (newTestRunStepId != 0)
                {
                    //update the status of the test run 
                    testRunsPending = testRunManager.RetrievePendingById(testRunsPendingId, true);
        ...
        public TestRunsPendingModel TestExecution_RetrieveTestRunsPending(int projectId, int testRunsPendingId)
        {
            ...
            try
            {
                ...
                testRunsPendingModel.SettingsExploratory = new List<UserSettingsExploratory>();
                UserSettingsExploratory userSettings = new UserSettingsExploratory();

                //Add the user settings if present
                if (projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_CASE_DESCRIPTION] is bool)
                {
                    userSettings.ShowCaseDescription = (bool)projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_CASE_DESCRIPTION];
                }
                if (projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_EXPECTED_RESULT] is bool)
                {
                    userSettings.ShowExpectedResult = (bool)projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_EXPECTED_RESULT];
                }
                if (projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_SAMPLE_DATA] is bool)
                {
                    userSettings.ShowSampleData = (bool)projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_SAMPLE_DATA];
                }
                ...
                if (projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_LAST_RESULT] is bool)
                {
                    userSettings.ShowLastResult = (bool)projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_LAST_RESULT];
                }

                if (userSettingsCollection[GlobalFunctions.USER_SETTINGS_GUIDED_TOURS_TEST_EXECUTION_EXPLORATORY_SEEN] is bool)
                {
                    userSettings.GuidedTourSeen = (bool)userSettingsCollection[GlobalFunctions.USER_SETTINGS_GUIDED_TOURS_TEST_EXECUTION_EXPLORATORY_SEEN];
                }
                ...
                testRunsPendingModel.SettingsExploratory.Add(userSettings);
                ...
                if (testRunsPending.TestRuns != null && testRunsPending.TestRuns.Count > 0)
                {
                    ...
                    foreach (TestRun testRun in testRunsPending.TestRuns)
                    {
                        ...
                        if (testRun.TestRunSteps != null && testRun.TestRunSteps.Count > 0)
                        {
                            ...
                            //Get the test run information about the last time this test case was run
                            bool wasRunBefore = false;
                            bool lastRunHadSteps = false;
                            
                            //first, create the filters to narrow search down to only this test case 
                            Hashtable testFilters = new Hashtable();
                            testFilters.Add("TestCaseId", testRun.TestCaseId);

                            //we also want to restrict the results to not return any test run marked as not run (that means it has not finished yet)
                            List<TestCase.ExecutionStatusEnum> notRunStatuses = new List<TestCase.ExecutionStatusEnum>
                            {
                                TestCase.ExecutionStatusEnum.Blocked,
                                TestCase.ExecutionStatusEnum.Caution,
                                TestCase.ExecutionStatusEnum.Failed,
                                TestCase.ExecutionStatusEnum.NotApplicable,
                                TestCase.ExecutionStatusEnum.Passed
                            };

                            // now add the filter
                            testFilters.Add("ExecutionStatusId", notRunStatuses);

                            // TODO: Exclude Current Test Run 
                            // Ideally we should exclude the current test run id from the hashtable as well, but there is (as of 5.3 on 2017-10) no way to do this.
                            // eg testFilters.Add("TestRunId", !testRun.TestRunId);


                            //retrieve the most recent test run that matches the test case, if it exists
                            TestRunView lastTestRun = new TestRunView();
                            lastTestRun = testRunManager.Retrieve(projectId, "EndDate", false, 1, 1, testFilters, 0).FirstOrDefault();

                            List<TestRunStepView> lastTestRunSteps = new List<TestRunStepView>();

                            wasRunBefore = lastTestRun != null;
                            if (wasRunBefore)
                            {
                                //get the test run steps from this test run
                                lastTestRunSteps = testRunManager.TestRunStep_RetrieveForTestRun(lastTestRun.TestRunId);
                                lastRunHadSteps = lastTestRunSteps.Count > 0;

                                //and add the id and release information to the test run
                                testRunModel.LastTestRunId = lastTestRun.TestRunId;
                                testRunModel.LastReleaseId = lastTestRun.ReleaseId;
                                testRunModel.LastReleaseVersion = lastTestRun.ReleaseVersionNumber;
                            }
                            ...
                            foreach (TestRunStep testRunStep in testRun.TestRunSteps)
                            {
                                ...
                                //and add on information about the last time that step was executed, if at all
                                //first narrow the list of test run steps down to only the one that matches this testRunStep
                                //and exclude test steps newly created in this run (by definition they cannot have been run before, and they do not have an id so will cause an error)
                                if (testRunStep.TestStepId != null && wasRunBefore && lastRunHadSteps)
                                {
                                    TestRunStepView lastTestRunStep = new TestRunStepView();
                                    lastTestRunStep = lastTestRunSteps.FirstOrDefault(step => step.TestStepId == testRunStep.TestStepId);

                                    //if there is a match then we can safely add the information to the step model
                                    if (lastTestRunStep != null)
                                    {
                                        testRunStepModel.LastTestRunStepId = lastTestRunStep.TestRunStepId;
                                        testRunStepModel.LastActualResult = lastTestRunStep.ActualResult;
                                        testRunStepModel.LastExecutionStatusId = lastTestRunStep.ExecutionStatusId;
                                        testRunStepModel.LastEndDate = lastTestRunStep.EndDate;
                                    }
                                }
                            ...
                            //Order the test run steps inside each test run by the test run step position
                            testRunModel.TestRunSteps = testRunModel.TestRunSteps.OrderBy(step => step.Position).ToList();
        ...
        /// <param name="userId">The current user</param>
        /// <param name="projectId">The current project</param>
        /// <param name="showCaseDescription">Whether or not to show the test case description of the test run</param>
        /// <param name="showExpectedResult">Whether or not to show expected result for the test run step</param>
        /// <param name="showSampleData">Whether or not to show sample data for the test run step</param>
        /// <param name="showCustomProperties">Whether or not to show custom properties of both the test run and the test run step</param>
        /// <param name="showLastResult">Whether or not to show the last result run for each relevant test run step</param>
        /// <param name="guidedTourSeen">true if the guided tour has already been seen</param>
        public void TestExecution_LogDisplaySettings(
            int projectId, 
            ...
            )
        {
            ...
            try
            {
                ...
                if (projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_CASE_DESCRIPTION] == null || (bool)projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_CASE_DESCRIPTION] != showCaseDescription)
                {
                    projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_CASE_DESCRIPTION] = showCaseDescription;
                    projectChanged = true;
                }
                if (projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_EXPECTED_RESULT] == null || (bool)projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_EXPECTED_RESULT] != showExpectedResult)
                {
                    projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_EXPECTED_RESULT] = showExpectedResult;
                    projectChanged = true;
                }
                if (projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_SAMPLE_DATA] == null || (bool)projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_SAMPLE_DATA] != showSampleData)
                {
                    projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_SAMPLE_DATA] = showSampleData;
                    projectChanged = true;
                }
                ...
                if (projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_LAST_RESULT] == null || (bool)projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_LAST_RESULT] != showLastResult)
                {
                    projectSettings[GlobalFunctions.PROJECT_SETTINGS_KEY_TE_SHOW_LAST_RESULT] = showLastResult;
                    projectChanged = true;
                }
        ...
        private enum FieldToUpdateEnum : int
        {
            TestRunName = 1,
