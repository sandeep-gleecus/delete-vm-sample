...
Inflectra.SpiraTest.Web.ServerControls.OrderedGridColumn = function()
{
    this.$header = null; //Start with $ prefix so that it does not includes in the Serialization used by the JavaScriptSerializer.
}
Inflectra.SpiraTest.Web.ServerControls.OrderedGridColumn.registerClass('Inflectra.SpiraTest.Web.ServerControls.OrderedGridColumn');

Inflectra.SpiraTest.Web.ServerControls.OrderedGrid = function (element)
{
    ...
    this._autoLoad = true;
    this._allowInlineEditing = true;
    this._alternateItemHasHyperlink = true;
    ...
    Inflectra.SpiraTest.Web.ServerControls.OrderedGrid.initializeBase(this, [element]);
}

Inflectra.SpiraTest.Web.ServerControls.OrderedGrid.prototype =
{

    initialize: function ()
    {
        Inflectra.SpiraTest.Web.ServerControls.OrderedGrid.callBaseMethod(this, 'initialize');
    ...
    dispose: function ()
    {
        ...
        Inflectra.SpiraTest.Web.ServerControls.OrderedGrid.callBaseMethod(this, 'dispose');
    },
    ...
    get_alternateItemHasHyperlink: function ()
    {
        return this._alternateItemHasHyperlink;
    },
    set_alternateItemHasHyperlink: function (value)
    {
        this._alternateItemHasHyperlink = value;
    },

    get_allowInlineEditing: function ()
    {
        return this._allowInlineEditing;
    },
    set_allowInlineEditing: function (value)
    {
        this._allowInlineEditing = value;
    },
    ...
    {
        var e = Function._validateParams(arguments, [{ name: 'value', type: Array, elementType: Inflectra.SpiraTest.Web.ServerControls.OrderedGridColumn, elementMayBeNull: false}]);
        if (e) throw e;
            ...
            for (var j = 0; j < this._tableRows.length; j++)
            {
                ...
                if (primaryKey)
                {
                    ...
                    if (artifactId != -1)
                    {
                        ...
                        webServiceClass.OrderedList_Refresh(this.get_projectId(), this._standardFilters, artifactId, Function.createDelegate(this, this.edit_success), Function.createDelegate(this, this.operation_failure), context);
                    }
                }
            }
        }
    ...
    _load_data: function ()
    {
        ...
        this.display_spinner();
        this.get_webServiceClass().OrderedList_Retrieve(this.get_projectId(), this._standardFilters, Function.createDelegate(this, this.retrieve_success), Function.createDelegate(this, this.operation_failure));
    },
    ...
    dataBind: function (callback)
    {
        ...
        for (var property in dataRow.Fields)
        {
            ...
            {
                column = new Inflectra.SpiraTest.Web.ServerControls.OrderedGridColumn();
        ...
        for (i = 0; i < columns.length; i++)
        {
            ...
            if (column.allowDragAndDrop && this._allowColumnPositioning)
            {
                ...
                var dragSource = $create(Inflectra.SpiraTest.Web.ServerControls.OrderedGridColumnBehavior, { columnIndex: i, dataField: column.dataField, 'parent': this }, null, null, th);
        ...
        if (this._allowEdit && this._allowInlineEditing)
        {
            ...
            this._editColumnHeader.appendChild(btn);
    ...
    _dataBind_footer: function (dataSource, columns, table, tBody, thead, callback)
    {
        ...
        $create(Inflectra.SpiraTest.Web.ServerControls.OrderedGridRowBehavior, { 'parent': this }, null, null, tr);
    ...
    {
        //If the div, td or tr was double-clicked then make the row editable
        var targ;
        if (evt.target)
        {
            targ = evt.target;
    ...
    _onTableRowKeyDown: function (evt, e)
    {
        ...
        if (e.thisRef._isInEdit)
        {
            if (keynum == 13)
            {
                //Fire update if ENTER pressed (unless in text area)
    ...
    _dataBindRow: function (dataRow, columns, table, tr, addDragHandlers)
    {
        ...
        if (addDragHandlers)
        {
            var dragSource = $create(Inflectra.SpiraTest.Web.ServerControls.OrderedGridRowBehavior, { 'primaryKey': dataRow.primaryKey, 'parent': this }, null, null, tr);
        ...
        //We also use this first column to store the indent position of the row
        td = d.ce('td');
        ...
        for (var j = 0; j < columns.length; j++)
        {
            ...
            if (column.fieldType == globalFunctions._fieldType_nameDescription)
            {
                //Set the id of the row, used for drag and drop editing
                ...
                img = d.ce('img');
                img.className = "w4 h4";
                img.id = this.get_element().id + '_icon_' + dataRow.primaryKey;
            ...
            if ((column.dataField != null) && (column.dataField.length > 0))
            {
                ...
                if (column.fieldType == globalFunctions._fieldType_dateTime || column.fieldType == globalFunctions._fieldType_timeInterval ||
                    ...
                    column.fieldType == globalFunctions._fieldType_customPropertyDate)
                {
                ...
                else if (column.fieldType == globalFunctions._fieldType_equalizer)
                {
                    ...
                    else
                    {
                        ...
                        td.className = dataRow.Fields[column.dataField].cssClass;
                    }
                }
                else
                {
                    ...
                    if (value != null)
                    {
                        //Truncate text longer than 50 characters and use tooltip
                        if (value.length > 50)
						{
                        ...
                        if (column.fieldType == globalFunctions._fieldType_nameDescription)
                        {
                            //If we have the alternate item, make sure hyperlinks enabled
                            if (dataRow.alternate && !this._alternateItemHasHyperlink)
                            {
                                a = d.ce('span');
                            ...
                            else
                            {
                                a = d.ce('a');
                                a.appendChild(document.createTextNode(value));
                                //See if we have a custom url or not
                                ...
                                {
                                    a.href = this._baseUrl.replace(globalFunctions.artifactIdToken, '' + dataRow.primaryKey);
                                }
                                td.appendChild(a);
                            }

                            //Add the asynchronous tooltip handler
                            var _nameDescMouseOverHandler = Function.createCallback(this.onNameDescMouseOver, { thisRef: this, primaryKey: dataRow.primaryKey, webServiceClass: this.get_webServiceClass() });
                            var _nameDescMouseOutHandler = Function.createCallback(this.onNameDescMouseOut, { thisRef: this, primaryKey: -1, webServiceClass: '' });
                        ...
                        else if ((column.fieldType == globalFunctions._fieldType_text || column.fieldType == globalFunctions._fieldType_lookup) && dataRow.Fields[column.dataField].tooltip)
                        {
                            //If we are a text/lookup field and have a tooltip, it should be used as a url instead
                            a = d.ce('a');
                            a.appendChild(document.createTextNode(value));
                        ...
                        else if (column.fieldType == globalFunctions._fieldType_hierarchyLookup)
                        {
                            ...
                            a = d.ce('a');
                            a.appendChild(document.createTextNode(value));
                            a.href = globalFunctions.getArtifactDefaultUrl(this._applicationPath, this._projectId, column.dataField, urlSuffix);
                            td.appendChild(a);
                        }
                        else
                        {
    ...
    select: function (index)
    {
        ...
        if ((this._selectedRowCssClass != null) && (this._selectedRowCssClass.length > 0) && this._dataRows[index].className != this._editRowCssClass)
        {
            this._dataRows[index].className = this._selectedRowCssClass;
        }
    ...
    add_rowEdit: function (handler)
    {
        this.get_events().addHandler('rowEdit', handler);
    },
    remove_rowEdit: function (handler)
    {
        this.get_events().removeHandler('rowEdit', handler);
    },
    add_rowEditAlternate: function (handler)
    {
        this.get_events().addHandler('rowEditAlternate', handler);
    },
    remove_rowEditAlternate: function (handler)
    {
        this.get_events().removeHandler('rowEditAlternate', handler);
    },
    //Returns true if handled
    raise_rowEdit: function (dataItem, evt)
    {
        if (dataItem.alternate)
        {
            var h = this.get_events().getHandler('rowEditAlternate');
            if (h)
            {
                h(dataItem, evt);
                return true;
        ...
        {
            var h = this.get_events().getHandler('rowEdit');
            if (h)
            {
                h(dataItem, evt);
                return true;
    ...
    add_iconDragStart: function (handler)
    {
        var e = Function._validateParams(arguments, [{ name: "handler", type: Function}]);
        if (e) throw e;

        this.get_events().addHandler('iconDragStart', handler);
    },
    remove_iconDragStart: function (handler)
    {
        var e = Function._validateParams(arguments, [{ name: "handler", type: Function}]);
        if (e) throw e;

        this.get_events().removeHandler('iconDragStart', handler);
    },
    ...
    _clearTableRowHandlers: function ()
    {
        if (this._allowEdit && this._allowInlineEditing)
        {
            ...
            {
                if (this._tableRowDblClickHandlers[i])
                {
                    // put around a try catch because when live loading risk mitigations or RQ use steps 
                    // we can be moving from a state of allowEdit false to true
                    // in those cases the below block of code was causing problems - because the handlers had not previously been there
                    try
                    {
    ...
    insert_item: function (artifact)
    {
        ...
        if (items.length <= 1)
        {
            ...
            //If we are in edit mode then save first - as long as we are not inserting a link
            if (this._isInEdit && artifact != "Link") 
            {
                this._lastEditWasInsert = true;
            ...
            else
            {
                ...
                //If in edit mode AND inserting a link then update the grid first then the let the first of the function continue
                if (this._isInEdit && artifact === "Link") {
                    this._lastEditWasInsert = false;
                    this._isInEdit = false;
                    this._onUpdateClick(null, null);
                ...
                var webServiceClass = this.get_webServiceClass();
                this.display_spinner();
                webServiceClass.OrderedList_Insert(this.get_projectId(), artifact, this._standardFilters, artifactId, Function.createDelegate(this, this.insert_success), Function.createDelegate(this, this.operation_failure));
            }
        }
        else
        {
    ...
    {
        this.hide_spinner();
        //Capture the new artifact id and reload the page
    ...
    delete_items: function ()
    {
        ...
        if (items.length > 0)
        {
            ...
            this.display_spinner();
            var webServiceClass = this.get_webServiceClass();
            webServiceClass.OrderedList_Delete(this.get_projectId(), this._standardFilters, items, Function.createDelegate(this, this.operation_success), Function.createDelegate(this, this.operation_failure));
        }
        else
        {
    ...
    move_items: function (sourceItems, destId)
    {
        ...
        else
        {
            ...
            this.display_spinner();
            var webServiceClass = this.get_webServiceClass();
            webServiceClass.OrderedList_Move(this.get_projectId(), this._standardFilters, sourceItems, destId, Function.createDelegate(this, this.operation_success), Function.createDelegate(this, this.operation_failure));
        }
    },
    copy_items: function ()
    {
        ...
        if (items.length > 0)
        {
            ...
            var webServiceClass = this.get_webServiceClass();
            this.display_spinner();
            webServiceClass.OrderedList_Copy(this.get_projectId(), this._standardFilters, items, Function.createDelegate(this, this.operation_success), Function.createDelegate(this, this.operation_failure));
        }
        else
        {
    ...
    _onUpdateClick: function (sender, e)
    {
        ...
        {
            element = frm.elements[i];
            //Handle text-boxes/areas and hidden fields (dropdowns)
            if ((element.type == 'text' || element.type == 'textarea' || element.type == 'hidden') && element.name.indexOf('edit_') != -1)
            {
                //Extract the field name and primary key
                fieldName = element.name.substring(element.name.indexOf('-') + 1, element.name.length);
                primaryKey = element.name.substring(element.name.indexOf('_') + 1, element.name.indexOf('-'));
                ...
                if (dataItem.Fields[fieldName].fieldType == globalFunctions._fieldType_html)
                {
                    //Get the data from ckEditor
                    var controlId = thisRef.get_element().id + '_edit_' + dataItem.primaryKey + '-' + fieldName + '_rte';
                    var rte = CKEDITOR.instances[controlId];
                    if (rte && rte.getData)
                    {
                        dataItem.Fields[fieldName].textValue = rte.getData();
                    }
                }
                ...
                if (dataItem.Fields[fieldName].fieldType == globalFunctions._fieldType_customPropertyMultiList)
                {
                    dataItem.Fields[fieldName].textValue = element.value;
                }
        ...
        webServiceClass.OrderedList_Update(thisRef.get_projectId(), thisRef._standardFilters, dataItems, Function.createDelegate(thisRef, thisRef.update_success), Function.createDelegate(thisRef, thisRef.operation_failure));
    },
    update_success: function (messages, context)
    {
        ...
        {
            //Now we need to reload the update data unless last time was an Inserted row edit
            //in which case we should insert a new row instead, which will do the retrieve later
    ...
    _onCancelClick: function (sender, e)
    {
        ...
        e.thisRef.get_webServiceClass().OrderedList_Retrieve(e.thisRef.get_projectId(), e.thisRef._standardFilters, Function.createDelegate(e.thisRef, e.thisRef.retrieve_cancel_success), Function.createDelegate(e.thisRef, e.thisRef.operation_failure));
    },
    ...
    _retrieve_updated_data: function ()
    {
        ...
        //We currently don't pass a value through so we get all rows
        //If performance is bad, need to modify by passing through as a 'standard filter'
        this.display_spinner();
        this.get_webServiceClass().OrderedList_Retrieve(this.get_projectId(), this._standardFilters, Function.createDelegate(this, this.retrieve_updated_success), Function.createDelegate(this, this.operation_failure));
    },
    ...
    _onFillClick: function (sender, e)
    {
        ...
        {
            element = frm.elements[i];
            //Handle text-boxes and plain text areas
            if ((element.type == 'text' || element.type == 'textarea') && element.name.indexOf('edit_') != -1 && !element.id)
            {
                fieldName = element.name.substring(element.name.indexOf('-') + 1, element.name.length);
            ...
            {
                fieldName = element.name.substring(element.name.indexOf('-') + 1, element.name.length);
                if (element.id.indexOf('_rte') != -1 && fieldName == e.fieldName)
                {
                    //get latest value from ckeditor
                    var value = e.source.value;
                    var sourceRte = CKEDITOR.instances[e.source.id];
                    if (sourceRte && sourceRte.getData)
                    {
                        value = sourceRte.getData();
                    }

                    var controlId = e.thisRef.get_element().id + '_' + element.name + '_rte';
                    var rte = CKEDITOR.instances[controlId];
                    if (rte && rte.setData)
                    {
                        rte.setData(value);
            ...
            //Handle dropdowns
            if (element.type == 'hidden' && element.name.indexOf('edit_') != -1)
            {
                fieldName = element.name.substring(element.name.indexOf('-') + 1, element.name.length);
                if (fieldName == e.fieldName)
                {
                    var dropdown = $find(e.thisRef.get_element().id + '_' + element.name + '_ddl');
                    if (dropdown)
                    {
    ...
    _onEditClick: function (evt, e)
    {
        ...
        context.tr = e.tr;
        e.thisRef.display_spinner();
        webServiceClass.OrderedList_Refresh(e.thisRef.get_projectId(), e.thisRef._standardFilters, artifactId, Function.createDelegate(e.thisRef, e.thisRef.edit_success), Function.createDelegate(e.thisRef, e.thisRef.operation_failure), context);
    },
    edit_success: function (dataItem, context)
    {
        ...
        {
            //See if we have a custom handler or not
            var wasHandled = this.raise_rowEdit(dataItem, context.evt);
            if (wasHandled)
            {
                return;
            }
            ...
            for (i = 0; i < columns.length; i++)
            {
                ...
                if (dataItem.Fields[fieldName] != null)
                {
                    if (dataItem.Fields[fieldName].editable)
                    {
                        ...
                        var td1 = d.ce('div');
                        td1.className = 'editable-cell-1 grow-1';
                        if (dataItem.Fields[fieldName].width)
                        {
                            td1.style.width = dataItem.Fields[fieldName].width + 'px';
                        }
                        ...
                        if (columns[i].fieldType == globalFunctions._fieldType_nameDescription)
                        {
                            ...
                            //Add a textbox
                            var textbox = d.ce('input');
                            textbox.name = 'edit_' + dataItem.primaryKey + '-' + fieldName;
                            textbox.type = 'text';
                            ...
                            columns[i].fieldType == globalFunctions._fieldType_flag || columns[i].fieldType == globalFunctions._fieldType_customPropertyMultiList)
                        {
                            ...
                            dropdown.set_name('edit_' + dataItem.primaryKey + '-' + fieldName);
                            //Allow multiple values for a 'multilist'
                            if (columns[i].fieldType == globalFunctions._fieldType_customPropertyMultiList)
                            {
                        ...
                        if (columns[i].fieldType == globalFunctions._fieldType_hierarchyLookup)
                        {
                            ...
                            dropdown = $create(Inflectra.SpiraTest.Web.ServerControls.DropDownHierarchy, { "listWidth": "200px" }, null, null, div);
                            dropdown.set_name('edit_' + dataItem.primaryKey + '-' + fieldName);
                            //Handle the images
                            globalFunctions.getHierarchyLookupImages(dropdown, fieldName);
                            if (!dataItem.Fields[fieldName].required)
                            {
                                dropdown.addItem('', '0', '', '', 'Y', resx.Global_None2);
                            }
                            //Add the lookup values
                            var lookups = dataItem.Fields[fieldName].lookups;
                        ...
                        if (columns[i].fieldType == globalFunctions._fieldType_dateTime || columns[i].fieldType == globalFunctions._fieldType_customPropertyDate)
                        {
                            ...
                            datePicker.set_name('edit_' + dataItem.primaryKey + '-' + fieldName);
                        ...
                        {
                            //Custom properties that are HTML are not editable on the list pages
                            if (dataItem.Fields[fieldName].fieldType != globalFunctions._fieldType_html)
                            {
                                //Add a textbox
                                var textbox = d.ce('input');
                        ...
                        if (columns[i].fieldType == globalFunctions._fieldType_html)
                        {
                            //Add a textarea
                            var textarea = d.ce('textarea');
                            textarea.id = this.get_element().id + '_edit_' + dataItem.primaryKey + '-' + fieldName + '_rte';
                            textarea.name = 'edit_' + dataItem.primaryKey + '-' + fieldName;
                            textarea.className = 'text-box';
                            td1.appendChild(textarea);
                            $(td1).addClass('rich-text-mini rich-text-w-100');

                            //Set the value from the datasource
                            if (dataItem.Fields[fieldName].textValue)
                            {
                                textarea.value = dataItem.Fields[fieldName].textValue;
                            }
                            textarea.setAttribute('fieldType', dataItem.Fields[fieldName].fieldType);
                            editable = true;
                            fillSource = textarea;

                            //Convert to ckEditor
                            var appRoot = this._applicationPath;
                            if (appRoot == '/')
                            {
                                appRoot = '';
                            }
                            CKEDITOR.replace(textarea.id, {
                                customConfig: '',
                                uploadUrl: appRoot + '/' + this._projectId + '/ScreenshotUpload/' + dataItem.primaryKey + '/' + this._artifactTypeId + '.aspx',
                                toolbar: globalFunctions.ckEditor_toolbarNormal,
                                disableNativeSpellChecker: false
                            });

                            //Make sure ckEditors work in dark mode
                            setTimeout(function () {
    ...
    onNameDescMouseOver: function (sender, e)
    {
        ...
        webServiceClass.RetrieveNameDesc(e.thisRef._projectId, artifactId, null, Function.createDelegate(e.thisRef, e.thisRef.retrieveNameDesc_success), Function.createDelegate(e.thisRef, e.thisRef.retrieveNameDesc_failure));
    },
    ...
    onNameDescMouseOut: function (sender, e)
    {
        hideddrivetip();
    ...
    _startDrag: function (evt, tr, dragSource)
    {
		...
		Sys.UI.DomElement.setLocation(this._visual, location.x, (location.y - 160)); // ordered
...
Inflectra.SpiraTest.Web.ServerControls.OrderedGrid.registerClass('Inflectra.SpiraTest.Web.ServerControls.OrderedGrid', Sys.UI.Control);

/* OrderedGridRow */
Inflectra.SpiraTest.Web.ServerControls.OrderedGridRowBehavior = function (element)
{
    Inflectra.SpiraTest.Web.ServerControls.OrderedGridRowBehavior.initializeBase(this, [element]);
    this._element = element;
    this._parent = null;
    this._primaryKey = null;
}
Inflectra.SpiraTest.Web.ServerControls.OrderedGridRowBehavior.prototype =
{
    /* Constructors */
    initialize: function ()
    {
        Inflectra.SpiraTest.Web.ServerControls.OrderedGridRowBehavior.callBaseMethod(this, 'initialize');
    ...
    {
        return (dataType == "OrderedGridRow" && data);
    },

    // drop this draggable item into this drop target
    drop: function (dragMode, dataType, data)
    {
        if (dataType == "OrderedGridRow" && data)
        {
...
Inflectra.SpiraTest.Web.ServerControls.OrderedGridRowBehavior.registerClass('Inflectra.SpiraTest.Web.ServerControls.OrderedGridRowBehavior', Sys.UI.Behavior, Inflectra.SpiraTest.Web.ServerControls.IDropTarget);

/* OrderedGridColumn */
Inflectra.SpiraTest.Web.ServerControls.OrderedGridColumnBehavior = function (element)
{
    Inflectra.SpiraTest.Web.ServerControls.OrderedGridColumnBehavior.initializeBase(this, [element]);
    this._element = element;
    this._parent = null;
    this._primaryKey = null;
}
Inflectra.SpiraTest.Web.ServerControls.OrderedGridColumnBehavior.prototype =
{
    /* Constructors */
    initialize: function ()
    {
        Inflectra.SpiraTest.Web.ServerControls.OrderedGridColumnBehavior.callBaseMethod(this, 'initialize');
    ...
    canDrop: function (dragMode, dataType, data)
    {
        return (dataType == "OrderedGridColumn" && data);
    },

    // drop this draggable item into this drop target
    drop: function (dragMode, dataType, data)
    {
        if (dataType == "OrderedGridColumn" && data)
        {
            $(this.get_element()).removeClass('drag-target');
...
Inflectra.SpiraTest.Web.ServerControls.OrderedGridColumnBehavior.registerClass('Inflectra.SpiraTest.Web.ServerControls.OrderedGridColumnBehavior', Sys.UI.Behavior, Inflectra.SpiraTest.Web.ServerControls.IDropTarget);
