...
{
    /// <summary>
    /// Stores the program-specific (not per-user) settings using configuration entities
    /// </summary>
    public class ProjectGroupSettingsProvider : SettingsProvider
    {
        private const string CLASS_NAME = "Inflectra.SpiraTest.Business.ProjectGroupSettingsProvider::";
        ...
        public override SettingsPropertyValueCollection GetPropertyValues(SettingsContext settingsContext, SettingsPropertyCollection collection)
        {
            ...
            if (settingsContext.Contains("ProjectGroupId") && settingsContext["ProjectGroupId"] is Int32)
            {
                projectGroupId = (int)settingsContext["ProjectGroupId"];
            }
            if (projectGroupId <= 0)
            {
               throw new InvalidOperationException("You need to pass in a ProjectGroupId to the settings provider");
            }
            ...
            try
            {
                using (SpiraTestEntitiesEx context = new SpiraTestEntitiesEx())
                {
                    ...
                    foreach (SettingsProperty propertyDef in collection)
                    {
                        ...
                        var query = from p in context.ProjectGroupSettingValues.Include(p => p.Setting)
                            where p.Setting.Name == propertyDefLocal.Name && p.ProjectGroupId == projectGroupId
									select p;

                        ProjectGroupSettingValue projectSettingValue = query.FirstOrDefault();
                ...
                using (SpiraTestEntitiesEx context = new SpiraTestEntitiesEx())
                {
                    foreach (SettingsPropertyValue propertyValue in collection)
                    {
                        ...
                        if (propertyValue.IsDirty)
                        {
                            ...
                            ProjectGroupSetting projectSetting = context.ProjectGroupSettings.SingleOrDefault(p => p.Name == propertyValueLocal.Name);
                            if (projectSetting == null)
                            {
                                //We need to add a new project setting entity first
                                projectSetting = new ProjectGroupSetting();
                                context.ProjectGroupSettings.AddObject(projectSetting);
                            ...
                            //We only handle strings, integers, longs, dates and booleans
                            ProjectGroupSettingValue projectSettingValue = context.ProjectGroupSettingValues.SingleOrDefault(p => p.Setting.Name == propertyValueLocal.Name && p.ProjectGroupId == projectGroupId);
                            if (projectSettingValue == null)
                            {
                                projectSettingValue = new ProjectGroupSettingValue();
                                projectSettingValue.ProjectGroupId = projectGroupId;
