...
{
    /// <summary>
    /// Stores the project-specific (not per-user) settings using configuration entities
    /// </summary>
    public class ProjectSettingsProvider : SettingsProvider
    {
        private const string CLASS_NAME = "Inflectra.SpiraTest.Business.ProjectSettingsProvider::";
        ...
        public override SettingsPropertyValueCollection GetPropertyValues(SettingsContext settingsContext, SettingsPropertyCollection collection)
        {
            ...
            int projectId = 0;
            if (settingsContext.Contains("ProjectId") && settingsContext["ProjectId"] is Int32)
            {
                projectId = (int)settingsContext["ProjectId"];
            }
            if (projectId <= 0)
            {
               throw new InvalidOperationException("You need to pass in a project id to the settings provider");
            }
            ...
            try
            {
                using (SpiraTestEntitiesEx context = new SpiraTestEntitiesEx())
                {
                    ...
                    foreach (SettingsProperty propertyDef in collection)
                    {
                        ...
                        var query = from p in context.ProjectSettingValues.Include(p => p.Setting)
                            where p.Setting.Name == propertyDefLocal.Name && p.ProjectId == projectId
                                    select p;

                        ProjectSettingValue projectSettingValue = query.FirstOrDefault();
                ...
                using (SpiraTestEntitiesEx context = new SpiraTestEntitiesEx())
                {
                    foreach (SettingsPropertyValue propertyValue in collection)
                    {
                        ...
                        if (propertyValue.IsDirty)
                        {
                            ...
                            ProjectSetting projectSetting = context.ProjectSettings.SingleOrDefault(p => p.Name == propertyValueLocal.Name);
                            if (projectSetting == null)
                            {
                                //We need to add a new project setting entity first
                                projectSetting = new ProjectSetting();
                                context.ProjectSettings.AddObject(projectSetting);
                            ...
                            //We only handle strings, integers, longs, dates and booleans
                            ProjectSettingValue projectSettingValue = context.ProjectSettingValues.SingleOrDefault(p => p.Setting.Name == propertyValueLocal.Name && p.ProjectId == projectId);
                            if (projectSettingValue == null)
                            {
                                projectSettingValue = new ProjectSettingValue();
                                projectSettingValue.ProjectId = projectId;
