...
namespace Inflectra.SpiraTest.Web.Administration.ProjectTemplate
{
    ...
    /// Administration Edit TestCases Workflows page and handling all raised events
    /// </summary>
    [
    HeaderSettings(GlobalNavigation.NavigationHighlightedLink.Administration, "Admin_EditTestCaseWorkflows_Title", "Template-Test-Cases/#test-case-workflows", "Admin_EditWorkflows_Title"),
    ...
    public partial class TestCaseWorkflows : AdministrationBase
    {
        private const string CLASS_NAME = "Inflectra.SpiraTest.Web.Administration.ProjectTemplate.TestCaseWorkflows";
        ...
        void grdEditWorkflows_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            try
            {
                ...
                if (e.CommandName.ToLowerInvariant() == "workflowadd")
                {
                    ...
                    new TestCaseWorkflowManager().Workflow_Insert(ProjectTemplateId, Resources.Main.Admin_Workflows_NewWorkflowName, false);
                ...
                if (e.CommandName.ToLowerInvariant() == "workflowcopy")
                {
                    ...
                    new TestCaseWorkflowManager().Workflow_Copy(workflowId);
                ...
                if (e.CommandName.ToLowerInvariant() == "workflowdelete")
                {
                    ...
                    try
                    {
                        new TestCaseWorkflowManager().Workflow_Delete(workflowId);
        ...
        /// <summary>
        /// Selects the workflow for each testCase type
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void grdTestCaseTypes_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                TestCaseType testCaseType = (TestCaseType)e.Row.DataItem;
                int testCaseTypeId = testCaseType.TestCaseTypeId;
                ...
                if (ddlWorkflow != null)
                {
                    try
                    {
                        ...
                        int workflowId = testCaseWorkflowManager.Workflow_GetForTestCaseType(testCaseTypeId);
        ...
        /// <summary>
        /// Loads the testCase types and testCase workflows configured for the current project
        /// </summary>
        protected void LoadAndBindData()
        {
            ...
            try
            {
                ...
                this.flagList = testCaseWorkflowManager.RetrieveFlagLookup();

                //Get the list of active workflows for this project (used as a lookup)
                this.workflows = testCaseWorkflowManager.Workflow_Retrieve(this.ProjectTemplateId, true);

                //Now get the list of all workflows and bind the workflow grid to that
                List<TestCaseWorkflow> testCaseWorkflows = testCaseWorkflowManager.Workflow_Retrieve(ProjectTemplateId, false);
                this.grdEditWorkflows.DataSource = testCaseWorkflows;
            ...
            try
            {
                ...
                for (int i = 0; i < this.grdEditWorkflows.Rows.Count; i++)
                {
                    ...
                    if (gvr.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        TestCaseWorkflow testCaseWorkflow = testCaseWorkflows.Find(w => w.TestCaseWorkflowId == workflowId);
                        ...
                        if (testCaseWorkflow != null)
                        {
                            //Update the various fields
                            testCaseWorkflow.StartTracking();
                            testCaseWorkflow.Name = txtName.Text;
                            testCaseWorkflow.IsDefault = radWorkflowDefault.Checked;
                            testCaseWorkflow.IsActive = chkActiveFlag.Checked;
                        }
                    }
                }
                ...
                try
                {
                    foreach (TestCaseWorkflow testCaseWorkflow in testCaseWorkflows)
                    {
                        testCaseWorkflowManager.Workflow_Update(testCaseWorkflow);
                    }
                }
                catch (WorkflowInUseException exception)
                {
