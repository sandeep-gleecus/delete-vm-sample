...
{
    /// <summary>
    /// This class encapsulates the NumberRangeFilter client component for use as a server-side control with effort ranges
    /// </summary>
    [
    ToolboxData("<{0}:EffortRangeFilter runat=server></{0}:EffortRangeFilter>"),
    DefaultProperty("Value"),
    ValidationProperty("Value")
    ]
    public class EffortRangeFilter : WebControl, IScriptControl, IPostBackDataHandler
    {
        ...
        public EffortRange Value
        {
            get
            {
                return (EffortRange)ViewState["Value"];
            }
        ...
        /// <summary>
        /// Returns whether the user entered a valid decimal-range or not
        /// </summary>
        [
        Category("Data"),
        DefaultValue(true)
        ...
        // Defines the RangeChanged event.
        public event System.EventHandler RangeChanged;

        //Invoke delegates registered with the Click event.
        protected virtual void OnRangeChanged(System.EventArgs e)
        {
        ...
        public bool LoadPostData(string postDataKey, NameValueCollection postColl)
        {
            ...
            if (postColl != null && !String.IsNullOrEmpty(postDataKey) && postColl[postDataKey] != null)
            {
                ...
                if (oldValue != postedValue)
                {
                    ...
                    try
                    {
                        EffortRange decimalRange;
                        if (EffortRange.TryParse(postedValue, out decimalRange))
                        {
                            this.Value = decimalRange;
                            return true;
                        }
                        else
                        {
