...
namespace Inflectra.SpiraTest.Web.Services.v4_0
{
	...
	/// <remarks>This API is available for all installations v4.0 or greater. There are older APIs available
	/// for older versions of the system (v3.0, v2.2, v1.5.2, v1.2)</remarks>
	/// <version>v4.0</version>
	public class ImportExport : ServiceBase, IImportExport
	{
		private const string CLASS_NAME = "Inflectra.SpiraTest.Web.Services.v4_0.ImportExport::";
		...
		public RemoteCustomProperty CustomProperty_AddDefinition(RemoteCustomProperty remoteCustomProperty, int? customListId)
		{
			...
			try
			{
				...
				if (customListId.HasValue)
				{
					...
					if (projectId != remoteCustomProperty.ProjectId)
					{
						throw new Exception(Resources.Messages.Services_CustomListNotBelongToProject);
					}
				}
		...
		/// <param name="testCaseFolderId">The TestCase folder to count in.</param>
		/// <returns>A number of all items (folder objects and test cases) in the given parent folder ID.</returns>
		public long TestCase_CountForFolder(List<RemoteFilter> remoteFilters, int testCaseFolderId)
		{
			...
			//Call the business object to actually retrieve the incident dataset
			long retNum = new TestCaseManager().Count(projectId, filters, testCaseFolderId, 0);
		...
		/// </summary>
		/// <returns></returns>
		public List<RemoteTestCaseFolder> TestCase_RetrieveFolderHierarchy()
		{
			const string METHOD_NAME = "TestCase_RetrieveFolderHierarchy";
		...
		public List<RemoteTestCase> TestCase_RetrieveByReleaseId(int releaseId, List<RemoteFilter> remoteFilters, int startingRow, int numberOfRows)
		{
			...
			//Call the business object to actually retrieve the testCase dataset
			TestCaseManager testCaseManager = new TestCaseManager();
			List<TestCaseReleaseView> testCases = testCaseManager.RetrieveByReleaseId(projectId, releaseId, "Name", true, startingRow, numberOfRows, filters, 0, TestCaseManager.TEST_CASE_FOLDER_ID_ALL_TEST_CASES);
		...
		/// </summary>
		/// <param name="testCaseFolderId">The id of the test case folder (0 for root folder)</param>
		/// <returns>List of testCases</returns>
		public List<RemoteTestCase> TestCase_RetrieveByFolder(int testCaseFolderId)
		{
			...
			//See if we have the special case of the root folder (0)
			string runningIndentLevel;
			List<TestCaseView> testCases;
			if (testCaseFolderId == 0)
			{
				//Next get the test cases in the root folder
				testCases = testCaseManager.Retrieve(projectId, "Name", true, 1, Int32.MaxValue, null, 0, null);
				runningIndentLevel = "AAA";
			}
			else
			{
				...
				testCases = testCaseManager.Retrieve(projectId, "Name", true, 1, Int32.MaxValue, null, 0, testCaseFolderId);
