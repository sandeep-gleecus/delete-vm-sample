<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="OpenIssues.ascx.cs" Inherits="Inflectra.SpiraTest.Web.UserControls.WebParts.ProjectHome.OpenIssues" %>
...
<tstsc:GridViewEx id="grdIssueList" EnableViewState="false" Runat="server" SkinID="WidgetGrid" WebServiceClass="Inflectra.SpiraTest.Web.Services.Ajax.IncidentsService">
	<Columns>
		...
		<tstsc:NameDescriptionFieldEx ItemStyle-Wrap="True" HeaderText="<%$Resources:Fields,Description %>" DataField="Name" NameMaxLength="60" CommandArgumentField="IncidentId" HeaderColumnSpan="-1" />
		<tstsc:TemplateFieldEx ItemStyle-Wrap="False" HeaderStyle-Width="75px">
			<HeaderTemplate>
				<asp:Localize ID="Localize3" runat="server" Text="<%$Resources:Fields,Priority %>" />
			</HeaderTemplate>
			<ItemTemplate>
		...
		<tstsc:TemplateFieldEx ItemStyle-Wrap="False" HeaderStyle-Width="75px">
			<HeaderTemplate>
				<asp:Localize ID="Localize3" runat="server" Text="<%$Resources:Fields,Severity %>" />
			</HeaderTemplate>
			<ItemTemplate>
		...
		<tstsc:TemplateFieldEx 
            ...
            <ItemTemplate>
                <tstsc:LabelEx 
                    ID="lblOwnedBy" 
                    Runat="server" 
                    Text='<%# Microsoft.Security.Application.Encoder.HtmlEncode(((IncidentView) Container.DataItem).OwnerName) %>'
                    />
            </ItemTemplate>
		</tstsc:TemplateFieldEx>
		<tstsc:TemplateFieldEx ItemStyle-Wrap="False" HeaderStyle-Wrap="False" HeaderStyle-Width="90px" HeaderStyle-CssClass="priority4" ItemStyle-CssClass="priority4" >
			<HeaderTemplate>
				<asp:Localize ID="Localize3" runat="server" Text="<%$Resources:Fields,DateOpened %>" />
			</HeaderTemplate>
			<ItemTemplate>
