    ...
    public static class SelfTrackingEntitiesContextExtensions
    {
        ...
        private static void UpdateOriginalValues(ObjectContext context, IObjectWithChangeTracker entity)
        {
            ...
            foreach(EdmProperty property in entityType.Properties)
            {
                ...
                {
    				//Inflectra - changed to fix issue when original value is NULL
    				//http://social.msdn.microsoft.com/Forums/en/adodotnetentityframework/thread/3605da26-efb2-44e6-931e-53bd050b43ec
    				//
                    // Deal with null original values.
                    // Since we cannot set an original value to null, remember the current value,
                    // and then set the current value to null.  Apply this change it to the original value,
                    // and set the current value back to what we started with.
                    // Inflectra - special code added to handle ConcurrencyDate concurrency field
    				if (value == null && property.Nullable)
    				{
                            int concurrencyFieldIndex = -1;
                            try
                            {
                                concurrencyFieldIndex = originalValueRecord.GetOrdinal("ConcurrencyDate");
                            }
                            catch (ArgumentOutOfRangeException)
                            {
                                concurrencyFieldIndex = -1;
                            }
                            catch (IndexOutOfRangeException)
                            {
                                concurrencyFieldIndex = -1;
                            }
                            DateTime concurrencyDate = DateTime.UtcNow;
                            if (concurrencyFieldIndex >= 0)
                            {
                                concurrencyDate = originalValueRecord.GetDateTime(concurrencyFieldIndex);
                            }
                            ...
                            if (concurrencyFieldIndex >= 0)
                            {
                                originalValueRecord.SetDateTime(concurrencyFieldIndex, concurrencyDate);
                            }
                    }
        ...
        private sealed class AddHelper
        {
            ...
            private void HandleStateManagerChange(object sender, System.ComponentModel.CollectionChangeEventArgs args)
            {
