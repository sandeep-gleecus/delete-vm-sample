...
{
    /// <summary>
    /// Stores the portfolio-specific (not per-user) settings using configuration entities
    /// </summary>
    public class PortfolioSettingsProvider : SettingsProvider
    {
        private const string CLASS_NAME = "Inflectra.SpiraTest.Business.PortfolioSettingsProvider::";
        ...
        public override SettingsPropertyValueCollection GetPropertyValues(SettingsContext settingsContext, SettingsPropertyCollection collection)
        {
            ...
            int portfolioId = 0;
            if (settingsContext.Contains("PortfolioId") && settingsContext["PortfolioId"] is Int32)
            {
                portfolioId = (int)settingsContext["PortfolioId"];
            }
            if (portfolioId <= 0)
            {
               throw new InvalidOperationException("You need to pass in a PortfolioId to the settings provider");
            }
            ...
            try
            {
                using (SpiraTestEntitiesEx context = new SpiraTestEntitiesEx())
                {
                    ...
                    foreach (SettingsProperty propertyDef in collection)
                    {
                        ...
                        var query = from p in context.PortfolioSettingValues.Include(p => p.Setting)
                            where p.Setting.Name == propertyDefLocal.Name && p.PortfolioId == portfolioId
									select p;

                        PortfolioSettingValue projectSettingValue = query.FirstOrDefault();
                ...
                using (SpiraTestEntitiesEx context = new SpiraTestEntitiesEx())
                {
                    foreach (SettingsPropertyValue propertyValue in collection)
                    {
                        ...
                        if (propertyValue.IsDirty)
                        {
                            ...
                            PortfolioSetting projectSetting = context.PortfolioSettings.SingleOrDefault(p => p.Name == propertyValueLocal.Name);
                            if (projectSetting == null)
                            {
                                //We need to add a new project setting entity first
                                projectSetting = new PortfolioSetting();
                                context.PortfolioSettings.AddObject(projectSetting);
                            ...
                            //We only handle strings, integers, longs, dates and booleans
                            PortfolioSettingValue projectSettingValue = context.PortfolioSettingValues.SingleOrDefault(p => p.Setting.Name == propertyValueLocal.Name && p.PortfolioId == portfolioId);
                            if (projectSettingValue == null)
                            {
                                projectSettingValue = new PortfolioSettingValue();
                                projectSettingValue.PortfolioId = portfolioId;
