...
{
	/// <summary>
	/// Displays a list of filters the user has saved
	/// </summary>
	public partial class SavedSearches : WebPartBase, IWebPartReloadable
	{
		private const string CLASS_NAME = "Inflectra.SpiraTest.Web.UserControls.WebParts.MyPage.SavedSearches::";
		...
		public void LoadAndBindData()
		{
			...
			//Now get the list of saved filters that belong to the current user
			SavedFilterManager savedFilterManager = new SavedFilterManager();
            List<DataModel.SavedFilter> savedFilters = savedFilterManager.Retrieve(UserId, filterProjectId);
		...
		void grdSavedSearches_RowCommand(object sender, GridViewCommandEventArgs e)
		{
			...
			if (e.CommandName == "DeleteSearch")
			{
				...
				Business.SavedFilterManager savedFilter = new Business.SavedFilterManager();
				int savedFilterId = Int32.Parse((string)e.CommandArgument);
				savedFilter.Delete(savedFilterId);
