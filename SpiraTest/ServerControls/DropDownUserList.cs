...
namespace Inflectra.SpiraTest.Web.ServerControls
{
    ...
    /// component that better handles user lists and the case of disabled values
    /// that you want displayed as read-only, but still displayed
    /// </summary>
    [ToolboxData("<{0}:DropDownUserList runat=server></{0}:DropDownUserList>")]
    public class DropDownUserList : DropDownList, IAuthorizedControl, IScriptControl
    {
        ...
        protected bool displayOnlineStatus = false;
        private string rawSelectedValues = "";

        /// <summary>
        /// Constructor - delegates to base class
        /// </summary>
        public DropDownUserList()
            : base()
        {
            //Instantiate the authorized control default implementation
            authorizedControlBase = new AuthorizedControlBase(this.ViewState);

            //Should we display online status
            displayOnlineStatus = (ArtifactManager.IsSupportedByLicense(DataModel.Artifact.ArtifactTypeEnum.Message) && ConfigurationSettings.Default.Message_Enabled);
        }
        ...
        /// <summary>
        /// Should we display the online status control
        /// </summary>
        public bool DisplayOnlineStatus
        {
            get
            {
                return this.displayOnlineStatus;
            }
        }
        ...
        /// <summary>
        /// Captures the raw values from the client control in case the data was bound client-side
        /// </summary>
        protected override bool LoadPostData(string postDataKey, System.Collections.Specialized.NameValueCollection postCollection)
        {
            this.rawSelectedValues = "";

            //The custom client drop-down list returns a single value
        ...
        public IEnumerable<ScriptDescriptor> GetScriptDescriptors()
        {
            ...
            ScriptControlDescriptor desc = new ScriptControlDescriptor("Inflectra.SpiraTest.Web.ServerControls.DropDownUserList", this.ClientID);
            ...
            for (int i = 0; i < this.Items.Count; i++)
            {
                ...
                string itemValue = this.Items[i].Value + "_"  + active;
            ...
            desc.AddProperty("displayOnlineStatus", this.DisplayOnlineStatus);
            ...
            desc.AddProperty("avatarBaseUrl", UrlRewriterModule.ResolveUrl("~/UserAvatar.ashx?" + GlobalFunctions.PARAMETER_USER_ID + "={0}&" + GlobalFunctions.PARAMETER_THEME_NAME + "=" + Page.Theme));
        ...
        {
            yield return new ScriptReference(Page.ClientScript.GetWebResourceUrl(this.GetType(), "Inflectra.SpiraTest.Web.ServerControls.ClientScripts.DropDownUserList.js"));
            //If we have instant messaging enabled, include the UserOnlineStatus library as well
    ...
    public class UnityDropDownUserList : DropDownUserList
    {
    }
}
