...
namespace Inflectra.SpiraTest.Web.Administration.ProjectTemplate
{
    ...
    /// Administration Edit Requirements Workflows page and handling all raised events
    /// </summary>
    [
    HeaderSettings(GlobalNavigation.NavigationHighlightedLink.Administration, "Admin_EditWorkflows_Title", "Template-Requirements/#workflows", "Admin_EditWorkflows_Title"),
    ...
    public partial class RequirementWorkflows : AdministrationBase
    {
        private const string CLASS_NAME = "Inflectra.SpiraTest.Web.Administration.ProjectTemplate.RequirementWorkflows";
        ...
        void grdEditWorkflows_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            try
            {
                ...
                if (e.CommandName.ToLowerInvariant() == "workflowadd")
                {
                    ...
                    new RequirementWorkflowManager().Workflow_Insert(ProjectTemplateId, Resources.Main.Admin_Workflows_NewWorkflowName, false);
                ...
                if (e.CommandName.ToLowerInvariant() == "workflowcopy")
                {
                    ...
                    new RequirementWorkflowManager().Workflow_Copy(workflowId);
                ...
                if (e.CommandName.ToLowerInvariant() == "workflowdelete")
                {
                    ...
                    try
                    {
                        new RequirementWorkflowManager().Workflow_Delete(workflowId);
        ...
        /// <summary>
        /// Loads the requirement types and requirement workflows configured for the current project
        /// </summary>
        protected void LoadAndBindData()
        {
            ...
            try
            {
                ...
                this.flagList = requirementWorkflowManager.RetrieveFlagLookup();

                //Get the list of active workflows for this project (used as a lookup)
                this.workflows = requirementWorkflowManager.Workflow_Retrieve(this.ProjectTemplateId, true);

                //Now get the list of all workflows and bind the workflow grid to that
                List<RequirementWorkflow> requirementWorkflows = requirementWorkflowManager.Workflow_Retrieve(ProjectTemplateId, false);
                this.grdEditWorkflows.DataSource = requirementWorkflows;
            ...
            try
            {
                ...
                for (int i = 0; i < this.grdEditWorkflows.Rows.Count; i++)
                {
                    ...
                    if (gvr.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        RequirementWorkflow requirementWorkflow = requirementWorkflows.Find(w => w.RequirementWorkflowId == workflowId);
                        ...
                        if (requirementWorkflow != null)
                        {
                            //Update the various fields
                            requirementWorkflow.StartTracking();
                            requirementWorkflow.Name = txtName.Text;
                            requirementWorkflow.IsDefault = radWorkflowDefault.Checked;
                            requirementWorkflow.IsActive = chkActiveFlag.Checked;
                        }
                    }
                }
                ...
                try
                {
                    foreach (RequirementWorkflow requirementWorkflow in requirementWorkflows)
                    {
                        requirementWorkflowManager.Workflow_Update(requirementWorkflow);
                    }
                }
                catch (WorkflowInUseException exception)
                {
