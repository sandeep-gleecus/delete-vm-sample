<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="AssignedIncidents.ascx.cs" Inherits="Inflectra.SpiraTest.Web.UserControls.WebParts.MyPage.AssignedIncidents" %>
<%@ Import namespace="Inflectra.SpiraTest.DataModel" %>
<%@ Import namespace="Inflectra.SpiraTest.Web" %>
<tstsc:GridViewEx id="grdOwnedIncidents" Runat="server" EnableViewState="false" SkinId="WidgetGrid" WebServiceClass="Inflectra.SpiraTest.Web.Services.Ajax.IncidentsService">
	<Columns>
		...
		<tstsc:TemplateFieldEx ItemStyle-Wrap="False" HeaderStyle-Wrap="False"  HeaderStyle-CssClass="priority4" ItemStyle-CssClass="priority4">
			<HeaderTemplate>
                <asp:Localize ID="Localize1" runat="server" Text="<%$Resources:Fields,DateOpened %>" />
			</HeaderTemplate>
			<ItemTemplate>
				<tstsc:LabelEx Runat="server" Text='<%# String.Format(GlobalFunctions.FORMAT_DATE,  GlobalFunctions.LocalizeDate(((IncidentView) Container.DataItem).CreationDate)) %>' Tooltip='<%# String.Format(GlobalFunctions.FORMAT_DATE_TIME,  GlobalFunctions.LocalizeDate(((IncidentView) Container.DataItem).CreationDate)) %>' ID="lblCreationDate"/>
			</ItemTemplate>
		</tstsc:TemplateFieldEx>
	</Columns> 
</tstsc:GridViewEx>
