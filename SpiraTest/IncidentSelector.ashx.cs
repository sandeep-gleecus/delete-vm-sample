...
{
    /// <summary>
    /// Redirects to either the Incident List page or the Incident Kanban Board
    /// </summary>
    public class IncidentSelector : LocalizedHttpHandler
    {
        private const string CLASS_NAME = "Inflectra.SpiraTest.Web.IncidentSelector::";
        ...
        public override void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                {
                    ProjectSettingsCollection projectSettingsCollection = new ProjectSettingsCollection(ProjectId, CurrentUserId.Value, GlobalFunctions.PROJECT_SETTINGS_INCIDENT_BOARD_SETTINGS);
                    ...
                    {
                        //Redirect to Board (-5)
                        context.Response.Redirect(UrlRewriterModule.RetrieveRewriterURL(UrlRoots.NavigationLinkEnum.Incidents, ProjectId, -5) + queryStringSuffix, true);
                    }
                    else
                    {
                        //Redirect to Table (-4)
                        context.Response.Redirect(UrlRewriterModule.RetrieveRewriterURL(UrlRoots.NavigationLinkEnum.Incidents, ProjectId, -4) + queryStringSuffix, true);
                    }
                }
                else
                {
