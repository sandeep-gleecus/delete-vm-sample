...
namespace Inflectra.SpiraTest.Common
{
    ...
    public class DateRange : IEquatable<DateRange>
    {
        ...
        /// </summary>
        /// <param name="timezone">The id of the timezone to convert FROM (if not provided we assume input string is already in UTC)</param>
        /// <param name="s">The string being parsed (in local time if timezone provided)</param>
        /// <param name="result">The populated date range object (if successful) in UTC</param>
        ...
        public static bool TryParse(string s, out DateRange result, TimeZoneInfo timezone = null)
        {
                    ...
                    else
                    {
                        ...
                        if (!String.IsNullOrEmpty(dates[0]))
                        {
                            if (DateTime.TryParse(dates[0], out startDate))
                            {
                                ...
                                {
                                    result.StartDate = TimeZoneInfo.ConvertTimeToUtc(startDate, timezone);
                                }
                            }
                            else
                            {
                        ...
                        if (!String.IsNullOrEmpty(dates[1]))
                        {
                            if (DateTime.TryParse(dates[1], out endDate))
                            {
                                ...
                                {
                                    result.EndDate = TimeZoneInfo.ConvertTimeToUtc(endDate, timezone);
                                }
                            }
                            else
                            {
		...
		public bool Contains(DateTime other)
		{
            if (ConsiderTimes)
            {
                if ((this.startDate.HasValue && this.startDate <= other || !this.startDate.HasValue)
                    && (this.endDate.HasValue && this.endDate >= other || !this.endDate.HasValue))
                {
                    return true;
                }
            }
            ...
            {
                if ((this.startDate.HasValue && this.startDate.Value.Date <= other.Date || !this.startDate.HasValue)
                    && (this.endDate.HasValue && this.endDate.Value.Date >= other.Date || !this.endDate.HasValue))
                {
                    return true;
                }
            }
