...
Inflectra.SpiraTest.Web.ServerControls.DropDownUserList = function (element)
{
    ...
    Inflectra.SpiraTest.Web.ServerControls.DropDownUserList.initializeBase(this, [this._element]);
    ...
    this._hidden = false;
    this._avatarBaseUrl = '';
    this._displayOnlineStatus = false;
...
Inflectra.SpiraTest.Web.ServerControls.DropDownUserList.prototype =
{
    /* Init / Dispose */
    initialize: function ()
    {
        Inflectra.SpiraTest.Web.ServerControls.DropDownUserList.callBaseMethod(this, 'initialize');
        ...
        //Create the user online status control if enabled
        if (this._displayOnlineStatus)
        {
            var div = d.ce('div');
            div.id = this._element.id + '_UserOnlineStatus';
            div.className = 'UserOnlineStatus';
            this._userOnlineStatus = $create(Inflectra.SpiraTest.Web.ServerControls.UserOnlineStatus, null, null, null, div);
            //insert online status before the actual dropdown list
            this._element.parentNode.insertBefore(div, this._element);
            this.updateOnlineStatusUserId();
        }
    ...
    dispose: function ()
    {
        ...
        if (this._userOnlineStatus) delete this._userOnlineStatus;
        delete this._valueTextBox;
        delete this._valueHidden;
        delete this._listDiv;
        delete this._avatarIcon;
        delete this._menuDown;

        Inflectra.SpiraTest.Web.ServerControls.DropDownUserList.callBaseMethod(this, 'dispose');
    },
    ...
    _onClick: function (e)
    {
        ...
        //Call the method to display the dropdown (if not already displayed and not on the input)
        if (tname == 'INPUT')
        {
    ...
    _onItemClick: function (itm)
    {
        ...
        //Don't need to set these because _onClick is called which toggles the display
        //this._expanded = false;
    ...
    _onAddItem: function (value, active, text)
    {
        var item = new Inflectra.SpiraTest.Web.ServerControls.DropDownUserListItem();
        item.initialize();
    ...
    update_state: function (state)
    {
        ...
        if (!state || state == 'hidden')
        {
            ...
            {
                $(element).addClass('dn');
                if (this._userOnlineStatus)
                {
                    $(this._userOnlineStatus.get_element()).addClass('dn');
                }
            }
            else
            {
                ...
                if (this._userOnlineStatus)
                {
                    $(this._userOnlineStatus.get_element()).removeClass('dn');
                }
            }
        }
    },
    updateOnlineStatusUserId: function ()
    {
        if (this._displayOnlineStatus)
        {
            if (this._valueHidden.value == '')
            {
                this._userOnlineStatus.set_userId(null);
            }
            else
            {
                var userId = parseInt(this._valueHidden.value);
                this._userOnlineStatus.set_userId(userId);
            }
        }
    },
    get_items: function ()
    ...
    get_displayOnlineStatus: function ()
    {
        return this._displayOnlineStatus;
    },
    set_displayOnlineStatus: function (value)
    {
        this._displayOnlineStatus = value;
    },
        ...
        if (this._dataBindIndex < this._itemKeys.length)
        {
            while (localIndex < 50 && this._dataBindIndex < this._itemKeys.length)
            {
                ...
                if (itemKey != '_type')
                {
                    ...
                    var value = items[0];
                    var active = items[1];
                    this._onAddItem(value, active, this._itemValues[this._dataBindIndex]);
                }
                this._dataBindIndex++;
                localIndex++;
            }
...
Inflectra.SpiraTest.Web.ServerControls.DropDownUserList.registerClass('Inflectra.SpiraTest.Web.ServerControls.DropDownUserList', Sys.UI.Control);

/* Dropdown list item definition */
Inflectra.SpiraTest.Web.ServerControls.DropDownUserListItem = function (element)
{
	this._element = element;

	if (!element) this._element = d.ce('div');

	Inflectra.SpiraTest.Web.ServerControls.DropDownUserListItem.initializeBase(this, [this._element]);
...
Inflectra.SpiraTest.Web.ServerControls.DropDownUserListItem.prototype =
{
    /* Init / Dispose */
    initialize: function ()
    {
        Inflectra.SpiraTest.Web.ServerControls.DropDownUserListItem.callBaseMethod(this, 'initialize');
    ...
    dispose: function ()
    {
        ...
        Inflectra.SpiraTest.Web.ServerControls.DropDownUserListItem.callBaseMethod(this, 'dispose');
    },
    ...
    set_value: function (value)
    {
        if (this._value != value)
        {
            ...
            if (value && value != '')
            {
                this._img.src = this._parent.get_avatarBaseUrl().replace('{0}', value);
            }
            else
            {
                this._img.src = '';
...
Inflectra.SpiraTest.Web.ServerControls.DropDownUserListItem.registerClass('Inflectra.SpiraTest.Web.ServerControls.DropDownUserListItem', Sys.UI.Control);
