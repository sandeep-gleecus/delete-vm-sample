<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPages/Administration.master"
    AutoEventWireup="true" CodeBehind="RequirementImportances.aspx.cs" Inherits="Inflectra.SpiraTest.Web.Administration.ProjectTemplate.RequirementImportances" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cplAdministrationContent" runat="server">
	<div class="container-fluid">
        <div class="row">
            <div class="col-lg-9">
                <h2>
                    <asp:Localize ID="Localize1" Text="<%$Resources:Main,RequirementImportances_Title %>" runat="server" />
                    <small>
                        <tstsc:HyperLinkEx 
                            ID="lnkAdminHome" 
                            runat="server" 
        ...
        <div class="row">
            <div class="col-lg-9">
                ...
                <p>
                    <asp:Localize ID="Localize2" runat="server" Text="<%$Resources:Main,RequirementImportances_Intro %>" />
                </p>
                ...
                <tstsc:GridViewEx ID="grdEditRequirementImportances" runat="server"
                    DataMember="RequirementImportance" AutoGenerateColumns="False" ShowFooter="False"
                    ShowSubHeader="False" CssClass="DataGrid" Width="100%">
                    <HeaderStyle CssClass="Header" />
                    <Columns>
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,ID %>">
                            <ItemTemplate>
                                <asp:Label runat="server" Text='<%# String.Format (GlobalFunctions.FORMAT_ID, ((Importance) Container.DataItem).ImportanceId) %>'
                                    ID="Label7" />
                            </ItemTemplate>
                        </tstsc:TemplateFieldEx>
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,Name %>" ItemStyle-CssClass="priority1" HeaderStyle-CssClass="priority1">
                            <ItemTemplate>
                                <tstsc:TextBoxEx MetaData='<%# ((Importance) Container.DataItem).ImportanceId %>'
                                    CssClass="text-box" runat="server" Text='<%# ((Importance) Container.DataItem).Name %>'
                                    Width="90%" MaxLength="20" ID="txtRequirementImportanceName" />
                                <asp:RequiredFieldValidator ID="Requiredfieldvalidator3" ControlToValidate="txtRequirementImportanceName"
                        ...
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,Color %>" ItemStyle-CssClass="priority3" HeaderStyle-CssClass="priority3">
                            <ItemTemplate>
                                <table class="inner-table priority1">
                                    <tr>
                                        <td>
                                            <tstsc:ColorPicker MaxLength="6" MetaData='<%# ((Importance) Container.DataItem).ImportanceId %>'
                                                CssClass="text-box" Width="80px" runat="server" Text='<%# ((Importance) Container.DataItem).Color %>'
                                                ID="colRequirementImportanceColor" />
                                        </td>
                                        <td>
                                            <asp:RequiredFieldValidator ID="Requiredfieldvalidator4" ControlToValidate="colRequirementImportanceColor"
                                            ...
                                            <asp:RegularExpressionValidator ID="Regularexpressionvalidator1" ValidationExpression="<%# GlobalFunctions.VALIDATION_REGEX_HTML_COLOR%>"
                                                ControlToValidate="colRequirementImportanceColor" ErrorMessage="<%$Resources:Messages,Admin_Priorities_ColorInvalid %>"
                                                Text="*" runat="server" />
                                        </td>
                                    </tr>
                                </table>
                        ...
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,Score %>" ItemStyle-CssClass="priority1" HeaderStyle-CssClass="priority1">
                            <ItemTemplate>
                                <tstsc:TextBoxEx MetaData='<%# ((Importance) Container.DataItem).ImportanceId %>'
                                    CssClass="text-box" runat="server" Text='<%# ((Importance) Container.DataItem).Score.ToString() %>'
                        ...
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,ActiveYn %>" ItemStyle-CssClass="priority1" HeaderStyle-CssClass="priority1">
                            <ItemTemplate>
                                <tstsc:CheckBoxYnEx runat="server" ID="ddlActive" NoValueItem="false"
                                    Checked='<%# (((Importance) Container.DataItem).IsActive) ? true : false%>' />
                            </ItemTemplate>
                        </tstsc:TemplateFieldEx>
                    </Columns>
                </tstsc:GridViewEx>
