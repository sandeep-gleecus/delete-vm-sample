...
namespace Inflectra.SpiraTest.Web.Services.Ajax
{
	...
	public class BaselineService : SortedListServiceBase, IBaselineService
	{
		private const string CLASS_NAME = "Web.Services.Ajax.BaselineService::";

		/// <summary>
		/// Set here as a constant for easy changing in the future. Right now, we're basing permissions on viewing a baseline on the
		/// 'RELEASE' artifact type.
		/// </summary>
		private const Artifact.ArtifactTypeEnum PERMISSION_ART_TYPE = Artifact.ArtifactTypeEnum.Release;

		#region IBaselineService Methods
		/// <summary>Counts the number of baselines</summary>
		/// <param name="projectId">The project id</param>
		/// <param name="releaseId">The Release ID to pull the count from.</param>
		/// <returns>The count</returns>
		public int Baseline_Count(int projectId, ArtifactReference artifactRef)
		{
			const string METHOD_NAME = CLASS_NAME + "Baseline_Count()";
			...
			Project.AuthorizationState authorizationState = IsAuthorized(projectId, Project.PermissionEnum.View, PERMISSION_ART_TYPE);
			if (authorizationState == Project.AuthorizationState.Prohibited)
				throw new FaultException(AjaxAuthModule.AUTHORIZATION_MESSAGE);

			count = new BaselineManager().Baseline_Count(projectId, artifactRef.ArtifactId);

			Logger.LogExitingEvent(METHOD_NAME);
			return count;
		}

		/// <summary>Creates a new baseline</summary>
		/// <param name="projectId">The project id</param>
		/// <param name="releaseId">The Release ID to pull the count from.</param>
		/// <param name="name">The baseline name</param>
		/// <param name="description">The baseline description</param>
		/// <returns>The count</returns>
		public int Baseline_Create(int projectId, int releaseId, string name, string description)
		{
			...
			//Make sure we're authorized to create baselines / releases
			Project.AuthorizationState authorizationState = IsAuthorized(projectId, Project.PermissionEnum.Create, PERMISSION_ART_TYPE);
			if (authorizationState == Project.AuthorizationState.Prohibited)
				throw new FaultException(AjaxAuthModule.AUTHORIZATION_MESSAGE);

			//Make sure the name field is populated
			if (string.IsNullOrEmpty(name))
				throw new FaultException(Resources.Messages.Baseline_Create_NameRequired);

			//Generate the Baseline.
			BaselineManager baselineManager = new BaselineManager();
			retVal = baselineManager.Baseline_Create(userId, name, description, projectId, releaseId).BaselineId;

			Logger.LogExitingEvent(METHOD_NAME);
			return retVal;
		}
		#endregion IBaselineService Methods
		...
		public string SortedList_UpdateSort(int productId, string sortProperty, bool sortAscending, int? displayTypeId)
		{
			...
			return UpdateSort(userId, productId, sortProperty, sortAscending, GlobalFunctions.PROJECT_SETTINGS_BASELINE_SORT_EXPRESSION);
		}
		...
		public SortedData SortedList_Retrieve(int projectId, JsonDictionaryOfStrings standardFilters, int? displayTypeId)
		{
			...
			//See if theuser is a ProjectAdmin.
			...
			//Get the Release ID.
			int releaseId = (int)GlobalFunctions.DeSerializeValue(standardFilters["ArtifactId"]);
			...
			Hashtable filterList = GetProjectSettings(userId, projectId, GlobalFunctions.PROJECT_SETTINGS_BASELINE_FILTERS_LIST);
			string sortCommand = GetProjectSetting(userId, projectId, GlobalFunctions.PROJECT_SETTINGS_BASELINE_SORT_EXPRESSION, GlobalFunctions.PROJECT_SETTINGS_KEY_SORT_EXPRESSION, "ChangeDate DESC");
			...
			PopulateShape(projectId, projectTemplateId, userId, filterItem, filterList, isProjectAdmin);
			sortedData.Items.Add(filterItem);

			//Now get the pagination information
			ProjectSettingsCollection paginationSettings = new ProjectSettingsCollection(projectId, userId, GlobalFunctions.PROJECT_SETTINGS_BASELINE_PAGINATION);
			...
			sortedData.CurrPage = filters.StartingPage;
			sortedData.StartRow = filters.StartingRowNumber;
			sortedData.SortProperty = filters.SortProperty;
			sortedData.SortAscending = filters.SortAscending;
			sortedData.PaginationOptions = RetrievePaginationOptions(projectId);

			//Sorting checks..
			if (sortedData.SortProperty.Equals("CreatorUserName"))
				filters.SortProperty = "Creator.Profile.FirstName";

			//Actually retrieve the list of baselines
			int totalPreFilter = 0;
			List<ProjectBaseline> baselines = bMgr.Baseline_RetrieveFilteredForProductRelease(projectId, releaseId, filters, GlobalFunctions.GetCurrentTimezoneUtcOffset(), out totalPreFilter);
			...
			sortedData.TotalCount = bMgr.Baseline_Count(projectId, releaseId);
		...
		/// <summary>Creates a new baseline from the highest applicable ChangeSetId.</summary>
		/// <param name="projectId">The ProductId to generate the Baseline in.</param>
		/// <param name="artifact">Not used.</param>
		/// <param name="standardFilters">Only the ArtifactId is used. Other filters will not be applied.</param>
		...
		{
			const string METHOD_NAME = CLASS_NAME + "SortedList_Insert()";
			Logger.LogEnteringEvent(METHOD_NAME);
			...
			if (standardFilters.ContainsKey("ArtifactId"))
				releaseId = (int?)GlobalFunctions.DeSerializeValue(standardFilters["ArtifactId"]);

			//Throw error is there is an issue.
			if (!releaseId.HasValue)
				throw new ArgumentException("standardFilters:ArtifactId", "ReleaseId is required.");

			//Generate the Baseline.
			BaselineManager bMgr = new BaselineManager();
			retVal = bMgr.Baseline_Create(userId, "(None)", null, projectId, releaseId.Value).BaselineId;
		...
		public SortedDataItem SortedList_Refresh(int projectId, int artifactId, int? displayTypeId)
		{
			...
			//See what level of modify this user has
			Project.AuthorizationState editAuthState = IsAuthorized(projectId, Project.PermissionEnum.Modify, Artifact.ArtifactTypeEnum.Release);
			...
			if (editAuthState == Project.AuthorizationState.Authorized)
			{
				//Modify All (Authorized)
				canEditRow = true;
			}
			else if (editAuthState == Project.AuthorizationState.Limited)
			{
				//Modify Owned (Limited)
				if (baseline.ReleaseId.HasValue)
				{
					try
					{
						Release release = new ReleaseManager().RetrieveById3(projectId, baseline.ReleaseId.Value);
						if (release.CreatorId == userId || (release.OwnerId.HasValue && release.OwnerId.Value == userId))
						{
							canEditRow = true;
						}
					}
					...
					{
						//Ignore, let the user have no edit permissions
					}
				}
			}

			//Populate the shape.
			PopulateShape(projectId, projectTemplateId, userId, dataItem, null, false);
		...
		{
			const string METHOD_NAME = CLASS_NAME + "SortedList_Update()";
			Logger.LogEnteringEvent(METHOD_NAME);
			...
			//Make sure we're authorized (limited OK because we need it for the counts)
			Project.AuthorizationState authorizationState = IsAuthorized(projectId, Project.PermissionEnum.Modify, PERMISSION_ART_TYPE);
			if (authorizationState == Project.AuthorizationState.Prohibited)
				throw new FaultException(AjaxAuthModule.AUTHORIZATION_MESSAGE);

			//Our return messages.
			List<ValidationMessage> retList = null;

			//Loop through any to update them.
			if (dataItems != null && dataItems.Count > 0)
			{
				BaselineManager bMgr = new BaselineManager();

				foreach (SortedDataItem bl in dataItems)
				{
					//Get the baseline's ID.
					int blId = bl.PrimaryKey;

					try
					{
						string newName = bl.Fields["Name"].TextValue;
						string newDesc = bl.Fields["Description"].TextValue;
						bool newIsActive = bl.Fields["IsActive"].TextValue.Equals("Y");

						bMgr.Baseline_Update(blId, newName, newDesc, newIsActive);
					}
					catch (Exception)
					{
		...
		/// <summary>Deletes the specified Baselines.</summary>
		...
		{
			const string METHOD_NAME = CLASS_NAME + "SortedList_Delete()";
			Logger.LogEnteringEvent(METHOD_NAME);
			...
			//Make sure we're authorized (limited OK because we need it for the counts)
			Project.AuthorizationState authorizationState = IsAuthorized(projectId, Project.PermissionEnum.Delete, PERMISSION_ART_TYPE);
			if (authorizationState == Project.AuthorizationState.Prohibited)
				throw new FaultException(AjaxAuthModule.AUTHORIZATION_MESSAGE);
			...
			foreach (var itemId in items)
			{
				...
				{
					bMgr.Baseline_Delete(projectId, id);
				}
			}
		...
		public JsonDictionaryOfStrings RetrievePaginationOptions(int projectId)
		{
			...
			JsonDictionaryOfStrings paginationDictionary = RetrievePaginationOptions(projectId, userId, GlobalFunctions.PROJECT_SETTINGS_BASELINE_PAGINATION);
		...
		public string UpdateFilters(int projectId, JsonDictionaryOfStrings filters, int? displayTypeId)
		{
			...
			ProjectSettingsCollection savedFilters = GetProjectSettings(userId, projectId, GlobalFunctions.PROJECT_SETTINGS_BASELINE_FILTERS_LIST);
		...
		private void PopulateShape(int projectId, int projectTemplateId, int userId, SortedDataItem dataItem, Hashtable filterList, bool isProjectAdmin)
		{
			...
			{
				FieldName = "Name",
				FieldType = (isProjectAdmin ? Artifact.ArtifactFieldTypeEnum.NameDescription : Artifact.ArtifactFieldTypeEnum.Text),
			...
			#region ChangeSet ID
			//We only display this field for project admins
			if (isProjectAdmin)
			{
				//Baseline ID
		...
		private JsonDictionaryOfStrings GetLookupValues(string lookupName, int projectId, int projectTemplateId)
		{
			...
			UserManager user = new Business.UserManager();
		...
		private void PopulateRow(int projectId, SortedDataItem dataItem, ProjectBaseline baseline, bool isProjectAdmin, bool canEditRow = false)
		{
			...
			foreach (KeyValuePair<string, DataItemField> dataItemFieldKVP in dataItem.Fields)
			{
				...
				dataItemField.Editable = canEditRow &&
					(dataItemField.FieldName.Equals("Name") ||
					dataItemField.FieldName.Equals("Description") ||
					dataItemField.FieldName.Equals("IsActive"));
