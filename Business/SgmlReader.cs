...
/*            set
    ...
    public class SgmlReader : XmlReader
    {
        ...
        private void OpenInput()
        {
            ...
            if (this.Href != null)
            {
                this.m_current = new DtdEntity("#document", null, this.m_href, this.m_proxy);
            }
            else if (this.m_inputStream != null)
            {
                this.m_current = new DtdEntity("#document", null, this.m_inputStream, this.m_proxy);           
            }
            else
            {
        ...
        /// <remarks>
        /// This version is slightly different from <see cref="DtdEntity.ScanLiteral"/> in that
        ...
        private string ScanLiteral(StringBuilder sb, char quote)
        {
            sb.Length = 0;
            char ch = this.m_current.ReadChar();
            while (ch != DtdEntity.EOF && ch != quote && ch != '>')
            {
                if (ch == '&')
                {
        ...
        private void ExpandEntity(StringBuilder sb, char terminator)
        {
            ...
            else
            {
                ...
                {
                    DtdEntity e = (DtdEntity)this.m_dtd.FindEntity(name);
                    if (e != null)
                    {
                        if (e.IsInternal)
                        {
                        ...
                        {
                            DtdEntity ex = new DtdEntity(name, e.PublicId, e.Uri, this.m_current.Proxy);
