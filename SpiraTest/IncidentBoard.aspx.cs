...
namespace Inflectra.SpiraTest.Web
{
    ...
    /// </summary>
    [HeaderSettings(GlobalNavigation.NavigationHighlightedLink.Incidents, "SiteMap_IncidentBoard", "Incident-Tracking/#incident-board")]
    public partial class IncidentBoard : PageLayout
    {
        private const string CLASS_NAME = "Inflectra.SpiraTest.Web.IncidentBoard::";
		...
		protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //Redirect if the user has limited view permissions for incidents. Grant access to sys admins if they are a member of the project in any way
            ...
            if (!IsPostBack)
            {
                ...
                int selectedReleaseId = GetProjectSetting(GlobalFunctions.PROJECT_SETTINGS_INCIDENT_BOARD_SETTINGS, GlobalFunctions.PROJECT_SETTINGS_KEY_SELECTED_RELEASE_ID, -2);
                bool includeDetails = GetProjectSetting(GlobalFunctions.PROJECT_SETTINGS_INCIDENT_BOARD_SETTINGS, GlobalFunctions.PROJECT_SETTINGS_KEY_INCLUDE_DETAILS, true);
                int groupByOption = GetProjectSetting(GlobalFunctions.PROJECT_SETTINGS_INCIDENT_BOARD_SETTINGS, GlobalFunctions.PROJECT_SETTINGS_KEY_GROUP_BY_OPTION, (int)ServerControls.PlanningBoard.PlanningGroupByOptions.ByStatus);
                ...
                if (listPage != "Board")
                {
                    SaveProjectSetting(GlobalFunctions.PROJECT_SETTINGS_INCIDENT_BOARD_SETTINGS, GlobalFunctions.PROJECT_SETTINGS_KEY_HOME_PAGE, "Board");
                }
                ...
                if (!releases.Any(r => r.ReleaseId == selectedReleaseId))
                {
                    ...
                    //the stored settings
                    SaveProjectSetting(GlobalFunctions.PROJECT_SETTINGS_INCIDENT_BOARD_SETTINGS, GlobalFunctions.PROJECT_SETTINGS_KEY_SELECTED_RELEASE_ID, -1);
                    selectedReleaseId = -1;
                }

                //Populate the standard fields of the incident creation dialog
                this.ddlDetectedRelease.BaseUrl = UrlRewriterModule.RetrieveRewriterURL(UrlRoots.NavigationLinkEnum.Releases, ProjectId, -2);
                this.ddlResolvedRelease.BaseUrl = UrlRewriterModule.RetrieveRewriterURL(UrlRoots.NavigationLinkEnum.Releases, ProjectId, -2);
				this.ddlVerifiedRelease.BaseUrl = UrlRewriterModule.RetrieveRewriterURL(UrlRoots.NavigationLinkEnum.Releases, ProjectId, -2);
				IncidentManager incidentManager = new IncidentManager();
				ComponentManager componentManager = new ComponentManager();
				List<IncidentPriority> priorities = incidentManager.RetrieveIncidentPriorities(this.ProjectTemplateId, true);
                List<IncidentSeverity> severities = incidentManager.RetrieveIncidentSeverities(this.ProjectTemplateId, true);
				List<IncidentType> types = incidentManager.RetrieveIncidentTypes(this.ProjectTemplateId, true);
                List<IncidentStatus> statuses = incidentManager.IncidentStatus_Retrieve(this.ProjectTemplateId);
                ...
                this.ddlSeverity.DataSource = severities;
                this.ddlOpener.DataSource = allUsers;
                this.ddlOwner.DataSource = activeUsers;

				//'detected-release' should display all releases (active and inactive), the other release fields only active releases. However we bind all to all releases and let the dropdown filter out inactive ones. That prevents the display showing 'none' if the current value is not active
				this.ddlDetectedRelease.DataSource = releases2;
				this.ddlResolvedRelease.DataSource = releases2;
                this.ddlVerifiedRelease.DataSource = releases2;
				this.ddlType.DataSource = types;
				this.ddlStatus.DataSource = statuses;
				this.ddlComponents.DataSource = components;
                ...
                List<ServerControls.PlanningBoard.GroupByOption> groupByOptions = this.ajxPlanningBoard.GetIncidentBoardGroupByOptions((selectedReleaseId == -1), (selectedReleaseId == -2), isIteration);
