...
namespace Inflectra.SpiraTest.DataModel
{
    ...
    public partial class TestSetTestCaseParameter: IObjectWithChangeTracker, INotifyPropertyChanged
    {
            ...
            set
            {
                if (!ReferenceEquals(_testSetTestCase, value))
                {
                    if (ChangeTracker.ChangeTrackingEnabled && ChangeTracker.State != ObjectState.Added && value != null)
                    {
                        // This the dependent end of an identifying relationship, so the principal end cannot be changed if it is already set,
                        // otherwise it can only be set to an entity with a primary key that is the same value as the dependent's foreign key.
                        if (TestSetTestCaseId != value.TestSetTestCaseId)
                        {
                            throw new InvalidOperationException("The principal end of an identifying relationship can only be changed when the dependent end is in the Added state.");
                        }
                    }
        ...
        private void FixupParameter(TestCaseParameter previousValue)
        {
            ...
            if (previousValue != null && previousValue.TestSetTestCaseParameterValues.Contains(this))
            {
                previousValue.TestSetTestCaseParameterValues.Remove(this);
            }
    
            if (Parameter != null)
            {
                if (!Parameter.TestSetTestCaseParameterValues.Contains(this))
                {
                    Parameter.TestSetTestCaseParameterValues.Add(this);
                }
        ...
        private void FixupTestSetTestCase(TestSetTestCase previousValue)
        {
            if (IsDeserializing)
            {
                return;
            ...
            {
                if (!TestSetTestCase.ParameterValues.Contains(this))
                {
                    TestSetTestCase.ParameterValues.Add(this);
                }
