...
{
	public partial class VW_OLD_HISTORY_CHANGE_LIST : Entity
	{
		/// <summary>
		...
		public string SignedValue
		{
			get
			{
				...
				if (String.IsNullOrEmpty(SIGNATURE_HASH))
				{
					return SignedStatusEnum.NotSigned.ToString();
				}

				//See if the hash matches
				string dataToTest = USER_ID + ":" + ARTIFACT_TYPE_ID + ":" + ARTIFACT_ID + ":" + CHANGE_DATE.ToString(DatabaseExtensions.FORMAT_DATE_TIME_SECONDS_INVARIANT, System.Globalization.CultureInfo.InvariantCulture);
				string hash = GetHashSha256(dataToTest);
				if (hash == SIGNATURE_HASH)
				{
