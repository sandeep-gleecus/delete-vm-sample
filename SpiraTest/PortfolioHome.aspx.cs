...
namespace Inflectra.SpiraTest.Web
{
    ...
    /// </summary>
    [HeaderSettings(GlobalNavigation.NavigationHighlightedLink.PortfolioHome, null, "Portfolio-Homepage")]
    public partial class PortfolioHome : PageLayout
    {
        private const string CLASS_NAME = "Inflectra.SpiraTest.Web.PortfolioHome::";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect(UrlRewriterModule.RetrieveRewriterURL(UrlRoots.NavigationLinkEnum.MyPage, this.ProjectId) + "?" + GlobalFunctions.PARAMETER_ERROR_MESSAGE + "=" + "You are not authorized to view the portfolio dashboard!", true);
            }
                ...
                if (portfolio != null)
                {
                    ...
                    if (!IsPostBack)
                    {
                        ...
                        try
                        {
                            ...
                            this.lblPortfolioId.Text = GlobalFunctions.ARTIFACT_PREFIX_PORTFOLIO + PortfolioId;
                            this.lblPortfolioName.Text = Microsoft.Security.Application.Encoder.HtmlEncode(portfolioName);

                            //Update the page title/description with the project id and name
                            ((MasterPages.Main)((MasterPages.Dashboard)this.Master).Master).PageTitle = GlobalFunctions.ARTIFACT_PREFIX_PORTFOLIO + PortfolioId + " - " + portfolioName;
                            ((MasterPages.Main)((MasterPages.Dashboard)this.Master).Master).TstGlobalNavigation.BreadcrumbText = portfolioName;
                        }
                        catch (ArtifactNotExistsException)
                        {
                            Response.Redirect(UrlRewriterModule.RetrieveRewriterURL(UrlRoots.NavigationLinkEnum.MyPage, this.ProjectId, 0) + "?" + GlobalFunctions.PARAMETER_ERROR_MESSAGE + "=" + Resources.Messages.PortfolioHome_PortfolioNotValid, true);
                        }
        ...
        protected void prtManager_Init(object sender, System.EventArgs e)
        {
            ...
            if (PortfolioId > 0)
            {
                this.prtManager.DashboardInstanceId = PortfolioId;
            }
