        ...
        _find: function _find(selector, context, single, filter) {
            ...
            foreach(selectors, function(selector) {
                if (typeof(selector) !== "string") {
					...
					//SHB edited the line below from original to comply with jQuery 3.5
					//else if (window.jQuery) {
					else if (window.jQuery && selector != "#") {
                        if (!filter) {
...
$type = Sys.UI.Control = function Control(element) {
    ...
    if (e) throw e;
    if (element.control) throw Error.invalidOperation(Sys.Res.controlAlreadyDefined + ': ' + element.id);
