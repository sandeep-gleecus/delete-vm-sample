...
namespace Inflectra.SpiraTest.Web.Administration.ProjectTemplate
{
	...
	/// Administration Release Workflow Details Page and handling all raised events
	/// </summary>
    [
    HeaderSettings(GlobalNavigation.NavigationHighlightedLink.Administration, "Admin_WorkflowDetails_Title", "Template-Releases/#edit-workflow-details", "Admin_WorkflowDetails_Title"),
    ...
    public partial class ReleaseWorkflowDetails : AdministrationBase
	{
        private const string CLASS_NAME = "Inflectra.SpiraTest.Web.Administration.ProjectTemplate.ReleaseWorkflowDetails::";

        protected int workflowId;
        protected List<ReleaseStatus> releaseStatuses;
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            //Set the return link
            this.lnkWorkflowList.NavigateUrl = UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "ReleaseWorkflows");
            ...
            if (!Int32.TryParse(Request.QueryString[GlobalFunctions.PARAMETER_WORKFLOW_ID], out workflowId))
            {
                ...
                Response.Redirect(UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "ReleaseWorkflows"));
                return;
            }
            ...
            //Now get the list of possible workflow steps (i.e. release statuses)
            ReleaseManager releaseManager = new ReleaseManager();
            this.releaseStatuses = releaseManager.RetrieveStatuses();
		...
		protected void LoadAndBindData()
		{
            ...
            ReleaseWorkflow workflow = releaseWorkflowManager.Workflow_RetrieveById(this.workflowId);
			...
			//Databind the form
            this.grdWorkflowSteps.DataSource = releaseStatuses;
            this.DataBind();
		...
		private void grdWorkflowSteps_RowCreated(object sender, GridViewRowEventArgs e)
		{
            ...
            {
                int statusId = ((ReleaseStatus)(e.Row.DataItem)).ReleaseStatusId;
                List<DataModel.ReleaseWorkflowTransition> transitions = new ReleaseWorkflowManager().WorkflowTransition_RetrieveByInputStatus(this.workflowId, statusId);
		...
		protected void rptWorkflowStepTransitions_ItemCommand(object source, RepeaterCommandEventArgs e)
		{
			...
			if (e.CommandName.ToLower(System.Globalization.CultureInfo.InvariantCulture) == "delete")
			{
                ...
                new ReleaseWorkflowManager().WorkflowTransition_Delete(workflowTransitionId);
		...
		private void btnAdd_Click(object sender, EventArgs e)
		{
			...
			int sourceReleaseStatusId = Int32.Parse (this.hdnSourceStep.Value);
			int destinationReleaseStatusId = Int32.Parse(this.ddlDestinationStep.RawSelectedValue);
			...
			//but when developers do the same thing they need to be the owner of the release
            ReleaseWorkflowManager releaseWorkflowManager = new Business.ReleaseWorkflowManager();
            releaseWorkflowManager.WorkflowTransition_Insert(this.workflowId, sourceReleaseStatusId, destinationReleaseStatusId, this.txtTransitionName.Text);
        ...
        /// </summary>
        /// <returns></returns>
        protected string GetReleaseStatusesAsJson()
        {
            const string METHOD_NAME = "GetReleaseStatusesAsJson";

            Logger.LogEnteringEvent(CLASS_NAME + METHOD_NAME);

            Dictionary<string, string> statusDic = new Dictionary<string, string>();
            foreach (ReleaseStatus releaseStatus in this.releaseStatuses)
            {
                if (!statusDic.ContainsKey(releaseStatus.ReleaseStatusId.ToString()))
                {
                    statusDic.Add(releaseStatus.ReleaseStatusId.ToString(), releaseStatus.Name);
                }
            }
