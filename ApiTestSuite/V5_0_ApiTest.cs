...
using Inflectra.SpiraTest.ApiTestSuite.SpiraSoapService50;
...
{
	/// <summary>
	/// This fixture tests that the v5.0 import/export web service interface works correctly
	/// </summary>
	[
	TestFixture,
	...
	public class V5_0_ApiTest
	{
		protected static SpiraSoapService50.SoapServiceClient spiraSoapClient;
		...
		public void Init()
		{
			...
			spiraSoapClient = new SpiraSoapService50.SoapServiceClient();
			spiraSoapClient.Endpoint.Address = new EndpointAddress(Properties.Settings.Default.WebServiceUrl + "v5_0/SoapService.svc");

			//Configure the HTTP Binding to handle session cookies
			BasicHttpBinding httpBinding = (BasicHttpBinding)spiraSoapClient.Endpoint.Binding;
		...
		public void CleanUp()
		{
			...
			spiraSoapClient.Connection_Authenticate("administrator", "Welcome123$");
			spiraSoapClient.Project_Delete(projectId1);

			//Delete any other projects
			if (projectId3 > 0)
			{
				spiraSoapClient.Project_Delete(projectId3);
			}
		...
		public void _01_Authentication()
		{
			...
			bool success = spiraSoapClient.Connection_Authenticate("fredbloggs", "PleaseChange");
			...
			success = spiraSoapClient.Connection_Authenticate2("administrator", "Welcome123$", "MyPlugIn");
			Assert.IsTrue(success, "Authentication 2 Failed");

			//Finally Make sure that an invalid authentication fails
			success = spiraSoapClient.Connection_Authenticate("testtest", "wrongwrong");
			Assert.IsFalse(success, "Authentication 3 Should Have Failed");

			//Lets test our API Key logins..
			success = spiraSoapClient.Connection_Authenticate3("fredbloggs", "{7A05FD06-83C3-4436-B37F-51BCF0060483}", "MyPlugIn");
			Assert.IsTrue(success, "Authentication 4 Failed");
			success = spiraSoapClient.Connection_Authenticate("fredbloggs", "{7A05FD06-83C3-4436-B37F-51BCF0060483}");
			Assert.IsTrue(success, "Authentication 5 Failed");
			success = spiraSoapClient.Connection_Authenticate2("fredbloggs", "{7A05FD06-83C3-4436-B37F-51BCF0060483}", "MyPlugIn");
			Assert.IsTrue(success, "Authentication 6 Failed");
			success = spiraSoapClient.Connection_Authenticate("fredbloggs", "{7A05FD06-83C3-4436-B37F-51BCF5560483}");
			Assert.IsFalse(success, "Authentication 7 Should Have Failed");
			success = spiraSoapClient.Connection_Authenticate2("fredbloggs", "{7A05FD06-83C3-4436-B37F-51BCF0066483}", "MyPlugIn");
			Assert.IsFalse(success, "Authentication 8 Should Have Failed");
			success = spiraSoapClient.Connection_Authenticate("donnaharkness", "{7A44FD06-83C3-4436-B37F-51BCF0060483}");
			Assert.IsFalse(success, "Authentication 9 Should Have Failed");
		}
		...
		public void _02_RetrieveProjectList()
		{
			...
			RemoteProject[] remoteProjects = spiraSoapClient.Project_Retrieve();
            ...
            success = spiraSoapClient.Connection_Authenticate("administrator", "Welcome123$");
			Assert.IsTrue(success);
			remoteProjects = spiraSoapClient.Project_Retrieve();
			...
			spiraSoapClient.Connection_Disconnect();
			...
			spiraSoapClient.Connection_Authenticate("wrong", "wrong");
			errorCaught = false;
			try
			{
				remoteProjects = spiraSoapClient.Project_Retrieve();
		...
		public void _03_CreateProject()
		{
			...
			remoteProject = spiraSoapClient.Project_Create(remoteProject, null);
			...
			remoteProject = spiraSoapClient.Project_RetrieveById(projectId1);
		...
		public void _04_ImportUsers()
		{
			...
			spiraSoapClient.Connection_Authenticate("administrator", "Welcome123$");
			spiraSoapClient.Connection_ConnectToProject(projectId1);
			...
			remoteUser.LdapDn = "";
			remoteUser = spiraSoapClient.User_Create(remoteUser, "aant123456789", "What is 2+3?", "5", InternalRoutines.PROJECT_ROLE_MANAGER);
			...
			remoteUser = spiraSoapClient.User_Create(remoteUser, null, null, null, InternalRoutines.PROJECT_ROLE_TESTER);
			userId2 = remoteUser.UserId.Value;

			//Verify that they inserted correctly
			//User #1
			remoteUser = spiraSoapClient.User_RetrieveById(userId1);
			...
			//User #2
			remoteUser = spiraSoapClient.User_RetrieveById(userId2);
			...
			RemoteProjectUser[] remoteProjectUsers = spiraSoapClient.Project_RetrieveUserMembership();
			...
			remoteUser.Active = true;
			remoteUser.LdapDn = "";
			remoteUser = spiraSoapClient.User_Create(remoteUser, "aant123456789", "What is 1+1?", "2", InternalRoutines.PROJECT_ROLE_MANAGER);
			int userId3 = remoteUser.UserId.Value;
			Assert.AreEqual(userId1, userId3);

			//Test that we can add an existing use to a project
			...
			spiraSoapClient.Project_AddUserMembership(remoteProjectUser);

			//Verify
			remoteProjectUsers = spiraSoapClient.Project_RetrieveUserMembership();
			...
			remoteUser.Active = true;
			remoteUser.Approved = true;
			remoteUser = spiraSoapClient.User_Create(remoteUser, "mypassword123456", "What is 1+1?", "2", InternalRoutines.PROJECT_ROLE_TESTER);
			int dummyUserId = remoteUser.UserId.Value;

			//Now update the user
			remoteUser = spiraSoapClient.User_RetrieveById(dummyUserId);
			...
			spiraSoapClient.User_Update(remoteUser);
			...
			//Test that we can inactivate a user (as long as not used in the system)
			spiraSoapClient.User_Delete(dummyUserId);

			//Verify
			remoteUser = spiraSoapClient.User_RetrieveById(dummyUserId);
			Assert.IsFalse(remoteUser.Active);

			//Only makes it inactive now, so do the real delete using business method
			new UserManager().DeleteUser(dummyUserId, true);

			//Verify
			remoteUser = spiraSoapClient.User_RetrieveById(dummyUserId);
			Assert.IsNull(remoteUser);
		}
		...
		public void _05_ImportCustomProperties()
		{
			...
			RemoteCustomProperty[] remoteCustomProperties = spiraSoapClient.CustomProperty_RetrieveForArtifactType((int)DataModel.Artifact.ArtifactTypeEnum.Requirement, false);
			...
			customListId1 = spiraSoapClient.CustomProperty_AddCustomList(remoteCustomList).CustomPropertyListId.Value;
			...
			customValueId2 = spiraSoapClient.CustomProperty_AddCustomListValue(remoteCustomListValue).CustomPropertyValueId.Value;

			//Lets verify that the list was correctly populated
			remoteCustomList = spiraSoapClient.CustomProperty_RetrieveCustomListById(customListId1);
			...
			remoteCustomList.Active = true;
			remoteCustomList.Values = new RemoteCustomListValue[] { remoteCustomListValue };
			customListId2 = spiraSoapClient.CustomProperty_AddCustomList(remoteCustomList).CustomPropertyListId.Value;

			//Now lets verify that we can retrieve all the lists in the project
			RemoteCustomList[] remoteCustomLists = spiraSoapClient.CustomProperty_RetrieveCustomLists();
			...
			remoteCustomList = spiraSoapClient.CustomProperty_RetrieveCustomListById(customListId2);
			remoteCustomList.Name = "Component Names";
			remoteCustomList.Values[0].Name = "Component One";
			spiraSoapClient.CustomProperty_UpdateCustomList(remoteCustomList);
			...
			spiraSoapClient.CustomProperty_AddDefinition(remoteCustomProperty, null);
			...
			spiraSoapClient.CustomProperty_AddDefinition(remoteCustomProperty, remoteCustomList.CustomPropertyListId);

			//Verify the additions
			remoteCustomProperties = spiraSoapClient.CustomProperty_RetrieveForArtifactType((int)DataModel.Artifact.ArtifactTypeEnum.Requirement, false);
			...
			//Verify the additions
			remoteCustomProperties = spiraSoapClient.CustomProperty_RetrieveForArtifactType((int)DataModel.Artifact.ArtifactTypeEnum.Incident, false);
			Assert.AreEqual(2, remoteCustomProperties.Length);
			Assert.AreEqual("Custom_01", remoteCustomProperties.FirstOrDefault(c => c.Name == "Source").CustomPropertyFieldName);
			Assert.AreEqual("Custom_02", remoteCustomProperties.FirstOrDefault(c => c.Name == "Req Type").CustomPropertyFieldName);
			Assert.AreEqual(1, remoteCustomProperties.FirstOrDefault(c => c.Name == "Source").CustomPropertyTypeId);
			...
			remoteCustomProperty.Options = options.ToArray();
			spiraSoapClient.CustomProperty_AddDefinition(remoteCustomProperty, null);
			...
			remoteCustomProperties = spiraSoapClient.CustomProperty_RetrieveForArtifactType((int)DataModel.Artifact.ArtifactTypeEnum.Task, false);
			...
			Assert.AreEqual(2, remoteCustomProperties[0].Options.Length);
			...
			Assert.AreEqual(3, remoteCustomProperties[1].Options.Length);
			...
			Assert.AreEqual(1, remoteCustomProperties[2].Options.Length);
			...
			Assert.AreEqual(1, remoteCustomProperties[3].Options.Length);
			...
			Assert.AreEqual(1, remoteCustomProperties[4].Options.Length);
			...
			Assert.AreEqual(1, remoteCustomProperties[5].Options.Length);
			...
			{
				spiraSoapClient.CustomProperty_DeleteDefinition(remoteCustomProperty2.CustomPropertyId.Value);
			}
			...
			spiraSoapClient.CustomProperty_AddDefinition(remoteCustomProperty, customListId2);
			remoteCustomProperty = new RemoteCustomProperty();
			remoteCustomProperty.CustomPropertyTypeId = (int)DataModel.CustomProperty.CustomPropertyTypeEnum.List;
			...
			remoteCustomProperties = spiraSoapClient.CustomProperty_RetrieveForArtifactType((int)DataModel.Artifact.ArtifactTypeEnum.TestSet, false);
			Assert.AreEqual(1, remoteCustomProperties.Length);
			remoteCustomProperties = spiraSoapClient.CustomProperty_RetrieveForArtifactType((int)DataModel.Artifact.ArtifactTypeEnum.TestRun, false);
			Assert.AreEqual(1, remoteCustomProperties.Length);
		...
		public void _06_ImportComponents()
		{
			...
			RemoteComponent[] components = spiraSoapClient.Component_Retrieve(true, false);
			Assert.AreEqual(0, components.Length);

			//Now lets add some components
			componentId1 = spiraSoapClient.Component_Create(new RemoteComponent() { Name = "Component 1", IsActive = true }).ComponentId.Value;
			componentId2 = spiraSoapClient.Component_Create(new RemoteComponent() { Name = "Component 2", IsActive = false }).ComponentId.Value;
			componentId3 = spiraSoapClient.Component_Create(new RemoteComponent() { Name = "Component 3", IsActive = true }).ComponentId.Value;

			//Verify they added
			components = spiraSoapClient.Component_Retrieve(true, false);
			...
			components = spiraSoapClient.Component_Retrieve(false, false);
			...
			//Try the other retrieve options
			components = spiraSoapClient.Component_Retrieve(false, true);
			Assert.AreEqual(3, components.Length);
			Assert.AreEqual(projectId1, components[0].ProjectId);
			Assert.AreEqual(componentId1, components[0].ComponentId);
			Assert.AreEqual("Component 1", components[0].Name);
			...
			RemoteComponent component = spiraSoapClient.Component_RetrieveById(componentId2);
			component.IsActive = true;
			component.Name = "Component 2b";
			spiraSoapClient.Component_Update(component);
			...
			spiraSoapClient.Component_Delete(componentId2);
			...
			spiraSoapClient.Component_Undelete(componentId2);
		...
		public void _07_ImportReleases()
		{
			...
			spiraSoapClient.Connection_Authenticate("aant", "aant123456789");
			spiraSoapClient.Connection_ConnectToProject(projectId1);
			...
			releaseId1 = spiraSoapClient.Release_Create(remoteRelease, null).ReleaseId.Value;
			//Iteration #1
			...
			remoteRelease.ResourceCount = 5;
			iterationId1 = spiraSoapClient.Release_Create(remoteRelease, releaseId1).ReleaseId.Value;
			//Iteration #2
			remoteRelease = new RemoteRelease();
			...
			remoteRelease.ResourceCount = 5;
			iterationId2 = spiraSoapClient.Release_Create(remoteRelease, releaseId1).ReleaseId.Value;

			//Now verify that the releases/iterations inserted correctly
			//First retrieve the release
			remoteRelease = spiraSoapClient.Release_RetrieveById(releaseId1);
			...
			remoteRelease = spiraSoapClient.Release_RetrieveById(iterationId1);
			Assert.AreEqual(4, remoteRelease.ArtifactTypeId);
			...
			remoteRelease = spiraSoapClient.Release_RetrieveById(iterationId2);
			remoteRelease.VersionNumber = "1.0.002b";
			remoteRelease.Description = "Second Sprint";
			spiraSoapClient.Release_Update(remoteRelease);
		...
		public void _08_ImportRequirements()
		{
			...
			remoteRequirement = spiraSoapClient.Requirement_Create1(remoteRequirement, 0);
			...
			remoteRequirement = spiraSoapClient.Requirement_Create1(remoteRequirement, 1);
			...
			//Add some custom property values
			remoteRequirement.CustomProperties = new RemoteArtifactCustomProperty[2];
			remoteRequirement.CustomProperties[0] = new RemoteArtifactCustomProperty() { PropertyNumber = 1, StringValue = "test value2" };
			remoteRequirement.CustomProperties[1] = new RemoteArtifactCustomProperty() { PropertyNumber = 2, IntegerValue = customValueId1 };
			remoteRequirement = spiraSoapClient.Requirement_Create1(remoteRequirement, 0);
			...
			spiraSoapClient.Requirement_AddStep(remoteRequirementStep, null, null);
			remoteRequirementStep = new RemoteRequirementStep();
			remoteRequirementStep.RequirementId = requirementId5;
			remoteRequirementStep.Description = "Requirement 4 Step 3";
			remoteRequirementStep.CreationDate = DateTime.UtcNow;
			remoteRequirementStep = spiraSoapClient.Requirement_AddStep(remoteRequirementStep, null, USER_ID_FRED_BLOGGS);
			...
			remoteRequirementStep.CreationDate = DateTime.UtcNow;
			spiraSoapClient.Requirement_AddStep(remoteRequirementStep, requirementStepId, null);

			//First retrieve the parent requirement
			remoteRequirement = spiraSoapClient.Requirement_RetrieveById(requirementId1);
			...
			remoteRequirement = spiraSoapClient.Requirement_RetrieveById(requirementId3);
			Assert.AreEqual(1, remoteRequirement.ArtifactTypeId);
			...
			remoteRequirement = spiraSoapClient.Requirement_RetrieveById(requirementId4);
			Assert.AreEqual(1, remoteRequirement.ArtifactTypeId);
			...
			remoteRequirement.Description = String.Empty;
			remoteRequirement.AuthorId = userId1;
			remoteRequirement = spiraSoapClient.Requirement_Create2(remoteRequirement, requirementId5);
			int requirementId6 = remoteRequirement.RequirementId.Value;
			//Verify insertion data
			remoteRequirement = spiraSoapClient.Requirement_RetrieveById(requirementId6);
			Assert.AreEqual("Test Child 1", remoteRequirement.Name);
			//Verify that parent became a summary
			remoteRequirement = spiraSoapClient.Requirement_RetrieveById(requirementId5);
			...
			//Verify insertion data
			remoteRequirement = spiraSoapClient.Requirement_RetrieveById(requirementId7);
			...
			spiraSoapClient.Requirement_Update(remoteRequirement);
			...
			spiraSoapClient.Requirement_Indent(requirementId3);
			remoteRequirement = spiraSoapClient.Requirement_RetrieveById(requirementId3);
			Assert.AreEqual("AAAAAAAAA", remoteRequirement.IndentLevel);
			remoteRequirement = spiraSoapClient.Requirement_RetrieveById(requirementId2);
			Assert.AreEqual(true, remoteRequirement.Summary);

			//Now put it back
			spiraSoapClient.Requirement_Outdent(requirementId3);
			remoteRequirement = spiraSoapClient.Requirement_RetrieveById(requirementId3);
			Assert.AreEqual("AAAAAB", remoteRequirement.IndentLevel);
			remoteRequirement = spiraSoapClient.Requirement_RetrieveById(requirementId2);
			...
			remoteRequirement = spiraSoapClient.Requirement_RetrieveById(requirementId5);
			Assert.AreEqual(3, remoteRequirement.Steps.Length);
			...
			RemoteRequirementStep[] requirementSteps = spiraSoapClient.Requirement_RetrieveSteps(requirementId5);
			...
			remoteRequirementStep = spiraSoapClient.Requirement_RetrieveStepById(requirementStepId);
			remoteRequirementStep.Description = "Requirement 4 Step 1A";
			spiraSoapClient.Requirement_UpdateStep(remoteRequirementStep);
			...
			spiraSoapClient.Requirement_MoveStep(requirementId5, requirementStepId, null);

			//Verify the move
			requirementSteps = spiraSoapClient.Requirement_RetrieveSteps(requirementId5);
			Assert.AreEqual(3, requirementSteps.Length);
			...
			//Now move it back
			spiraSoapClient.Requirement_MoveStep(requirementId5, requirementStepId, requirementSteps[0].RequirementStepId.Value);
			...
			spiraSoapClient.Requirement_DeleteStep(requirementId5, requirementSteps[1].RequirementStepId.Value);
		...
		public void _09_ImportTestCases()
		{
			...
			remoteTestCaseFolder = spiraSoapClient.TestCase_CreateFolder(remoteTestCaseFolder);
			...
			remoteTestCase.ComponentIds = new int[] { componentId2, componentId3 };
			remoteTestCase = spiraSoapClient.TestCase_Create(remoteTestCase);
			...
			remoteTestCase.ComponentIds = new int[] { componentId1, componentId2 };
			remoteTestCase = spiraSoapClient.TestCase_Create(remoteTestCase);
			...
			remoteTestCaseFolder = spiraSoapClient.TestCase_RetrieveFolderById(testFolderId1);
			...
			remoteTestCaseFolder = spiraSoapClient.TestCase_RetrieveFolderById(testFolderId2);
			...
			remoteTestCase = spiraSoapClient.TestCase_RetrieveById(testCaseId4);
			...
			spiraSoapClient.TestCase_AddParameter(remoteTestCaseParameter);
			...
			RemoteTestCaseParameter[] remoteTestCaseParameters = spiraSoapClient.TestCase_RetrieveParameters(testCaseId2);
			...
			remoteTestCase.AuthorId = userId2;
			remoteTestCase.OwnerId = userId1;
			spiraSoapClient.TestCase_Update(remoteTestCase);
			...
			RemoteTestCaseFolder[] remoteTestCaseFolders = spiraSoapClient.TestCase_RetrieveFolders();
		...
		public void _10_ImportTestSteps()
		{
			...
			remoteTestStep = spiraSoapClient.TestCase_AddStep(remoteTestStep, testCaseId3);
			...
			remoteTestStep.ExpectedResult = "It should work";
			remoteTestStep.SampleData = "test 123";
			remoteTestStep = spiraSoapClient.TestCase_AddStep(remoteTestStep, testCaseId4);
			...
			RemoteTestCase remoteTestCase = spiraSoapClient.TestCase_RetrieveById(testCaseId3);
			Assert.AreEqual(2, remoteTestCase.ArtifactTypeId);
			...
			testStepId5 = spiraSoapClient.TestCase_AddLink(testCaseId1, 1, testCaseId3, null);
			...
			testStepId6 = spiraSoapClient.TestCase_AddLink(testCaseId1, 2, testCaseId2, parameterArray);

			//Retrieve the linked test case steps
			remoteTestCase = spiraSoapClient.TestCase_RetrieveById(testCaseId1);
			...
			RemoteTestStepParameter[] remoteTestStepParameters = spiraSoapClient.TestCase_RetrieveStepParameters(testCaseId1, testStepId6);
		...
		public void _11_ImportCoverage()
		{
			...
			spiraSoapClient.Requirement_AddTestCoverage(remoteRequirementTestCaseMapping);
			//Entry 2
			...
			RemoteRequirementTestCaseMapping[] requirementTestCaseMappings = spiraSoapClient.Requirement_RetrieveTestCoverage(requirementId2);
			...
			remoteRequirementTestCaseMapping.RequirementId = requirementId2;
			remoteRequirementTestCaseMapping.TestCaseId = testCaseId1;
			spiraSoapClient.Requirement_RemoveTestCoverage(remoteRequirementTestCaseMapping);
			//Entry 2
			remoteRequirementTestCaseMapping = new RemoteRequirementTestCaseMapping();
			remoteRequirementTestCaseMapping.RequirementId = requirementId2;
			remoteRequirementTestCaseMapping.TestCaseId = testCaseId2;
			...
			//Now verify the coverage data
			requirementTestCaseMappings = spiraSoapClient.Requirement_RetrieveTestCoverage(requirementId2);
			...
			RemoteReleaseTestCaseMapping[] releaseTestCaseMappings = spiraSoapClient.Release_RetrieveTestMapping(releaseId1);
			Assert.AreEqual(3, releaseTestCaseMappings.Length);
			releaseTestCaseMappings = spiraSoapClient.Release_RetrieveTestMapping(iterationId1);
			Assert.AreEqual(0, releaseTestCaseMappings.Length);
			releaseTestCaseMappings = spiraSoapClient.Release_RetrieveTestMapping(iterationId2);
			Assert.AreEqual(0, releaseTestCaseMappings.Length);
			...
			spiraSoapClient.Release_AddTestMapping(remoteReleaseTestCaseMapping);
			...
			releaseTestCaseMappings = spiraSoapClient.Release_RetrieveTestMapping(releaseId1);
			Assert.AreEqual(3, releaseTestCaseMappings.Length);
			releaseTestCaseMappings = spiraSoapClient.Release_RetrieveTestMapping(iterationId1);
			...
			remoteReleaseTestCaseMapping.ReleaseId = releaseId1;
			remoteReleaseTestCaseMapping.TestCaseId = testCaseId1;
			spiraSoapClient.Release_RemoveTestMapping(remoteReleaseTestCaseMapping);
			//Iteration 1
			remoteReleaseTestCaseMapping = new RemoteReleaseTestCaseMapping();
			remoteReleaseTestCaseMapping.ReleaseId = iterationId1;
			remoteReleaseTestCaseMapping.TestCaseId = testCaseId1;
			...
			remoteReleaseTestCaseMapping.TestCaseId = testCaseId1;
			remoteReleaseTestCaseMappings.Add(remoteReleaseTestCaseMapping);
			spiraSoapClient.Release_AddTestMapping2(remoteReleaseTestCaseMappings.ToArray());
		...
		public void _12_ImportTestSets()
		{
			...
			testSetFolderId1 = spiraSoapClient.TestSet_CreateFolder(remoteTestSetFolder).TestSetFolderId.Value;
			...
			remoteTestSet.CustomProperties = new RemoteArtifactCustomProperty[1] { new RemoteArtifactCustomProperty() { PropertyNumber = 1, IntegerValue = customValueId3 } };
			testSetId1 = spiraSoapClient.TestSet_Create(remoteTestSet).TestSetId.Value;

			//Verify that it inserted correctly
			remoteTestSet = spiraSoapClient.TestSet_RetrieveById(testSetId1);
			...
			Assert.AreEqual(1, remoteTestSet.CustomProperties.Length);
			...
			testSetId2 = spiraSoapClient.TestSet_Create(remoteTestSet).TestSetId.Value;
			//Verify that it inserted correctly
			remoteTestSet = spiraSoapClient.TestSet_RetrieveById(testSetId2);
			Assert.AreEqual(8, remoteTestSet.ArtifactTypeId);
			...
			RemoteTestSetTestCaseMapping[] remoteTestSetTestCaseMappings = spiraSoapClient.TestSet_AddTestMapping(remoteTestSetTestCaseMapping, null, null);
			...
			remoteTestSetTestCaseMappings = spiraSoapClient.TestSet_AddTestMapping(remoteTestSetTestCaseMapping, null, null);
			Assert.AreEqual(1, remoteTestSetTestCaseMappings.Length);
			Assert.AreEqual(testSetId1, remoteTestSetTestCaseMappings[0].TestSetId);
			...
			//Now verify the data
			RemoteTestCase[] remoteTestCases = spiraSoapClient.TestCase_RetrieveByTestSetId(testSetId1);
			...
			remoteTestSetTestCaseMappings = spiraSoapClient.TestSet_RetrieveTestCaseMapping(testSetId1);
			...
			remoteTestSetTestCaseMapping.OwnerId = USER_ID_FRED_BLOGGS;
			spiraSoapClient.TestSet_AddTestMapping(remoteTestSetTestCaseMapping, existingTestSetTestCaseId, parameters.ToArray());

			//Now verify the data
			remoteTestCases = spiraSoapClient.TestCase_RetrieveByTestSetId(testSetId1);
		...
		public void _13_ImportTestRun()
		{
			...
			RemoteManualTestRun[] remoteManualTestRuns = spiraSoapClient.TestRun_CreateFromTestCases(new int[] { testCaseId3, testCaseId4 }, iterationId1);
			...
			remoteManualTestRuns = spiraSoapClient.TestRun_Save(remoteManualTestRuns, DateTime.UtcNow);
			...
			RemoteTestRun remoteTestRun = spiraSoapClient.TestRun_RetrieveById(testRunId1);
			...
			RemoteManualTestRun remoteManualTestRun = spiraSoapClient.TestRun_RetrieveManualById(testRunId1);
			...
			remoteManualTestRuns = spiraSoapClient.TestRun_CreateFromTestSet(testSetId1);
			...
			remoteManualTestRuns = spiraSoapClient.TestRun_Save(remoteManualTestRuns, DateTime.UtcNow.AddMinutes(1));
			testRunId1 = remoteManualTestRuns[0].TestRunId.Value;
			testRunId2 = remoteManualTestRuns[1].TestRunId.Value;
			testRunStepId1 = remoteManualTestRuns[0].TestRunSteps[0].TestRunStepId.Value;
			testRunStepId2 = remoteManualTestRuns[0].TestRunSteps[1].TestRunStepId.Value;
			...
			remoteTestCase = spiraSoapClient.TestCase_RetrieveById(testCaseId3);
			Assert.AreEqual((int)TestCase.ExecutionStatusEnum.Caution, remoteTestCase.ExecutionStatusId, "TestCase");
			Assert.IsNotNull(remoteTestCase.ExecutionDate);

			//Now check the overall test set status (available since v4.0)
			RemoteTestSet remoteTestSet = spiraSoapClient.TestSet_RetrieveById(testSetId1);
			Assert.AreEqual(0, remoteTestSet.CountBlocked);
			Assert.AreEqual(1, remoteTestSet.CountCaution);
			Assert.AreEqual(1, remoteTestSet.CountFailed);
			Assert.AreEqual(0, remoteTestSet.CountNotApplicable);   //Due to the linked step
			Assert.AreEqual(1, remoteTestSet.CountNotRun);
			Assert.AreEqual(0, remoteTestSet.CountPassed);

			//Now check the test run
			remoteManualTestRun = spiraSoapClient.TestRun_RetrieveManualById(testRunId1);
			Assert.AreEqual((int)TestCase.ExecutionStatusEnum.Failed, remoteManualTestRun.ExecutionStatusId, "TestRun");
			...
			testRunId3 = spiraSoapClient.TestRun_RecordAutomated1(remoteAutomatedTestRun).TestRunId.Value;

			//Now retrieve the test run to check that it saved correctly - using the web service
			remoteAutomatedTestRun = spiraSoapClient.TestRun_RetrieveAutomatedById(testRunId3);
			...
			remoteAutomatedTestRun.TestRunFormatId = 1; //Plain Text
			testRunId4 = spiraSoapClient.TestRun_RecordAutomated1(remoteAutomatedTestRun).TestRunId.Value;

			//Now retrieve the test run to check that it saved correctly - using the web service
			remoteAutomatedTestRun = spiraSoapClient.TestRun_RetrieveAutomatedById(testRunId4);
			...
			testRunId5 = spiraSoapClient.TestRun_RecordAutomated2("aant", "aant123456789", projectId1, userId2, testCaseId1, iterationId2, null, null, null, DateTime.UtcNow, DateTime.UtcNow.AddSeconds(20), (int)TestCase.ExecutionStatusEnum.Failed, "TestSuite", "02_Test_Method", 5, "Expected 1, Found 0", "Error Stack Trace........", 1);

			//Now retrieve the test run to check that it saved correctly - using the web service
			//Need to restore the use of cookies first
			httpBinding = (BasicHttpBinding)spiraSoapClient.Endpoint.Binding;
			httpBinding.AllowCookies = true;
			remoteAutomatedTestRun = spiraSoapClient.TestRun_RetrieveAutomatedById(testRunId5);
			...
			remoteAutomatedTestRuns.Add(remoteAutomatedTestRun);
			RemoteAutomatedTestRun[] remoteAutomatedTestRuns2 = spiraSoapClient.TestRun_RecordAutomated3(remoteAutomatedTestRuns.ToArray());
			...
			spiraSoapClient.Connection_Authenticate("administrator", "Welcome123$");
			spiraSoapClient.Connection_ConnectToProject(projectId1);
			spiraSoapClient.Project_RefreshProgressExecutionStatusCaches(null, false);
			spiraSoapClient.Connection_Authenticate("aant", "aant123456789");
			spiraSoapClient.Connection_ConnectToProject(projectId1);

			//Verify the data
			remoteAutomatedTestRun = spiraSoapClient.TestRun_RetrieveAutomatedById(batchTestRunId1);
			Assert.AreEqual((int)TestCase.ExecutionStatusEnum.Caution, remoteAutomatedTestRun.ExecutionStatusId);
			Assert.AreEqual(1, remoteAutomatedTestRun.TestRunFormatId);
			Assert.IsNull(remoteAutomatedTestRun.TestRunSteps);
			remoteAutomatedTestRun = spiraSoapClient.TestRun_RetrieveAutomatedById(batchTestRunId2);
			...
			remoteAutomatedTestRun.TestRunFormatId = 2;    //HTML
														   //Test Run Steps
			remoteAutomatedTestRun.TestRunSteps = new RemoteTestRunStep[3];
			//Test Step #1 Failed - linked to a real test step
			remoteAutomatedTestRun.TestRunSteps[0] = new RemoteTestRunStep();
			remoteAutomatedTestRun.TestRunSteps[0].Description = "Verifying that winMain displays title";
			remoteAutomatedTestRun.TestRunSteps[0].ExpectedResult = "Title displayed";
			remoteAutomatedTestRun.TestRunSteps[0].SampleData = "test 123";
			remoteAutomatedTestRun.TestRunSteps[0].ActualResult = "Object Not Available on Screen, see <img class=\"rapise_report_embedded_image img-responsive\" src=\"#{cb267200-470e-4b6c-8eed-64a5210eb699}\" scale=\"0\" alt=\"1.DoAction([]) returned true\">.";
			remoteAutomatedTestRun.TestRunSteps[0].ExecutionStatusId = 1;   //Failed
			remoteAutomatedTestRun.TestRunSteps[0].TestStepId = testStepId1;
			//Test Step #2 Caution - not linked to a test step
			remoteAutomatedTestRun.TestRunSteps[1] = new RemoteTestRunStep();
			remoteAutomatedTestRun.TestRunSteps[1].Description = "txtName = 'The Scowrers'";
			remoteAutomatedTestRun.TestRunSteps[1].ExpectedResult = "The Scowrers";
			remoteAutomatedTestRun.TestRunSteps[1].ActualResult = "the scowrers, see <img class=\"rapise_report_embedded_image img-responsive\" src=\"#{6743731B-C6FD-4DF7-93F3-872960AD26B2}\" scale=\"0\" alt=\"1.DoAction([]) returned true\">.";
			remoteAutomatedTestRun.TestRunSteps[1].ExecutionStatusId = 6;   //Caution
																			//Test Step #3 Passed - not linked to a test step
			remoteAutomatedTestRun.TestRunSteps[2] = new RemoteTestRunStep();
			remoteAutomatedTestRun.TestRunSteps[2].Description = "cboAuthor Matches Author";
			remoteAutomatedTestRun.TestRunSteps[2].ExpectedResult = "AuthorId=5";
			remoteAutomatedTestRun.TestRunSteps[2].ExecutionStatusId = 2;   //Passed
			testRunId6 = spiraSoapClient.TestRun_RecordAutomated1(remoteAutomatedTestRun).TestRunId.Value;
			...
			remoteDocument.ProjectAttachmentFolderId = null;    //Use default
			remoteDocument.FilenameOrUrl = "Step1.png";
			remoteDocument.Description = "#{cb267200-470e-4b6c-8eed-64a5210eb699}";
			...
			remoteDocument.AttachedArtifacts = linkedArtifacts.ToArray();
			remoteDocument = spiraSoapClient.Document_AddFile(remoteDocument, dummyImage);
			...
			remoteDocument.ProjectAttachmentFolderId = null;    //Use default
			remoteDocument.FilenameOrUrl = "Step2.png";
			remoteDocument.Description = "#{6743731B-C6FD-4DF7-93F3-872960AD26B2}";
			...
			string realUrl1 = spiraSoapClient.System_GetArtifactUrl(/*Attachment*/-14, projectId1, attachmentId1, null);
			realUrl1 = realUrl1.Replace("~", spiraSoapClient.System_GetWebServerUrl());
			string realUrl2 = spiraSoapClient.System_GetArtifactUrl(/*Attachment*/-14, projectId1, attachmentId2, null);
			realUrl2 = realUrl2.Replace("~", spiraSoapClient.System_GetWebServerUrl());

			//Test Run
			remoteAutomatedTestRun = spiraSoapClient.TestRun_RetrieveAutomatedById(testRunId6);
			...
			Assert.AreEqual(2, remoteAutomatedTestRun.TestRunFormatId);
			//Test Run Steps
			Assert.AreEqual(3, remoteAutomatedTestRun.TestRunSteps.Length);
			//Step 1
			...
			spiraSoapClient.Document_Delete(remoteDocument.AttachmentId.Value);
		}
		...
		public void _14_ImportIncidents()
		{
			...
			RemoteIncidentType[] remoteIncidentTypes = spiraSoapClient.Incident_RetrieveTypes();
			Assert.AreEqual(remoteIncidentTypes.Length, 8);
			RemoteIncidentStatus[] remoteIncidentStatuses = spiraSoapClient.Incident_RetrieveStatuses();
			Assert.AreEqual(remoteIncidentStatuses.Length, 8);
			RemoteIncidentPriority[] remoteIncidentPriorities = spiraSoapClient.Incident_RetrievePriorities();
			Assert.AreEqual(remoteIncidentPriorities.Length, 4);
			RemoteIncidentSeverity[] remoteIncidentSeverities = spiraSoapClient.Incident_RetrieveSeverities();
			Assert.AreEqual(remoteIncidentSeverities.Length, 4);

			//Also verify that we can get the default status and type for the project
			RemoteIncidentType defaultRemoteIncidentType = spiraSoapClient.Incident_RetrieveDefaultType();
			Assert.AreEqual("Incident", defaultRemoteIncidentType.Name);
			Assert.AreEqual(true, defaultRemoteIncidentType.Active);
			RemoteIncidentStatus defaultRemoteIncidentStatus = spiraSoapClient.Incident_RetrieveDefaultStatus();
			...
			incidentTypeId = spiraSoapClient.Incident_AddType(remoteIncidentType).IncidentTypeId.Value;
			//Status
			...
			incidentStatusId = spiraSoapClient.Incident_AddStatus(remoteIncidentStatus).IncidentStatusId.Value;
			//Priority
			...
			int priorityId = spiraSoapClient.Incident_AddPriority(remoteIncidentPriority).PriorityId.Value;
			//Severity
			...
			int severityId = spiraSoapClient.Incident_AddSeverity(remoteIncidentSeverity).SeverityId.Value;
			...
			remoteIncident.ComponentIds = new int[] { componentId1, componentId2 };
			//Add some custom property values
			remoteIncident.CustomProperties = new RemoteArtifactCustomProperty[2];
			remoteIncident.CustomProperties[0] = new RemoteArtifactCustomProperty() { PropertyNumber = 1, StringValue = "test value" };
			remoteIncident.CustomProperties[1] = new RemoteArtifactCustomProperty() { PropertyNumber = 2, IntegerValue = customValueId2 };
			incidentId1 = spiraSoapClient.Incident_Create(remoteIncident).IncidentId.Value;
			//Incident #2
			...
			remoteIncident.ComponentIds = new int[] { componentId2, componentId3 };
			incidentId2 = spiraSoapClient.Incident_Create(remoteIncident).IncidentId.Value;
			//Incident #3
			remoteIncident = new RemoteIncident();
			remoteIncident.IncidentTypeId = incidentTypeId;
			remoteIncident.IncidentStatusId = incidentStatusId;
			...
			remoteIncident.OpenerId = userId1;
			remoteIncident.OwnerId = userId2;
			remoteIncident.TestRunStepIds = new int[] { testRunStepId2 };
			remoteIncident.ClosedDate = DateTime.UtcNow;
			incidentId3 = spiraSoapClient.Incident_Create(remoteIncident).IncidentId.Value;
			...
			spiraSoapClient.Incident_AddComments(remoteComments);

			//Now verify that the import was successful

			//Retrieve the first incident
			remoteIncident = spiraSoapClient.Incident_RetrieveById(incidentId1);
			...
			remoteComments = spiraSoapClient.Incident_RetrieveComments(incidentId1);
			...
			remoteIncident = spiraSoapClient.Incident_RetrieveById(incidentId2);
			Assert.AreEqual(3, remoteIncident.ArtifactTypeId);
			Assert.AreEqual(incidentTypeId, remoteIncident.IncidentTypeId, "IncidentTypeId");
			...
			remoteComments = spiraSoapClient.Incident_RetrieveComments(incidentId2);
			Assert.AreEqual(0, remoteComments.Length);

			//Retrieve the third incident
			remoteIncident = spiraSoapClient.Incident_RetrieveById(incidentId3);
			Assert.AreEqual(3, remoteIncident.ArtifactTypeId);
			Assert.AreEqual(incidentTypeId, remoteIncident.IncidentTypeId, "IncidentTypeId");
			Assert.AreEqual(incidentStatusId, remoteIncident.IncidentStatusId, "IncidentStatusId");
			...
			remoteComments = spiraSoapClient.Incident_RetrieveComments(incidentId3);
			...
			spiraSoapClient.Incident_Update(remoteIncident);
			...
			//Test Run Step
			RemoteIncident[] remoteIncidents = spiraSoapClient.Incident_RetrieveByTestRunStep(testRunStepId2);
			Assert.AreEqual(1, remoteIncidents.Length);
			Assert.AreEqual(incidentId3, remoteIncidents[0].IncidentId);
			//Test Step
			remoteIncidents = spiraSoapClient.Incident_RetrieveByTestStep(testStepId2);
			Assert.AreEqual(1, remoteIncidents.Length);
			Assert.AreEqual(incidentId3, remoteIncidents[0].IncidentId);
			//Test Case
			remoteIncidents = spiraSoapClient.Incident_RetrieveByTestCase(testCaseId3, false);
		...
		public void _15_ImportAttachments()
		{
			...
			remoteDocument.AttachedArtifacts = new RemoteLinkedArtifact[] { remoteLinkedArtifact };
			attachmentId1 = spiraSoapClient.Document_AddFile(remoteDocument, attachmentData).AttachmentId.Value;
			...
			remoteDocument.AuthorId = userId2;
			remoteDocument.AttachedArtifacts = new RemoteLinkedArtifact[] { remoteLinkedArtifact };
			attachmentId2 = spiraSoapClient.Document_AddFile(remoteDocument, attachmentData).AttachmentId.Value;

			//Now lets get the attachment meta-data and verify
			remoteDocument = spiraSoapClient.Document_RetrieveById(attachmentId1);
			...
			remoteLinkedArtifact.ArtifactTypeId = (int)DataModel.Artifact.ArtifactTypeEnum.TestCase;
			remoteDocument.AttachedArtifacts = new RemoteLinkedArtifact[] { remoteLinkedArtifact };
			attachmentId3 = spiraSoapClient.Document_AddUrl(remoteDocument).AttachmentId.Value;

			//Now lets get the attachment meta-data and verify
			remoteDocument = spiraSoapClient.Document_RetrieveById(attachmentId3);
		...
		public void _16_ImportTasks()
		{
			...
			remoteTaskFolder = spiraSoapClient.Task_CreateFolder(remoteTaskFolder);
			...
			taskId1 = spiraSoapClient.Task_Create(remoteTask).TaskId.Value;

			//Verify the data
			remoteTask = spiraSoapClient.Task_RetrieveById(taskId1);
			...
			taskId2 = spiraSoapClient.Task_Create(remoteTask).TaskId.Value;

			//Verify the data
			remoteTask = spiraSoapClient.Task_RetrieveById(taskId2);
			...
			spiraSoapClient.Task_Update(remoteTask);
			...
			RemoteTaskFolder[] taskFolders = spiraSoapClient.Task_RetrieveFolders();
			...
			taskFolders = spiraSoapClient.Task_RetrieveFoldersByParent(null);
			Assert.AreEqual(1, taskFolders.Length);
			Assert.AreEqual("Task Folder A", taskFolders[0].Name);
			taskFolders = spiraSoapClient.Task_RetrieveFoldersByParent(taskFolderId1);
			Assert.AreEqual(1, taskFolders.Length);
			Assert.AreEqual("Task Folder B", taskFolders[0].Name);

			//Verify that we can update one of the folders
			remoteTaskFolder = spiraSoapClient.Task_RetrieveFolderById(taskFolderId1);
			remoteTaskFolder.Name = "Task Folder A1";
			spiraSoapClient.Task_UpdateFolder(remoteTaskFolder);
			...
			spiraSoapClient.Task_DeleteFolder(taskFolderId2);
			remoteTask = spiraSoapClient.Task_RetrieveById(taskId2);
		...
		public void _17_RetrieveIncidents()
		{
			...
			RemoteIncident[] remoteIncidents = spiraSoapClient.Incident_RetrieveNew(DateTime.Parse("1/1/2000"), 1, 100);
			...
			Assert.AreEqual("Adam Ant", remoteIncident.OpenerName, "OpenerName");
			Assert.IsNull(remoteIncident.OwnerName, "OwnerName");
			RemoteComment[] remoteComments = spiraSoapClient.Incident_RetrieveComments(incidentId1);
			Assert.AreEqual(0, remoteComments.Length);
			...
			remoteIncidents = spiraSoapClient.Incident_Retrieve(remoteFilters.ToArray(), remoteSort, 1, 999999);
			...
			remoteIncidents = spiraSoapClient.Incident_RetrieveForOwner();
		...
		public void _18_RetrieveTasks()
		{
			...
			RemoteTask[] remoteTasks = spiraSoapClient.Task_RetrieveNew(DateTime.Parse("1/1/2000"), 1, 100);
			...
			remoteSort.SortAscending = true;
			remoteTasks = spiraSoapClient.Task_Retrieve(remoteFilters.ToArray(), remoteSort, 1, 999999);
			...
			remoteTasks = spiraSoapClient.Task_RetrieveForOwner();
		...
		public void _19_RetrieveRequirements()
		{
			...
			remoteFilters.Add(remoteFilter);
			RemoteRequirement[] remoteRequirements = spiraSoapClient.Requirement_Retrieve(remoteFilters.ToArray(), 1, 999999);
			...
			remoteRequirements = spiraSoapClient.Requirement_RetrieveForOwner();
			...
			RemoteRequirementType[] requirementTypes = spiraSoapClient.Requirement_RetrieveTypes();
			...
			RemoteRequirementStatus[] requirementStatuses = spiraSoapClient.Requirement_RetrieveStatuses();
			Assert.AreEqual(17, requirementStatuses.Length);
		...
		public void _20_RetrieveTestCases()
		{
			...
			remoteFilter.DateRangeValue.EndDate = DateTime.UtcNow;
			remoteFilters.Add(remoteFilter);
			RemoteTestCase[] remoteTestCases = spiraSoapClient.TestCase_Retrieve(remoteFilters.ToArray(), remoteSort, 1, 999999, null);
			...
			remoteTestCases = spiraSoapClient.TestCase_Retrieve(remoteFilters.ToArray(), remoteSort, 1, 999999, 1);
			Assert.AreEqual(5, remoteTestCases.Length);
			Assert.AreEqual("Ability to create new author", remoteTestCases[0].Name);
			...
			RemoteTestRun[] remoteTestRuns = spiraSoapClient.TestRun_Retrieve(remoteFilters.ToArray(), remoteSort, 1, 999999);
			...
			remoteTestCases = spiraSoapClient.TestCase_RetrieveForOwner();
			...
			remoteSort.PropertyName = "Name";
			remoteSort.SortAscending = true;
			remoteTestCases = spiraSoapClient.TestCase_RetrieveByFolder(1, null, remoteSort, 1, 999999, null);
			...
			RemoteTestCaseType[] testCaseTypes = spiraSoapClient.TestCase_RetrieveTypes();
			...
			RemoteTestCaseStatus[] testCaseStatuses = spiraSoapClient.TestCase_RetrieveStatuses();
		...
		public void _21_RetrieveReleases()
		{
			...
			remoteFilters.Add(remoteFilter);
			RemoteRelease[] remoteReleases = spiraSoapClient.Release_Retrieve2(remoteFilters.ToArray(), 1, 999999);
			...
			RemoteReleaseType[] releaseTypes = spiraSoapClient.Release_RetrieveTypes();
			...
			RemoteReleaseStatus[] releaseStatuses = spiraSoapClient.Release_RetrieveStatuses();
		...
		public void _22_RetrieveTestSets()
		{
			...
			//Verify the data returned
			Assert.AreEqual(2, remoteTestSets.Length);
			...
			remoteFilter.DateRangeValue.EndDate = DateTime.UtcNow.AddDays(-8);
			remoteFilters.Add(remoteFilter);
			remoteTestSets = spiraSoapClient.TestSet_Retrieve(remoteFilters.ToArray(), remoteSort, 1, 999999, 1);
			...
			remoteTestSets = spiraSoapClient.TestSet_RetrieveForOwner();
		...
		public void _23_RetrieveVersion()
		{
			...
			RemoteVersion remoteVersion = spiraSoapClient.System_GetProductVersion();
		...
		public void _24_RetrieveSettings()
		{
			...
			spiraSoapClient.Connection_Authenticate("fredbloggs", "PleaseChange");
			// Pull the version object.
			RemoteSetting[] remoteSettings = spiraSoapClient.System_GetSettings();
			// Verify they meet requirements.
			...
			string url = spiraSoapClient.System_GetArtifactUrl(1, 2, 5, "Comments");
			...
			url = spiraSoapClient.System_GetArtifactUrl(2, 3, -2, "TestSteps");
		...
		public void _25_RetrieveProjectRoles()
		{
			...
			RemoteProjectRole[] remoteProjectRoles = spiraSoapClient.ProjectRole_Retrieve();
			...
			Assert.AreEqual(61, remoteProjectRoles[0].Permissions.Length);
		...
		public void _26_Concurrency_Handling()
		{
			...
			int taskId = spiraSoapClient.Task_Create(remoteTask).TaskId.Value;

			//Now retrieve the task back into two copies
			RemoteTask remoteTask1 = spiraSoapClient.Task_RetrieveById(taskId);
			RemoteTask remoteTask2 = spiraSoapClient.Task_RetrieveById(taskId);

			//Now make a change to field and update
			remoteTask1.TaskPriorityId = tk_priorityHighId;
			spiraSoapClient.Task_Update(remoteTask1);

			//Verify it updated correctly using separate data object
			RemoteTask remoteTask3 = spiraSoapClient.Task_RetrieveById(taskId);
			...
			try
			{
				...
				spiraSoapClient.Task_Update(remoteTask2);
			}
			...
			remoteTask3 = spiraSoapClient.Task_RetrieveById(taskId);
			Assert.AreEqual(tk_priorityHighId, remoteTask3.TaskPriorityId);

			//Now refresh the old dataset and try again and verify it works
			remoteTask2 = spiraSoapClient.Task_RetrieveById(taskId);
			remoteTask2.TaskPriorityId = tk_priorityLowId;
			spiraSoapClient.Task_Update(remoteTask2);
			...
			int incidentId = spiraSoapClient.Incident_Create(remoteIncident).IncidentId.Value;

			//Now retrieve the incident back into two copies
			RemoteIncident remoteIncident1 = spiraSoapClient.Incident_RetrieveById(incidentId);
			RemoteIncident remoteIncident2 = spiraSoapClient.Incident_RetrieveById(incidentId);

			//Now make a change to field and update
			remoteIncident1.Name = "New Incident 1 Mod001";
			spiraSoapClient.Incident_Update(remoteIncident1);

			//Verify it updated correctly using separate data object
			RemoteIncident remoteIncident3 = spiraSoapClient.Incident_RetrieveById(incidentId);
			...
			try
			{
				...
				spiraSoapClient.Incident_Update(remoteIncident2);
			}
			catch (FaultException<ServiceFaultMessage> soapException)
			{
				ServiceFaultMessage detail = soapException.Detail;
			...
			//Verify it didn't update using separate dataset
			remoteIncident3 = spiraSoapClient.Incident_RetrieveById(incidentId);
			Assert.AreEqual("New Incident 1 Mod001", remoteIncident3.Name);

			//Now refresh the old dataset and try again and verify it works
			remoteIncident2 = spiraSoapClient.Incident_RetrieveById(incidentId);
			remoteIncident2.Name = "New Incident 1 Mod002";
			spiraSoapClient.Incident_Update(remoteIncident2);
			...
			spiraSoapClient.Incident_Delete(incidentId);
			...
			RemoteRequirement remoteRequirement1 = spiraSoapClient.Requirement_RetrieveById(requirementId);
			RemoteRequirement remoteRequirement2 = spiraSoapClient.Requirement_RetrieveById(requirementId);

			//Now make a change to field and update
			remoteRequirement1.ImportanceId = rq_importanceHighId;
			spiraSoapClient.Requirement_Update(remoteRequirement1);

			//Verify it updated correctly using separate dataset
			RemoteRequirement remoteRequirement3 = spiraSoapClient.Requirement_RetrieveById(requirementId);
			...
			try
			{
				...
				spiraSoapClient.Requirement_Update(remoteRequirement2);
			}
			catch (FaultException<ServiceFaultMessage> soapException)
			{
				ServiceFaultMessage detail = soapException.Detail;
			...
			remoteRequirement3 = spiraSoapClient.Requirement_RetrieveById(requirementId);
			Assert.AreEqual(rq_importanceHighId, remoteRequirement3.ImportanceId);

			//Now refresh the old data object and try again and verify it works
			remoteRequirement2 = spiraSoapClient.Requirement_RetrieveById(requirementId);
			remoteRequirement2.ImportanceId = rq_importanceLowId;
			spiraSoapClient.Requirement_Update(remoteRequirement2);
			...
			remoteRelease.EndDate = DateTime.UtcNow.AddMonths(3);
			remoteRelease.ResourceCount = 5;
			int releaseId = spiraSoapClient.Release_Create(remoteRelease, null).ReleaseId.Value;

			//Now retrieve the release back and keep a copy of the dataset
			RemoteRelease remoteRelease1 = spiraSoapClient.Release_RetrieveById(releaseId);
			RemoteRelease remoteRelease2 = spiraSoapClient.Release_RetrieveById(releaseId);

			//Now make a change to field and update
			remoteRelease1.DaysNonWorking = 3;
			spiraSoapClient.Release_Update(remoteRelease1);

			//Verify it updated correctly using separate dataset
			RemoteRelease remoteRelease3 = spiraSoapClient.Release_RetrieveById(releaseId);
			...
			try
			{
				...
				spiraSoapClient.Release_Update(remoteRelease2);
			}
			catch (FaultException<ServiceFaultMessage> soapException)
			{
				ServiceFaultMessage detail = soapException.Detail;
			...
			//Verify it didn't update using separate dataset
			remoteRelease3 = spiraSoapClient.Release_RetrieveById(releaseId);
			Assert.AreEqual(3, remoteRelease3.DaysNonWorking);

			//Now refresh the old dataset and try again and verify it works
			remoteRelease2 = spiraSoapClient.Release_RetrieveById(releaseId);
			remoteRelease2.DaysNonWorking = 4;
			spiraSoapClient.Release_Update(remoteRelease2);
			...
			remoteTestSet.TestSetStatusId = (int)TestSet.TestSetStatusEnum.NotStarted;
			remoteTestSet.TestRunTypeId = (int)TestRun.TestRunTypeEnum.Manual;
			int testSetId = spiraSoapClient.TestSet_Create(remoteTestSet).TestSetId.Value;

			//Now retrieve the testSet back and keep a copy of the dataset
			RemoteTestSet remoteTestSet1 = spiraSoapClient.TestSet_RetrieveById(testSetId);
			RemoteTestSet remoteTestSet2 = spiraSoapClient.TestSet_RetrieveById(testSetId);

			//Now make a change to field and update
			remoteTestSet1.TestSetStatusId = (int)TestSet.TestSetStatusEnum.Blocked;
			spiraSoapClient.TestSet_Update(remoteTestSet1);

			//Verify it updated correctly using separate dataset
			RemoteTestSet remoteTestSet3 = spiraSoapClient.TestSet_RetrieveById(testSetId);
			...
			try
			{
				...
				spiraSoapClient.TestSet_Update(remoteTestSet2);
			}
			catch (FaultException<ServiceFaultMessage> soapException)
			{
				ServiceFaultMessage detail = soapException.Detail;
			...
			//Verify it didn't update using separate dataset
			remoteTestSet3 = spiraSoapClient.TestSet_RetrieveById(testSetId);
			Assert.AreEqual((int)TestSet.TestSetStatusEnum.Blocked, remoteTestSet3.TestSetStatusId);

			//Now refresh the old dataset and try again and verify it works
			remoteTestSet2 = spiraSoapClient.TestSet_RetrieveById(testSetId);
			remoteTestSet2.TestSetStatusId = (int)TestSet.TestSetStatusEnum.Deferred;
			spiraSoapClient.TestSet_Update(remoteTestSet2);
			...
			RemoteTestCase remoteTestCase1 = spiraSoapClient.TestCase_RetrieveById(testCaseId);
			RemoteTestCase remoteTestCase2 = spiraSoapClient.TestCase_RetrieveById(testCaseId);

			//Now make a change to field and update
			remoteTestCase1.TestCasePriorityId = tc_priorityHighId;
			spiraSoapClient.TestCase_Update(remoteTestCase1);

			//Verify it updated correctly using separate dataset
			RemoteTestCase remoteTestCase3 = spiraSoapClient.TestCase_RetrieveById(testCaseId);
			...
			try
			{
				...
				spiraSoapClient.TestCase_Update(remoteTestCase2);
			}
			catch (FaultException<ServiceFaultMessage> soapException)
			{
				ServiceFaultMessage detail = soapException.Detail;
			...
			//Verify it didn't update using separate dataset
			remoteTestCase3 = spiraSoapClient.TestCase_RetrieveById(testCaseId);
			Assert.AreEqual(tc_priorityHighId, remoteTestCase3.TestCasePriorityId);

			//Now refresh the old dataset and try again and verify it works
			remoteTestCase2 = spiraSoapClient.TestCase_RetrieveById(testCaseId);
			remoteTestCase2.TestCasePriorityId = tc_priorityLowId;
			spiraSoapClient.TestCase_Update(remoteTestCase2);
			...
			remoteTestStep.Position = 1;
			spiraSoapClient.TestCase_AddStep(remoteTestStep, testCaseId);

			//Now retrieve the test step back and keep a copy of the dataset
			remoteTestCase1 = spiraSoapClient.TestCase_RetrieveById(testCaseId);
			remoteTestCase2 = spiraSoapClient.TestCase_RetrieveById(testCaseId);
		...
		public void _27_WorkflowTransition_Pulling()
		{
			...
			//Connect to the pre-existing project 2
			spiraSoapClient.Connection_Authenticate("administrator", "Welcome123$");
			spiraSoapClient.Connection_ConnectToProject(PROJECT_ID);

			//Get the incident.
			RemoteIncident incident = spiraSoapClient.Incident_RetrieveById(INCIDENT_ID);
			...
			//Get transitions.
			RemoteWorkflowTransition[] transitions = spiraSoapClient.Incident_RetrieveWorkflowTransitions(incident.IncidentTypeId.Value, incident.IncidentStatusId.Value, isDetector, isOwner);
			...
			//Task Transitions
			transitions = spiraSoapClient.Task_RetrieveWorkflowTransitions(1, 1, true, true);
			...
			transitions = spiraSoapClient.Requirement_RetrieveWorkflowTransitions(1, 1, true, true);
			...
			transitions = spiraSoapClient.TestCase_RetrieveWorkflowTransitions(1, 1, true, true);
			Assert.AreEqual(2, transitions.Length);
			Assert.AreEqual("Reject Test Case", transitions[0].Name);
			Assert.AreEqual("Review Test Case", transitions[1].Name);

			//Release Transitions
			transitions = spiraSoapClient.Release_RetrieveWorkflowTransitions(1, 1, true, true);
		...
		public void _28_WorkflowTransitionFields_Pulling()
		{
			...
			RemoteWorkflowField[] fields = spiraSoapClient.Incident_RetrieveWorkflowFields(incident.IncidentTypeId.Value, incident.IncidentStatusId.Value);
			...
			RemoteWorkflowCustomProperty[] customProperties = spiraSoapClient.Incident_RetrieveWorkflowCustomProperties(incident.IncidentTypeId.Value, incident.IncidentStatusId.Value);
			...
			//Task Fields
			fields = spiraSoapClient.Task_RetrieveWorkflowFields(1, 1);
			Assert.AreEqual(2, fields.Length);
			customProperties = spiraSoapClient.Task_RetrieveWorkflowCustomProperties(1, 1);
			Assert.AreEqual(0, customProperties.Length);

			//Requirement Fields
			fields = spiraSoapClient.Requirement_RetrieveWorkflowFields(1, 1);
			Assert.AreEqual(0, fields.Length);
			customProperties = spiraSoapClient.Task_RetrieveWorkflowCustomProperties(1, 1);
			Assert.AreEqual(0, customProperties.Length);

			//Test Case Fields
			fields = spiraSoapClient.TestCase_RetrieveWorkflowFields(1, 1);
			Assert.AreEqual(0, fields.Length);
			customProperties = spiraSoapClient.Task_RetrieveWorkflowCustomProperties(1, 1);
			Assert.AreEqual(0, customProperties.Length);

			//Release Fields
			fields = spiraSoapClient.Release_RetrieveWorkflowFields(1, 1);
			Assert.AreEqual(0, fields.Length);
			customProperties = spiraSoapClient.Task_RetrieveWorkflowCustomProperties(1, 1);
			Assert.AreEqual(0, customProperties.Length);
		}
		...
		public void _29_RetrieveDeleteAttachments()
		{
			...
			remoteSort.SortAscending = true;
			RemoteDocument[] remoteDocuments = spiraSoapClient.Document_RetrieveForFolder(projectAttachmentFolderId, null, remoteSort, 1, 999);
			...
			remoteSort.SortAscending = true;
			remoteDocuments = spiraSoapClient.Document_RetrieveForFolder(projectAttachmentFolderId, new RemoteFilter[] { remoteFilter }, remoteSort, 1, 999);
			...
			remoteDocuments = spiraSoapClient.Document_Retrieve(new RemoteFilter[] { remoteFilter }, remoteSort, 1, 999);
			...
			remoteSort.PropertyName = "UploadDate";
			remoteSort.SortAscending = false;
			remoteDocuments = spiraSoapClient.Document_RetrieveForArtifact((int)DataModel.Artifact.ArtifactTypeEnum.TestCase, testCaseId1, null, remoteSort);
			...
			remoteDocument = spiraSoapClient.Document_RetrieveById(remoteDocument.AttachmentId.Value);
			Assert.AreEqual(1, remoteDocument.AttachedArtifacts.Length);
			...
			remoteSort.SortAscending = true;
			remoteDocuments = spiraSoapClient.Document_RetrieveForArtifact((int)DataModel.Artifact.ArtifactTypeEnum.TestCase, testCaseId1, new RemoteFilter[] { remoteFilter }, remoteSort);
			...
			byte[] attachmentData = spiraSoapClient.Document_OpenFile(attachmentId1);
			UnicodeEncoding unicodeEncoding = new UnicodeEncoding();
			...
			spiraSoapClient.Document_DeleteFromArtifact(attachmentId2, (int)DataModel.Artifact.ArtifactTypeEnum.TestCase, testCaseId1);
		...
		public void _30_CreateRetrieveUpdateAssociations()
		{
			...
			spiraSoapClient.Association_Create(remoteAssociation);
			...
			remoteSort.SortAscending = true;
			RemoteAssociation[] remoteAssociations = spiraSoapClient.Association_RetrieveForArtifact((int)DataModel.Artifact.ArtifactTypeEnum.Incident, incidentId1, null, remoteSort);
			...
			remoteAssociations = spiraSoapClient.Association_RetrieveForArtifact((int)DataModel.Artifact.ArtifactTypeEnum.Incident, incidentId1, new RemoteFilter[] { remoteFilter }, remoteSort);
			...
			spiraSoapClient.Association_Update(remoteAssociation);
			remoteAssociations = spiraSoapClient.Association_RetrieveForArtifact((int)DataModel.Artifact.ArtifactTypeEnum.Incident, incidentId1, new RemoteFilter[] { remoteFilter }, remoteSort);
			Assert.AreEqual(1, remoteAssociations.Length);
			Assert.AreEqual(incidentId2, remoteAssociations[0].DestArtifactId);
			Assert.AreEqual("Incident", remoteAssociations[0].DestArtifactTypeName);
		...
		public void _31_AutomatedTestLauncher()
		{
			...
			spiraSoapClient.TestCase_AddUpdateAutomationScript(testCaseId1, AUTOMATION_ENGINE_ID_QTP, "file:///c:/mytests/test1.qtp", "", null, "1.0", null, null);
			spiraSoapClient.TestCase_AddUpdateAutomationScript(testCaseId2, AUTOMATION_ENGINE_ID_QTP, "test2.qtp", "", binaryData, "1.0", null, null);
			spiraSoapClient.TestCase_AddUpdateAutomationScript(testCaseId3, AUTOMATION_ENGINE_ID_QTP, "file:///c:/mytests/test3.qtp", "", null, "1.0", null, null);
			...
			int automationHostId = spiraSoapClient.AutomationHost_Create(remoteAutomationHost).AutomationHostId.Value;
			...
			spiraSoapClient.TestSet_Update(remoteTestSet);
			...
			RemoteAutomatedTestRun[] remoteAutomatedTestRuns = spiraSoapClient.TestRun_CreateForAutomationHost("Win8", dateRange);
			...
			Assert.AreEqual(1, remoteAutomatedTestRun.CustomProperties.Length);
			...
			RemoteDocument remoteTestScript = spiraSoapClient.Document_RetrieveById(automationAttachmentId);
			Assert.AreEqual("test2.qtp", remoteTestScript.FilenameOrUrl);
			binaryData = spiraSoapClient.Document_OpenFile(automationAttachmentId);
			...
			//Verify that testCaseId1 has the default parameter value specified (new to the 4.0 API)
			remoteAutomatedTestRun = remoteAutomatedTestRuns[0];
			Assert.AreEqual(1, remoteAutomatedTestRun.Parameters.Length);
			...
			remoteAutomatedTestRun = spiraSoapClient.TestRun_RecordAutomated1(remoteAutomatedTestRun);

			//Verify that the results can be retrieved
			remoteAutomatedTestRun = spiraSoapClient.TestRun_RetrieveAutomatedById(remoteAutomatedTestRun.TestRunId.Value);
			Assert.AreEqual(testCaseId2, remoteAutomatedTestRun.TestCaseId);
			Assert.AreEqual(testSetId1, remoteAutomatedTestRun.TestSetId);
			Assert.AreEqual(AUTOMATION_ENGINE_ID_QTP, remoteAutomatedTestRun.AutomationEngineId);
			Assert.AreEqual(automationHostId, remoteAutomatedTestRun.AutomationHostId);
			...
			remoteAutomatedTestRuns = spiraSoapClient.TestRun_CreateForAutomatedTestSet(testSetId1, "Win8");
			...
			//Delete the host
			spiraSoapClient.AutomationHost_Delete(automationHostId);

			//Verify it has been deleted
			Assert.IsNull(spiraSoapClient.AutomationHost_RetrieveById(automationHostId));
		}
		...
		public void _32_ImportRetrieveArtifactComments()
		{
			...
			spiraSoapClient.Requirement_CreateComment(remoteComment);
			//Verify the data
			RemoteComment[] remoteComments = spiraSoapClient.Requirement_RetrieveComments(requirementId1);
			...
			remoteComment.Text = "This is a comment";
			spiraSoapClient.Release_CreateComment(remoteComment);
			//Verify the data
			remoteComments = spiraSoapClient.Release_RetrieveComments(releaseId1);
			Assert.AreEqual(1, remoteComments.Length);
			Assert.AreEqual("This is a comment", remoteComments[0].Text);
			Assert.IsNotNull(remoteComments[0].CreationDate);
			...
			remoteComment.Text = "This is a comment";
			spiraSoapClient.Task_CreateComment(remoteComment);
			//Verify the data
			remoteComments = spiraSoapClient.Task_RetrieveComments(taskId1);
			Assert.AreEqual(1, remoteComments.Length);
			Assert.AreEqual("This is a comment", remoteComments[0].Text);
			Assert.IsNotNull(remoteComments[0].CreationDate);
			...
			remoteComment.Text = "This is a comment";
			spiraSoapClient.TestSet_CreateComment(remoteComment);
			//Verify the data
			remoteComments = spiraSoapClient.TestSet_RetrieveComments(testSetId1);
			Assert.AreEqual(1, remoteComments.Length);
			Assert.AreEqual("This is a comment", remoteComments[0].Text);
			Assert.IsNotNull(remoteComments[0].CreationDate);
			...
			remoteComment.Text = "This is a comment";
			spiraSoapClient.TestCase_CreateComment(remoteComment);
			//Verify the data
			remoteComments = spiraSoapClient.TestCase_RetrieveComments(testCaseId1);
			Assert.AreEqual(1, remoteComments.Length);
			Assert.AreEqual("This is a comment", remoteComments[0].Text);
			Assert.IsNotNull(remoteComments[0].CreationDate);
		}
		...
		public void _33_DataMappingChanges()
		{
			...
			int userId = spiraSoapClient.User_Create(remoteUser, "test123", "What is 3+4?", "7", 1).UserId.Value;

			//Verify that we have no mappings for this user
			RemoteDataMapping[] remoteUserMappings = spiraSoapClient.DataMapping_RetrieveUserMappings(1);
			...
			spiraSoapClient.DataMapping_AddUserMappings(1, remoteUserMappings);

			//Verify the data was added
			remoteUserMappings = spiraSoapClient.DataMapping_RetrieveUserMappings(1);
		...
		public void _34_RetrieveAutomationHostsAndEngines()
		{
			...
			spiraSoapClient.Connection_Authenticate("administrator", "Welcome123$");
			RemoteAutomationEngine[] remoteAutomationEngines = spiraSoapClient.AutomationEngine_Retrieve(true);
			...
			RemoteAutomationEngine remoteAutomationEngine = spiraSoapClient.AutomationEngine_RetrieveById(remoteAutomationEngines[1].AutomationEngineId.Value);
			...
			remoteAutomationEngine = spiraSoapClient.AutomationEngine_RetrieveByToken("QuickTestPro9");
			...
			remoteAutomationEngines = spiraSoapClient.AutomationEngine_Retrieve(false);
			...
			int automationEngineId = spiraSoapClient.AutomationEngine_Create(remoteAutomationEngine).AutomationEngineId.Value;

			//Verify
			remoteAutomationEngine = spiraSoapClient.AutomationEngine_RetrieveById(automationEngineId);
			...
			spiraSoapClient.AutomationEngine_Update(remoteAutomationEngine);
			...
			remoteSort.PropertyName = "Name";
			remoteSort.SortAscending = true;
			RemoteAutomationHost[] remoteAutomationHosts = spiraSoapClient.AutomationHost_Retrieve(null, remoteSort, 1, 9999);
			...
			remoteAutomationHost.Active = true;
			int automationHostId1 = spiraSoapClient.AutomationHost_Create(remoteAutomationHost).AutomationHostId.Value;
			...
			remoteAutomationHost.Active = true;
			int automationHostId2 = spiraSoapClient.AutomationHost_Create(remoteAutomationHost).AutomationHostId.Value;

			//Verify that we can retrieve them
			remoteAutomationHosts = spiraSoapClient.AutomationHost_Retrieve(null, remoteSort, 1, 9999);
			...
			remoteAutomationHosts = spiraSoapClient.AutomationHost_Retrieve(new RemoteFilter[] { remoteFilter }, remoteSort, 1, 9999);
			...
			remoteAutomationHost = spiraSoapClient.AutomationHost_RetrieveById(automationHostId1);
			...
			remoteAutomationHost = spiraSoapClient.AutomationHost_RetrieveByToken("Test02");
			Assert.AreEqual(9, remoteAutomationHost.ArtifactTypeId);
			...
			spiraSoapClient.AutomationHost_Update(remoteAutomationHost);
			...
			spiraSoapClient.AutomationHost_Delete(automationHostId2);
		...
		public void _35_RetrieveDocumentFoldersTypesAndVersions()
		{
			...
			spiraSoapClient.Connection_Authenticate("administrator", "Welcome123$");
			spiraSoapClient.Connection_ConnectToProject(PROJECT_ID);
			RemoteDocumentType[] remoteDocumentTypes = spiraSoapClient.Document_RetrieveTypes(true);
			...
			RemoteDocumentFolder[] remoteDocumentFolders = spiraSoapClient.Document_RetrieveFolders();
			...
			int folderId1 = spiraSoapClient.Document_AddFolder(remoteDocumentFolder).ProjectAttachmentFolderId.Value;
			...
			int folderId2 = spiraSoapClient.Document_AddFolder(remoteDocumentFolder).ProjectAttachmentFolderId.Value;
			remoteDocumentFolder = new RemoteDocumentFolder();
			remoteDocumentFolder.Name = "Documents";
			remoteDocumentFolder.ParentProjectAttachmentFolderId = folderId1;
			int folderId3 = spiraSoapClient.Document_AddFolder(remoteDocumentFolder).ProjectAttachmentFolderId.Value;

			//Verify the new folders were added
			remoteDocumentFolders = spiraSoapClient.Document_RetrieveFolders();
			...
			remoteDocumentFolder = spiraSoapClient.Document_RetrieveFolderById(folderId3);
			...
			remoteDocumentFolder.ParentProjectAttachmentFolderId = oldRootFolder;
			spiraSoapClient.Document_UpdateFolder(remoteDocumentFolder);
			...
			spiraSoapClient.Document_DeleteFolder(folderId3);
			...
			int attachmentId1 = spiraSoapClient.Document_AddFile(remoteDocument, attachmentData).AttachmentId.Value;
			...
			spiraSoapClient.Document_AddFileVersion(remoteDocumentVersion, attachmentData2, true);
			...
			remoteDocument.ProjectAttachmentFolderId = folderId2;
			remoteDocument.CurrentVersion = "1.0";
			int attachmentId2 = spiraSoapClient.Document_AddUrl(remoteDocument).AttachmentId.Value;
			...
			remoteDocumentVersion.VersionNumber = "2.0";
			remoteDocumentVersion.AuthorId = USER_ID_JOE_SMITH;
			spiraSoapClient.Document_AddUrlVersion(remoteDocumentVersion, true);

			//Verify
			remoteDocument = spiraSoapClient.Document_RetrieveById(attachmentId2);
			...
			spiraSoapClient.Document_Delete(attachmentId1);
			spiraSoapClient.Document_Delete(attachmentId2);

			//Verify that they were deleted
			Assert.IsNull(spiraSoapClient.Document_RetrieveById(attachmentId1));
			Assert.IsNull(spiraSoapClient.Document_RetrieveById(attachmentId2));
		}

		/// <summary>Tests that we can get project information back from known items..</summary>
		[Test, SpiraTestCase(1450)]
		...
		public void _36b_RequirementTestStepAssociations()
		{
			...
			RemoteRequirementTestStepMapping[] mappings = spiraSoapClient.Requirement_RetrieveTestStepCoverage(requirementId2);
			Assert.AreEqual(0, mappings.Length);
			mappings = spiraSoapClient.TestStep_RetrieveRequirementCoverage(testStepId1);
			Assert.AreEqual(0, mappings.Length);

			//Also make sure the requirement and test step exist OK
			RemoteRequirement requirement = spiraSoapClient.Requirement_RetrieveById(requirementId2);
			Assert.IsNotNull(requirement);
			RemoteTestStep testStep = spiraSoapClient.TestCase_RetrieveStepById(testStepId1);
			...
			spiraSoapClient.Requirement_AddTestStepCoverage(mapping);

			//Verify
			mappings = spiraSoapClient.Requirement_RetrieveTestStepCoverage(requirementId2);
			...
			mapping.RequirementId = requirementId2;
			mapping.TestStepId = testStepId1;
			spiraSoapClient.Requirement_RemoveTestStepCoverage(mapping);
		...
		public void _37_DeleteItems()
		{
			...
			spiraSoapClient.Requirement_Delete(requirementId1);
			Assert.IsNull(spiraSoapClient.Requirement_RetrieveById(requirementId1));

			//Delete a release and verify
			spiraSoapClient.Release_Delete(iterationId1);
			Assert.IsNull(spiraSoapClient.Release_RetrieveById(iterationId1));

			//Delete a test case and verify
			spiraSoapClient.TestCase_Delete(testCaseId1);
			Assert.IsNull(spiraSoapClient.TestCase_RetrieveById(testCaseId1));

			//Delete a test set and verify
			spiraSoapClient.TestSet_Delete(testSetId1);
			Assert.IsNull(spiraSoapClient.TestSet_RetrieveById(testSetId1));

			//Delete an incident and verify
			spiraSoapClient.Incident_Delete(incidentId1);
			Assert.IsNull(spiraSoapClient.Incident_RetrieveById(incidentId1));

			//Delete a task and verify
			spiraSoapClient.Task_Delete(taskId1);
			Assert.IsNull(spiraSoapClient.Task_RetrieveById(taskId1));
			...
			//Disable the user and verify (this code used to delete)
			spiraSoapClient.User_Delete(remoteUser.UserId.Value);
			Assert.IsFalse(spiraSoapClient.User_RetrieveById(remoteUser.UserId.Value).Active);

			//Now actually delete and verify
			new UserManager().DeleteUser(remoteUser.UserId.Value, true);
			Assert.IsNull(spiraSoapClient.User_RetrieveById(remoteUser.UserId.Value));
		...
		public void _38_BuildManagement()
		{
			...
			{
				BuildStatusId = 2,  //Succeeded
				ReleaseId = iterationId2,
				Name = "Build 0001",
				Description = "The first build",
				CreationDate = DateTime.UtcNow.AddDays(-1)
			};
			List<RemoteBuildSourceCode> revisions = new List<RemoteBuildSourceCode>
			{
				new RemoteBuildSourceCode { RevisionKey = "rev001" },
				new RemoteBuildSourceCode { RevisionKey = "rev002" },
				new RemoteBuildSourceCode { RevisionKey = "rev003" },
				new RemoteBuildSourceCode { RevisionKey = "rev004" },
				new RemoteBuildSourceCode { RevisionKey = "rev005" }
			};
			remoteBuild1.Revisions = revisions.ToArray();
			remoteBuild1 = spiraSoapClient.Build_Create(remoteBuild1);
			//Build 2
			...
			remoteBuild2 = spiraSoapClient.Build_Create(remoteBuild2);

			//Now test that we can retrieve a single build with its revisions
			RemoteBuild remoteBuild = spiraSoapClient.Build_RetrieveById(iterationId2, remoteBuild1.BuildId.Value);
			...
			RemoteBuild[] remoteBuilds = spiraSoapClient.Build_RetrieveByReleaseId(iterationId2, null, null, 1, 15);
			Assert.AreEqual(2, remoteBuilds.Length);
			RemoteBuild remoteBuildChk = remoteBuilds.SingleOrDefault(r => r.Name.Equals("Build 0001"));
			Assert.IsNotNull(remoteBuildChk);
			Assert.AreEqual(2, remoteBuildChk.BuildStatusId);
			Assert.AreEqual("Succeeded", remoteBuild.BuildStatusName);
			Assert.AreEqual("Build 0001", remoteBuildChk.Name);
			Assert.AreEqual("The first build", remoteBuildChk.Description);
			Assert.AreEqual(5, remoteBuildChk.Revisions.Length);
			Assert.AreEqual("rev001", remoteBuildChk.Revisions[0].RevisionKey);
			Assert.AreEqual("rev002", remoteBuildChk.Revisions[1].RevisionKey);
			Assert.AreEqual("rev003", remoteBuildChk.Revisions[2].RevisionKey);
			Assert.AreEqual("rev004", remoteBuildChk.Revisions[3].RevisionKey);
			Assert.AreEqual("rev005", remoteBuildChk.Revisions[4].RevisionKey);
			...
			List<RemoteFilter> remoteFilters = new List<RemoteFilter>
			{
				new RemoteFilter { PropertyName = "BuildStatusId", IntValue = 2 }
			};
			remoteBuilds = spiraSoapClient.Build_RetrieveByReleaseId(iterationId2, remoteFilters.ToArray(), remoteSort, 1, 15);
			...
			RemoteTestRun[] remoteTestRuns = spiraSoapClient.TestRun_Retrieve(remoteFilters.ToArray(), remoteSort, 1, 15);
			...
			RemoteIncident remoteIncident = new RemoteIncident
			{
				...
				Name = "Fixed Incident",
				ResolvedReleaseId = iterationId2,
				FixedBuildId = remoteBuild1.BuildId.Value,
				Description = "This is a test incident fixed in the build",
				CreationDate = DateTime.Parse("1/5/2005")
			};
			incidentId1 = spiraSoapClient.Incident_Create(remoteIncident).IncidentId.Value;
			...
			remoteSort.SortAscending = true;
			RemoteIncident[] remoteIncidents = spiraSoapClient.Incident_Retrieve(remoteFilters.ToArray(), remoteSort, 1, 15);
			Assert.AreEqual(1, remoteTestRuns.Length);
			Assert.AreEqual(remoteBuild1.BuildId, remoteTestRuns[0].BuildId.Value);
		}
		...
		public void _39_MoveArtifacts()
		{
			...
			RemoteRequirement remoteRequirement = new RemoteRequirement
			{
				...
				Name = "Requirement To Move",
				RequirementTypeId = rq_typeFeatureId
			};
			remoteRequirement = spiraSoapClient.Requirement_Create2(remoteRequirement, null);
			...
			RemoteRelease remoteRelease = new RemoteRelease
			{
				...
				ReleaseTypeId = (int)Release.ReleaseTypeEnum.MajorRelease,
			...
			RemoteTestCase remoteTestCase = new RemoteTestCase
			{
				Name = "Test Case To Move",
				TestCaseStatusId = (int)TestCase.TestCaseStatusEnum.ReadyForTest,
				TestCaseTypeId = tc_typeFunctionalId,
			...
			remoteTestStep = spiraSoapClient.TestCase_AddStep(remoteTestStep, testCaseId);
			...
			RemoteTestSet remoteTestSet = new RemoteTestSet
			{
				Name = "Test Set 1",
				CreatorId = userId1,
				TestSetStatusId = (int)TestSet.TestSetStatusEnum.NotStarted,
				TestRunTypeId = (int)TestRun.TestRunTypeEnum.Manual
			};
			int testSetId = spiraSoapClient.TestSet_Create(remoteTestSet).TestSetId.Value;

			//Verify requirement starting positions
			Assert.AreEqual("AAA", spiraSoapClient.Requirement_RetrieveById(requirementId1).IndentLevel);
			Assert.AreEqual("AAB", spiraSoapClient.Requirement_RetrieveById(requirementId2).IndentLevel);

			//Now move the requirement to the start of the list
			spiraSoapClient.Requirement_Move(requirementId2, requirementId1);
			//Verify
			Assert.AreEqual("AAA", spiraSoapClient.Requirement_RetrieveById(requirementId2).IndentLevel);
			//Now move back to the end of the list
			spiraSoapClient.Requirement_Move(requirementId2, null);
			//Verify
			Assert.AreEqual("AAB", spiraSoapClient.Requirement_RetrieveById(requirementId2).IndentLevel);

			//Now move the test case under a folder
			spiraSoapClient.TestCase_Move(testCaseId, testFolderId1);
			//Verify
			RemoteSort remoteSort = new RemoteSort() { PropertyName = "Name", SortAscending = true };
			Assert.IsTrue(spiraSoapClient.TestCase_RetrieveByFolder(testFolderId1, null, remoteSort, 1, 999999, null).Any(t => t.TestCaseId == testCaseId));
			//Now move back to the end of the root
			spiraSoapClient.TestCase_Move(testCaseId, null);
			//Verify
			Assert.IsFalse(spiraSoapClient.TestCase_RetrieveByFolder(testFolderId1, null, remoteSort, 1, 999999, null).Any(t => t.TestCaseId == testCaseId));

			//Now move the test set under a folder
			spiraSoapClient.TestSet_Move(testSetId, testSetFolderId1);
			//Verify
			RemoteTestSet[] testSets = spiraSoapClient.TestSet_RetrieveByFolder(testSetFolderId1, null, remoteSort, 1, 999999, null);
			Assert.AreEqual(2, testSets.Length);
			//Now move back to the end of the list
			spiraSoapClient.TestSet_Move(testSetId, null);
			//Verify
			testSets = spiraSoapClient.TestSet_RetrieveByFolder(testSetFolderId1, null, remoteSort, 1, 999999, null);
			Assert.AreEqual(1, testSets.Length);

			//Now move the release to the start of the list
			spiraSoapClient.Release_Move(releaseId, releaseId1);
			//Verify
			Assert.AreEqual("AAA", spiraSoapClient.Release_RetrieveById(releaseId).IndentLevel);
			//Now move back to the end of the list
			spiraSoapClient.Release_Move(releaseId, null);
			//Verify
			Assert.AreNotEqual("AAA", spiraSoapClient.Release_RetrieveById(releaseId).IndentLevel);

			//Now Try moving one of the test steps in the test case
			spiraSoapClient.TestCase_MoveStep(testCaseId, testStepId2, testStepId1);
			remoteTestCase = spiraSoapClient.TestCase_RetrieveById(testCaseId);
			...
			spiraSoapClient.TestCase_MoveStep(testCaseId, testStepId2, null);
			remoteTestCase = spiraSoapClient.TestCase_RetrieveById(testCaseId);
			...
			spiraSoapClient.TestCase_DeleteStep(testCaseId, testStepId1);
			remoteTestCase = spiraSoapClient.TestCase_RetrieveById(testCaseId);
		...
		public void _40_CheckCounts()
		{
			...
			long numInc = spiraSoapClient.Incident_Count(null);
			long numReq = spiraSoapClient.Requirement_Count(null);
			long numRel = spiraSoapClient.Release_Count(null);
			long numTsk = spiraSoapClient.Task_Count(null);
			long numTC = spiraSoapClient.TestCase_Count(null, null);
			long numTS = spiraSoapClient.TestSet_Count(null, null);
			long numTR = spiraSoapClient.TestRun_Count(null);

			Assert.GreaterOrEqual(numInc, 1);
			Assert.GreaterOrEqual(numReq, 1);
			Assert.GreaterOrEqual(numRel, 1);
			Assert.GreaterOrEqual(numTsk, 1);
			Assert.GreaterOrEqual(numTC, 1);
			Assert.GreaterOrEqual(numTS, 1);
			Assert.GreaterOrEqual(numTR, 1);
		}

		/// <summary>Tests the various functions in the DataSync API</summary>
		[Test, SpiraTestCase(1449)]
		public void _41_DataSyncApi()
		{
			//First authenticate the DataSync API
			bool success = spiraSoapClient.Connection_Authenticate2("administrator", "Welcome123$", "UnitTest");
			Assert.IsTrue(success);

			//Get the list of data-sync systems configured
			RemoteDataSyncSystem[] dataSyncSystems = spiraSoapClient.DataSyncSystem_Retrieve();
			...
			success = spiraSoapClient.Connection_Authenticate2("administrator", "Welcome123$", "UnitTest");
			Assert.IsTrue(success);
			success = spiraSoapClient.Connection_ConnectToProject(1);
			Assert.IsTrue(success);
			...
			//Incident Severity
			RemoteDataMapping[] fieldMappings = spiraSoapClient.DataMapping_RetrieveFieldValueMappings(dataSyncSystem1, 1);
			//Incident Priority
			fieldMappings = spiraSoapClient.DataMapping_RetrieveFieldValueMappings(dataSyncSystem1, 2);
			//Incident Status
			fieldMappings = spiraSoapClient.DataMapping_RetrieveFieldValueMappings(dataSyncSystem1, 3);
			//Incident Type
			fieldMappings = spiraSoapClient.DataMapping_RetrieveFieldValueMappings(dataSyncSystem1, 4);
			//Requirement Importance
			fieldMappings = spiraSoapClient.DataMapping_RetrieveFieldValueMappings(dataSyncSystem1, 18);
			//Requirement Status
			fieldMappings = spiraSoapClient.DataMapping_RetrieveFieldValueMappings(dataSyncSystem1, 16);
			//Requirement Type
			fieldMappings = spiraSoapClient.DataMapping_RetrieveFieldValueMappings(dataSyncSystem1, 140);
			//Component
			fieldMappings = spiraSoapClient.DataMapping_RetrieveFieldValueMappings(dataSyncSystem1, 141);
			//Task Priority
			fieldMappings = spiraSoapClient.DataMapping_RetrieveFieldValueMappings(dataSyncSystem1, 59);
			//Task Status
			fieldMappings = spiraSoapClient.DataMapping_RetrieveFieldValueMappings(dataSyncSystem1, 57);
			//Task Type
			fieldMappings = spiraSoapClient.DataMapping_RetrieveFieldValueMappings(dataSyncSystem1, 145);

			//Verify that we can retrieve custom property mapping
			RemoteDataMapping customPropertyMapping = spiraSoapClient.DataMapping_RetrieveCustomPropertyMapping(dataSyncSystem1, 1, 1);

			//Verify that we can retrieve custom property mapping
			RemoteDataMapping[] customPropertyValueMappings = spiraSoapClient.DataMapping_RetrieveCustomPropertyValueMappings(dataSyncSystem1, 1, 1);
			...
			dataSyncSystems = spiraSoapClient.DataSyncSystem_Retrieve();
		...
		public void _42_SourceCodeQueries()
		{
			try
			{
				...
				RemoteSourceCodeBranch[] branches = spiraSoapClient.SourceCode_RetrieveBranches();
				...
				//Get the root folder
				RemoteSourceCodeFolder[] folders = spiraSoapClient.SourceCode_RetrieveFoldersByParent(branchId, null);
                ...
                folders = spiraSoapClient.SourceCode_RetrieveFoldersByParent(branchId, "test://MyRepository");
				...
				RemoteSourceCodeFile[] files = spiraSoapClient.SourceCode_RetrieveFilesByFolder(branchId, folderId);
				...
				RemoteSourceCodeFile file = spiraSoapClient.SourceCode_RetrieveFileById(branchId, fileId);
				...
				RemoteSourceCodeRevision[] revisions = spiraSoapClient.SourceCode_RetrieveRevisionsForFile(branchId, fileId);
				Assert.IsTrue(revisions.Length >= 0);
				//Assert.IsNotNullOrEmpty(revisions[0].Id); Commented out here for no data
			//	Assert.IsNotNullOrEmpty(revisions[0].Name);
				...
				//remoteSort.PropertyName = "Name";
				//remoteSort.SortAscending = true;
				//revisions = spiraSoapClient.SourceCode_RetrieveRevisions(branchId, 1, 15, remoteSort, null);
				//Assert.IsTrue(revisions.Length > 0);
				...
				//RemoteSourceCodeRevision revision = spiraSoapClient.SourceCode_RetrieveRevisionById(branchId, revisionId);
				...
				//files = spiraSoapClient.SourceCode_RetrieveFilesByRevision(branchId, revisionId);
				//Assert.IsTrue(files.Length > 0);
				//Assert.IsNotNullOrEmpty(files[0].Id);
				//Assert.IsNotNullOrEmpty(files[0].Name);
				//Assert.IsNotNullOrEmpty(files[0].Path);
				...
				//byte[] data = spiraSoapClient.SourceCode_OpenFileById(branchId, fileId, null);
				//Assert.IsTrue(data != null && data.Length > 0);

				////Verify that we can open a specific revision of a file
				//data = spiraSoapClient.SourceCode_OpenFileById(branchId, fileId, revisionId);
				//Assert.IsTrue(data != null && data.Length > 0);

				////Verify that we can get the revisions associated with an artifact
				//revisions = spiraSoapClient.SourceCode_RetrieveRevisionsForArtifact(branchId, /*Incident*/3, 7);
				//Assert.IsTrue(revisions.Length > 0);

				////Verify that we can get the artifacts for this revision
				//RemoteLinkedArtifact[] artifacts = spiraSoapClient.SourceCode_RetrieveArtifactsForRevision(branchId, revisions[0].Id);
				//Assert.IsTrue(artifacts.Length > 0);

				////verify that we can get the files associated with an artifact
				//files = spiraSoapClient.SourceCode_RetrieveFilesForArtifact(branchId, /*Incident*/3, 7);
				//Assert.IsTrue(revisions.Length > 0);
			}
		...
		public void _43_InstantMessenger()
		{
			...
			remoteUser = spiraSoapClient.User_Create(remoteUser, "PleaseChange", "What is 1+1?", "2", 1);
			...
			spiraSoapClient.Connection_Disconnect();
			bool authenticated = spiraSoapClient.Connection_Authenticate3("imuser1", "{C9C569C6-2BBF-47C2-B498-1238F8A7A0AF}", "NUnit");
			Assert.IsTrue(authenticated);

			//Verify that we have no messages waiting and that our user is online
			RemoteMessageInfo messageInfo = spiraSoapClient.Message_GetInfo();
			Assert.AreEqual(0, messageInfo.UnreadMessages);
			Assert.IsTrue(messageInfo.OnlineUsers.Length > 0);
			Assert.IsTrue(messageInfo.OnlineUsers.Any(u => u == imUser1));

			//Now connect as the other user and send a message to the first user
			authenticated = spiraSoapClient.Connection_Authenticate3("imuser2", "{78197F7D-2166-49B0-BE7D-AD13FEBB4858}", "NUnit");
			Assert.IsTrue(authenticated);
			messageInfo = spiraSoapClient.Message_GetInfo();
			Assert.AreEqual(0, messageInfo.UnreadMessages);
			Assert.IsTrue(messageInfo.OnlineUsers.Length > 0);
			Assert.IsTrue(messageInfo.OnlineUsers.Any(u => u == imUser2));
			spiraSoapClient.Message_PostNew(imUser1, "This is a message 1 from user 2");

			//Verify that the first user sees the message
			authenticated = spiraSoapClient.Connection_Authenticate3("imuser1", "{C9C569C6-2BBF-47C2-B498-1238F8A7A0AF}", "NUnit");
			Assert.IsTrue(authenticated);
			messageInfo = spiraSoapClient.Message_GetInfo();
			...
			RemoteUserMessage[] messageUsers = spiraSoapClient.Message_GetUnreadMessageSenders();
			...
			RemoteMessage[] messages = spiraSoapClient.Message_RetrieveUnread();
			...
			//Mark it as read
			spiraSoapClient.Message_MarkAllAsRead(imUser2);

			//Verify no more unread messages from this user and in general
			messageInfo = spiraSoapClient.Message_GetInfo();
			Assert.AreEqual(0, messageInfo.UnreadMessages);
			messageUsers = spiraSoapClient.Message_GetUnreadMessageSenders();
			...
			RemoteUser[] contacts = spiraSoapClient.User_RetrieveContacts();
			Assert.AreEqual(0, contacts.Length);

			//Add a contact to the second user
			spiraSoapClient.User_AddContact(imUser2);

			//Verify the contact is added
			contacts = spiraSoapClient.User_RetrieveContacts();
			Assert.AreEqual(1, contacts.Length);
			Assert.AreEqual("imuser2", contacts[0].UserName);

			//Remove the contact
			spiraSoapClient.User_RemoveContact(imUser2);
		...
		public void _44_RetrieveArtifactHistory()
		{
			...
			RemoteSort remoteSort = new RemoteSort();
			remoteSort.PropertyName = "ChangeDate";
			remoteSort.SortAscending = false;
			RemoteHistoryChange[] changes = spiraSoapClient.History_RetrieveForArtifact(/*Requirement*/1, 4, 1, Int32.MaxValue, remoteSort, null);
			Assert.AreEqual(0, changes.Length);
			//Assert.AreEqual(12, changes[0].ChangeSetId); commentted out for no data
			...
			changes = spiraSoapClient.History_RetrieveForArtifact(/*Requirement*/1, 4, 1, Int32.MaxValue, remoteSort, new RemoteFilter[] { remoteFilter });
			Assert.AreEqual(0, changes.Length);
			//Assert.AreEqual(12, changes[0].ChangeSetId); commentted out for no data
			//Assert.AreEqual("Status", changes[0].FieldCaption);
			//Assert.AreEqual("In Progress", changes[0].OldValue);
			...
			RemoteHistoryChangeSet changeSet = spiraSoapClient.History_RetrieveById(12);
			...
			//Assert.AreEqual("Developed", changeSet.Changes[0].NewValue); commentted out for no data
		...
		public void _45_SubscriptionsAndSavedSearches()
		{
			...
			RemoteSavedFilter[] savedFilters = spiraSoapClient.SavedFilter_RetrieveForUser();
			...
			Assert.AreEqual(2, savedFilter.Filters.Length);
			...
			spiraSoapClient.Connection_Authenticate("administrator", "Welcome123$");
			spiraSoapClient.Connection_ConnectToProject(1);
			RemoteArtifactSubscription[] subscriptions = spiraSoapClient.Subscription_RetrieveForUser();
			Assert.AreEqual(0, subscriptions.Length);

			//Subscribe to a requirement and verify
			spiraSoapClient.Subscription_SubscribeToArtifact(1, 4);
			subscriptions = spiraSoapClient.Subscription_RetrieveForUser();
			...
			subscriptions = spiraSoapClient.Subscription_RetrieveForArtifact(1, 4);
			Assert.IsTrue(subscriptions.Length > 0);
			Assert.IsTrue(subscriptions.Any(s => s.UserId == 1));

			//Unsubscribe
			spiraSoapClient.Subscription_UnsubscribeFromArtifact(1, 4);
		...
		public void _46_RetrieveTestConfigurations()
		{
			...
			RemoteTestConfigurationSet[] testConfigurationSets = spiraSoapClient.TestConfiguration_RetrieveSets();
			...
			RemoteTestConfigurationSet testConfigurationSet = spiraSoapClient.TestConfiguration_RetrieveForTestSet(1);
			Assert.IsNull(testConfigurationSet);

			//Retrieve a test configuration set by its id and verify the entries as well
			testConfigurationSet = spiraSoapClient.TestConfiguration_RetrieveSetById(1);
			...
			//The entries
			Assert.AreEqual(6, testConfigurationSet.Entries.Length);
			Assert.AreEqual(2, testConfigurationSet.Entries[0].ParameterValues.Length);
		...
		[Test]
		[SpiraTestCase(1857)]
		public void _47_CheckCompatibilityWithOldStaticIDs()
		{
			...
			spiraSoapClient.Connection_ConnectToProject(projectId3);
			...
			remoteRequirement.RequirementTypeId = 4; // User Story
			remoteRequirement.Name = "Test Requirement";
			remoteRequirement = spiraSoapClient.Requirement_Create1(remoteRequirement, 0);
			int requirementId = remoteRequirement.RequirementId.Value;

			//Verify the data
			remoteRequirement = spiraSoapClient.Requirement_RetrieveById(requirementId);
			Assert.AreNotEqual(1, remoteRequirement.ImportanceId);
			Assert.AreNotEqual(4, remoteRequirement.RequirementTypeId);
			...
			remoteRequirement.RequirementTypeId = 4;
			spiraSoapClient.Requirement_Update(remoteRequirement);
			...
			remoteTask.TaskTypeId = 2;  //Testing
			int taskId = spiraSoapClient.Task_Create(remoteTask).TaskId.Value;

			//Verify the data
			remoteTask = spiraSoapClient.Task_RetrieveById(taskId);
			Assert.AreNotEqual(3, remoteTask.TaskPriorityId);
			Assert.AreNotEqual(2, remoteTask.TaskTypeId);

			//Now try and update it back and verify it remains with the correct ID
			remoteTask.TaskPriorityId = 3;
			remoteTask.TaskTypeId = 4;
			spiraSoapClient.Task_Update(remoteTask);
			...
			remoteTestCase.TestCaseStatusId = (int)TestCase.TestCaseStatusEnum.ReadyForReview;
			remoteTestCase.TestCasePriorityId = 3;  //Medium
			remoteTestCase.TestCaseTypeId = 4;  //Integration
			int testCaseId = spiraSoapClient.TestCase_Create(remoteTestCase).TestCaseId.Value;

			//Verify the data
			remoteTestCase = spiraSoapClient.TestCase_RetrieveById(testCaseId);
			Assert.AreNotEqual(3, remoteTestCase.TestCasePriorityId);
			Assert.AreNotEqual(4, remoteTestCase.TestCaseTypeId);

			//Now try and update it back and verify it remains with the correct ID
			remoteTestCase.TestCasePriorityId = 3;
			remoteTestCase.TestCaseTypeId = 4;
			spiraSoapClient.TestCase_Update(remoteTestCase);
