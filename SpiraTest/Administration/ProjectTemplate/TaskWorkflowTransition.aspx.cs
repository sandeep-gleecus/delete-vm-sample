...
namespace Inflectra.SpiraTest.Web.Administration.ProjectTemplate
{
	...
	/// </summary>
    [
    HeaderSettings(GlobalNavigation.NavigationHighlightedLink.Administration, "Admin_WorkflowTransition_Title", "Template-Tasks/#edit-workflow-transition", "Admin_WorkflowTransition_Title"),
    ...
    public partial class TaskWorkflowTransition : AdministrationBase
	{
		//Datasets and entities
        DataModel.TaskWorkflowTransition workflowTransition;
        protected SortedList<string, string> flagList;

		protected int workflowId;
		protected int workflowTransitionId;

        private const string CLASS_NAME = "Inflectra.SpiraTest.Web.Administration.ProjectTemplate.TaskWorkflowTransition::";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack) 
			{
                ...
                this.lnkIncomingStep.Text = workflowTransition.InputTaskStatus.Name + " (" + workflowTransition.InputTaskStatusId.ToString() + ")"; ;
                this.lnkIncomingStep.NavigateUrl = UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "TaskWorkflowStep") + "?" + GlobalFunctions.PARAMETER_WORKFLOW_ID + "=" + workflowId.ToString() + "&" + GlobalFunctions.PARAMETER_TASK_STATUS_ID + "=" + workflowTransition.InputTaskStatusId.ToString();
                this.lnkOutgoingStep.Text = workflowTransition.OutputTaskStatus.Name + " (" + workflowTransition.OutputTaskStatusId.ToString() + ")"; ;
                this.lnkOutgoingStep.NavigateUrl = UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "TaskWorkflowStep") + "?" + GlobalFunctions.PARAMETER_WORKFLOW_ID + "=" + workflowId.ToString() + "&" + GlobalFunctions.PARAMETER_TASK_STATUS_ID + "=" + workflowTransition.OutputTaskStatusId.ToString();
        ...
        {
            Response.Redirect(UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "TaskWorkflowDetails") + "?" + GlobalFunctions.PARAMETER_WORKFLOW_ID + "=" + workflowId);
        }
		...
		private void btnUpdate_Click(object sender, EventArgs e)
		{
            ...
            foreach (GridViewRow gvr in this.grdExecuteRoles.Rows)
            {
                ...
                if (gvr.RowType == DataControlRowType.DataRow)
                {
                    ...
                    TaskWorkflowTransitionRole workflowTransitionRole = this.workflowTransition.TransitionRoles.FirstOrDefault(r => r.ProjectRoleId == projectRoleId && r.WorkflowTransitionRoleTypeId == (int)WorkflowTransitionRoleType.WorkflowTransitionRoleTypeEnum.Execute);
