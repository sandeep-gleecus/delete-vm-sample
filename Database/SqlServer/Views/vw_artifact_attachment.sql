...
GO
CREATE VIEW [VW_ARTIFACT_ATTACHMENT]
AS
...
UNION
SELECT	(AAT.ARTIFACT_ID * 10 + AAT.ARTIFACT_TYPE_ID) AS ARTIFACT_LINK_ID, AAT.ARTIFACT_TYPE_ID, AAT.ARTIFACT_ID, ART.NAME AS ARTIFACT_TYPE_NAME, OBJ.NAME AS ARTIFACT_NAME, '' AS COMMENT, OBJ.CREATOR_ID, OBJ.CREATION_DATE, (RTRIM(USR.FIRST_NAME + ' ' + ISNULL(USR.MIDDLE_INITIAL,'')) + ' ' + USR.LAST_NAME) AS CREATOR_NAME,
		AAT.ATTACHMENT_ID, AAT.PROJECT_ID, STA.NAME AS ARTIFACT_STATUS_NAME
FROM	TST_ARTIFACT_ATTACHMENT AAT INNER JOIN TST_ARTIFACT_TYPE ART
ON		AAT.ARTIFACT_TYPE_ID = ART.ARTIFACT_TYPE_ID INNER JOIN TST_RISK OBJ 
ON		AAT.ARTIFACT_ID = OBJ.RISK_ID INNER JOIN TST_USER_PROFILE USR 
ON		OBJ.CREATOR_ID = USR.USER_ID INNER JOIN TST_RISK_STATUS STA 
ON		OBJ.RISK_STATUS_ID = STA.RISK_STATUS_ID 
