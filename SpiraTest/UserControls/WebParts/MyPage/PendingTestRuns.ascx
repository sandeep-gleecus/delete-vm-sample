<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PendingTestRuns.ascx.cs" Inherits="Inflectra.SpiraTest.Web.UserControls.WebParts.MyPage.PendingTestRuns" %>
...
<tstsc:GridViewEx EnableViewState="false" id="grdSavedTestRuns" Runat="server" DataMember="TestRunsPending" SkinID="WidgetGrid">
	...
	<Columns>
		<tstsc:TemplateFieldEx ItemStyle-CssClass="priority1" HeaderStyle-CssClass="priority1">
			...
			<ItemTemplate>
			    <tstsc:ImageEx CssClass="w4 h4 priority1" ImageUrl="Images/artifact-TestRun.svg" AlternateText="Test Run" ID="imgIcon" runat="server" />
                <tstsc:LabelEx Runat="server" Tooltip='<%# ((TestRunsPendingView) Container.DataItem).Name %>' Text='<%#: GlobalFunctions.TruncateName(((TestRunsPendingView) Container.DataItem).Name, 50) %>' ID="lblName" />
                <asp:Label ID="lblTestCount" Runat="server" Text='<%# ((TestRunsPendingView) Container.DataItem).CountNotRun %>' CssClass="badge"/>
			</ItemTemplate>
		</tstsc:TemplateFieldEx>
		<tstsc:BoundFieldEx DataField="ProjectName" HeaderText="<%$Resources:Fields,Project %>" MaxLength="20" HtmlEncode="true"  HeaderStyle-CssClass="priority2" ItemStyle-CssClass="priority2"/>
		<tstsc:TemplateFieldEx ItemStyle-Wrap="False" HeaderStyle-Wrap="False"  HeaderStyle-CssClass="priority3" ItemStyle-CssClass="priority3">
			...
			<ItemTemplate>
				<tstsc:LabelEx Runat="server" Tooltip='<%# String.Format(GlobalFunctions.FORMAT_DATE_TIME,  GlobalFunctions.LocalizeDate(((TestRunsPendingView) Container.DataItem).LastUpdateDate)) %>' Text='<%# String.Format(GlobalFunctions.FORMAT_DATE,  GlobalFunctions.LocalizeDate(((TestRunsPendingView) Container.DataItem).LastUpdateDate)) %>' ID="Label14" />
			</ItemTemplate>
		</tstsc:TemplateFieldEx>
        ...
        <tstsc:TemplateFieldEx runat="server" HeaderStyle-CssClass="priority1" ItemStyle-CssClass="priority1" ItemStyle-Wrap="false">
            <ItemTemplate>
				<div class="btn-group">
				    <tstsc:LinkButtonEx Runat="server" CommandName="DeletePending" CommandArgument='<%# ((TestRunsPendingView) Container.DataItem).TestRunsPendingId%>' Tooltip='<%# Resources.Buttons.Delete + " " + GlobalFunctions.DISPLAY_SINGLE_QUOTE +((TestRunsPendingView) Container.DataItem).Name + GlobalFunctions.DISPLAY_SINGLE_QUOTE %>' ID="btnDelete" CausesValidation="false" Confirmation="true" ConfirmationMessage="<%$Resources:Messages,PendingTestRuns_DeleteConfirm %>">
                        <span class="fas fa-trash-alt"></span>
				    </tstsc:LinkButtonEx>
                    <tstsc:HyperLinkEx runat="server" Authorized_ArtifactType="TestRun" Authorized_Permission="Create" NavigateUrl="javascript:void(0)" ClientScriptMethod='<%# "grdSavedTestRuns_showReassignDialog(" + ((TestRunsPendingView) Container.DataItem).ProjectId + "," + ((TestRunsPendingView) Container.DataItem).TestRunsPendingId + ")"%>' ToolTip="<%$Resources:Main,PendingTestRuns_Reassign%>"  CssClass="btn btn-default">
                        <span class="fas fa-users"></span>
                    </tstsc:HyperLinkEx>
                    <tstsc:HyperLinkEx Runat="server" NavigateUrl='<%# ((TestRunsPendingView) Container.DataItem).TestRunsPendingId%>' Tooltip='<%# Resources.Buttons.Resume + " " + GlobalFunctions.DISPLAY_SINGLE_QUOTE +((TestRunsPendingView) Container.DataItem).Name + GlobalFunctions.DISPLAY_SINGLE_QUOTE %>' ID="btnResume" CssClass="btn btn-primary">
