...
namespace Inflectra.SpiraTest.Web.Administration.ProjectTemplate
{
	...
	/// </summary>
    [
    HeaderSettings(GlobalNavigation.NavigationHighlightedLink.Administration, "Admin_WorkflowTransition_Title", "Template-Requirements/#edit-workflow-transition", "Admin_WorkflowTransition_Title"),
    ...
    public partial class RequirementWorkflowTransition : AdministrationBase
	{
		//Datasets and entities
        DataModel.RequirementWorkflowTransition workflowTransition;
        protected SortedList<string, string> flagList;

		protected int workflowId;
		protected int workflowTransitionId;

        private const string CLASS_NAME = "Inflectra.SpiraTest.Web.Administration.ProjectTemplate.RequirementWorkflowTransition::";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack) 
			{
                ...
                this.lnkIncomingStep.Text = workflowTransition.InputRequirementStatus.Name + " (" + workflowTransition.InputRequirementStatusId.ToString() + ")"; ;
                this.lnkIncomingStep.NavigateUrl= UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "RequirementWorkflowStep") + "?" + GlobalFunctions.PARAMETER_WORKFLOW_ID + "=" + workflowId.ToString() + "&" + GlobalFunctions.PARAMETER_REQUIREMENT_STATUS_ID + "=" + workflowTransition.InputRequirementStatusId.ToString();
                this.lnkOutgoingStep.Text = workflowTransition.OutputRequirementStatus.Name + " (" + workflowTransition.OutputRequirementStatusId.ToString() + ")"; ;
                this.lnkOutgoingStep.NavigateUrl= UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "RequirementWorkflowStep") + "?" + GlobalFunctions.PARAMETER_WORKFLOW_ID + "=" + workflowId.ToString() + "&" + GlobalFunctions.PARAMETER_REQUIREMENT_STATUS_ID + "=" + workflowTransition.OutputRequirementStatusId.ToString();
        ...
        {
            Response.Redirect(UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "RequirementWorkflowDetails") + "?" + GlobalFunctions.PARAMETER_WORKFLOW_ID + "=" + workflowId);
        }
		...
		private void btnUpdate_Click(object sender, EventArgs e)
		{
            ...
            foreach (GridViewRow gvr in this.grdExecuteRoles.Rows)
            {
                ...
                if (gvr.RowType == DataControlRowType.DataRow)
                {
                    ...
                    RequirementWorkflowTransitionRole workflowTransitionRole = this.workflowTransition.TransitionRoles.FirstOrDefault(r => r.ProjectRoleId == projectRoleId && r.WorkflowTransitionRoleTypeId == (int)WorkflowTransitionRoleType.WorkflowTransitionRoleTypeEnum.Execute);
