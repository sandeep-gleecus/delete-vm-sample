...
namespace Inflectra.SpiraTest.Web.ServerControls
{
    ...
    /// </summary>
    [
    ToolboxData("<{0}:IntRangeFilter runat=server></{0}:IntRangeFilter>"),
    DefaultProperty("Value"),
    ValidationProperty("Value")
    ]
    public class IntRangeFilter : WebControl, IScriptControl, IPostBackDataHandler
    {
        ...
        public bool LoadPostData(string postDataKey, NameValueCollection postColl)
        {
            ...
            if (postColl != null && !String.IsNullOrEmpty(postDataKey) && postColl[postDataKey] != null)
            {
                ...
                if (oldValue != postedValue)
                {
                    ...
                    try
                    {
                        IntRange decimalRange;
                        if (IntRange.TryParse(postedValue, out decimalRange))
                        {
