<%@ Page Language="c#" CodeBehind="TestCaseWorkflowDetails.aspx.cs" AutoEventWireup="True"
    Inherits="Inflectra.SpiraTest.Web.Administration.ProjectTemplate.TestCaseWorkflowDetails" MasterPageFile="~/MasterPages/Administration.master" %>
...
<asp:Content ContentPlaceHolderID="cplAdministrationContent" runat="server" ID="Content2">
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-10">
                ...
                <p>
                    <asp:Localize runat="server" Text="<%$Resources:Main,Admin_TestCaseWorkflowDetails_Intro %>" />
                </p>
                <tstsc:GridViewEx ID="grdWorkflowSteps" runat="server" AutoGenerateColumns="False" Width="100%" CssClass="DataGrid WorkflowDiagram workflow-list" ShowFooter="False" ShowHeader="True">
                    ...
                    <Columns>
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,workflow_step_name %>" HeaderStyle-CssClass="priority1" ItemStyle-CssClass="priority1 v-mid">
                            <ItemTemplate>
                                <div class="Step display-inline-block ml4" data-workflowStep="<%# ((Inflectra.SpiraTest.DataModel.TestCaseStatus) Container.DataItem).Name%>">
                                    <tstsc:HyperLinkEx CssClass="large" runat="server" NavigateUrl='<%# UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "TestCaseWorkflowStep") + "?" + GlobalFunctions.PARAMETER_WORKFLOW_ID + "=" + workflowId.ToString() + "&" + GlobalFunctions.PARAMETER_TEST_CASE_STATUS_ID + "=" + ((Inflectra.SpiraTest.DataModel.TestCaseStatus) Container.DataItem).TestCaseStatusId%>'><%# ((Inflectra.SpiraTest.DataModel.TestCaseStatus) Container.DataItem).Name%></tstsc:HyperLinkEx>
                                    <asp:Label runat="server" Visible='<%# ((Inflectra.SpiraTest.DataModel.TestCaseStatus) Container.DataItem).TestCaseStatusId == (int)TestCase.TestCaseStatusEnum.Draft%>'
                                         ID="Label1" CssClass="badge badge-inline-text"><small><asp:Localize runat="server" Text="<%$Resources:Main,Admin_WorkflowDetails_Default %>"/></small></asp:Label>
                                </div>
                        ...
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,workflow_transitions %>" HeaderStyle-CssClass="priority1" ItemStyle-CssClass="priority1">
                            <ItemTemplate>
                                ...
                                <asp:Repeater runat="server" OnItemCommand="rptWorkflowStepTransitions_ItemCommand" ID="rptWorkflowStepTransitions">
                                    <ItemTemplate>
                                        <div class="btn-group">
                                            <tstsc:HyperLinkEx SkinID="ButtonDefault" Font-Italic="true" runat="server" NavigateUrl='<%# UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "TestCaseWorkflowTransition") + "?" + GlobalFunctions.PARAMETER_WORKFLOW_ID + "=" + workflowId.ToString() + "&" + GlobalFunctions.PARAMETER_WORKFLOW_TRANSITION_ID + "=" + ((Inflectra.SpiraTest.DataModel.TestCaseWorkflowTransition) Container.DataItem).WorkflowTransitionId%>'><%#: ((Inflectra.SpiraTest.DataModel.TestCaseWorkflowTransition) Container.DataItem).Name%></tstsc:HyperLinkEx>
                                            <tstsc:LinkButtonEx runat="server" CommandName="Delete" CommandArgument='<%# ((Inflectra.SpiraTest.DataModel.TestCaseWorkflowTransition) Container.DataItem).WorkflowTransitionId%>' CausesValidation="false"
                                                ID="Linkbutton2" Tooltip="<%$Resources:Main,Admin_WorkflowDetails_Delete %>">
                                                <span class="fas fa-trash-alt"></span>
                                            </tstsc:LinkButtonEx>
                                        </div>
                                        <span class="fas fa-arrow-right mx4 "></span>
                                        <div class="Step display-inline-block muted" data-workflowStep="<%# ((Inflectra.SpiraTest.DataModel.TestCaseWorkflowTransition)Container.DataItem).OutputTestCaseStatus.Name%>">
                                            <asp:Label runat="server">
                                                <%# ((Inflectra.SpiraTest.DataModel.TestCaseWorkflowTransition)Container.DataItem).OutputTestCaseStatus.Name%>
                                            </asp:Label>
                                        </div>
                                        <div class="Spacer">
                                        </div>
                        ...
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Fields,workflow_status_id %>" HeaderStyle-CssClass="priority4" ItemStyle-CssClass="priority4">
                            <ItemTemplate>
                                <%# String.Format(GlobalFunctions.FORMAT_ID, ((Inflectra.SpiraTest.DataModel.TestCaseStatus)Container.DataItem).TestCaseStatusId)%>
                            </ItemTemplate>
                        </tstsc:TemplateFieldEx>
                        <tstsc:TemplateFieldEx HeaderText="<%$Resources:Main,Global_Operations %>" HeaderStyle-CssClass="priority1" ItemStyle-CssClass="priority1">
                            <ItemTemplate>
                                <tstsc:HyperLinkEx runat="server" NavigateUrl="javascript:void(0)" ClientScriptMethod='<%# "pnlAddTransition_display(event," + ((Inflectra.SpiraTest.DataModel.TestCaseStatus) Container.DataItem).TestCaseStatusId + ")"%>'
            ...
            <tr>
                ...
                <td>
                    <tstsc:DropDownListEx ID="ddlDestinationStep" runat="server" CssClass="DropDownList"
                        DataTextField="Name" DataValueField="TestCaseStatusId" Width="250px" />
                </td>
            </tr>
            <tr>
                <td>
    ...
    <script type="text/javascript">
        ...
        var incidentStatuses = <%=GetTestCaseStatusesAsJson() %>;
