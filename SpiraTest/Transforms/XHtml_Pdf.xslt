...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="common-atts">
    ...
    <xsl:if test="@width and @width != ''">
      <xsl:attribute name="width">
        <xsl:if test="contains(@width, 'px') or contains(@width, 'pt') or contains(@width, '%') or contains(@width, 'pc') or contains(@width, 'em') or contains(@width, 'in') or contains(@width, 'cm') or contains(@width, 'mm')">
          <xsl:value-of select="@width"/>
        </xsl:if>
        <xsl:if test="not(contains(@width, 'px') or contains(@width, 'pt') or contains(@width, '%') or contains(@width, 'pc') or contains(@width, 'em') or contains(@width, 'in') or contains(@width, 'cm') or contains(@width, 'mm'))">
    ...
    <xsl:if test="@height and @height != ''">
      <xsl:attribute name="height">
        <xsl:if test="contains(@height, 'px') or contains(@height, 'pt') or contains(@height, '%') or contains(@height, 'pc') or contains(@height, 'em') or contains(@height, 'in') or contains(@height, 'cm') or contains(@height, 'mm')">
          <xsl:value-of select="@width"/>
        </xsl:if>
        <xsl:if test="not(contains(@height, 'px') or contains(@height, 'pt') or contains(@height, '%') or contains(@height, 'pc') or contains(@height, 'em') or contains(@height, 'in') or contains(@height, 'cm') or contains(@height, 'mm'))">
  ...
  <xsl:template match="xhtml:th|th">
    <fo:table-cell font-weight="bold" padding="2px" border-style="solid" border-width="0.5px" background-color="#e0e0e0">
      <xsl:call-template name="common-atts"/>
      <xsl:if test="@colspan">
        <xsl:attribute name="number-columns-spanned">
  ...
  <xsl:template match="xhtml:td|td">
    <fo:table-cell padding="2px" border-style="solid" border-width="0.5px">
      <xsl:call-template name="common-atts"/>
      <xsl:if test="@colspan">
        <xsl:attribute name="number-columns-spanned">
          <xsl:value-of select="@colspan"/>
