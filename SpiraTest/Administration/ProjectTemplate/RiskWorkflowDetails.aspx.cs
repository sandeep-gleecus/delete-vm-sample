...
namespace Inflectra.SpiraTest.Web.Administration.ProjectTemplate
{
	...
	/// Administration Risk Workflow Details Page and handling all raised events
	/// </summary>
    [
    HeaderSettings(GlobalNavigation.NavigationHighlightedLink.Administration, "Admin_WorkflowDetails_Title", "Template-Risks/#edit-workflow-detailsn", "Admin_WorkflowDetails_Title"),
    ...
    public partial class RiskWorkflowDetails : AdministrationBase
	{
        private const string CLASS_NAME = "Inflectra.SpiraTest.Web.Administration.ProjectTemplate.RiskWorkflowDetails::";

        protected int workflowId;
        protected List<RiskStatus> riskStatuses;
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            if (!Int32.TryParse(Request.QueryString[GlobalFunctions.PARAMETER_WORKFLOW_ID], out workflowId))
            {
                ...
                Response.Redirect(UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "RiskWorkflows"));
                return;
            }
            ...
            //Now get the list of possible workflow steps (i.e. risk statuses)
            RiskManager riskManager = new RiskManager();
            this.riskStatuses = riskManager.RiskStatus_Retrieve(ProjectTemplateId);

            //Set the the return URL
            this.lnkWorkflowList.NavigateUrl = UrlRoots.RetrieveTemplateAdminUrl(ProjectTemplateId, "RiskWorkflows");
		...
		protected void LoadAndBindData()
		{
            ...
            RiskWorkflow workflow = riskWorkflowManager.Workflow_RetrieveById(this.workflowId);
			...
			//Databind the form
            this.grdWorkflowSteps.DataSource = riskStatuses;
            this.DataBind();
		...
		private void grdWorkflowSteps_RowCreated(object sender, GridViewRowEventArgs e)
		{
            ...
            {
                int statusId = ((RiskStatus)(e.Row.DataItem)).RiskStatusId;
                List<DataModel.RiskWorkflowTransition> transitions = new RiskWorkflowManager().WorkflowTransition_RetrieveByInputStatus(this.workflowId, statusId);
		...
		protected void rptWorkflowStepTransitions_ItemCommand(object source, RepeaterCommandEventArgs e)
		{
			...
			if (e.CommandName.ToLower(System.Globalization.CultureInfo.InvariantCulture) == "delete")
			{
                ...
                new RiskWorkflowManager().WorkflowTransition_Delete(workflowTransitionId);
		...
		private void btnAdd_Click(object sender, EventArgs e)
		{
			...
			int sourceRiskStatusId = Int32.Parse (this.hdnSourceStep.Value);
			int destinationRiskStatusId = Int32.Parse(this.ddlDestinationStep.RawSelectedValue);
			...
			//but when developers do the same thing they need to be the owner of the risk
            RiskWorkflowManager riskWorkflowManager = new Business.RiskWorkflowManager();
            riskWorkflowManager.WorkflowTransition_Insert(this.workflowId, sourceRiskStatusId, destinationRiskStatusId, this.txtTransitionName.Text);
        ...
        /// </summary>
        /// <returns></returns>
        protected string GetRiskStatusesAsJson()
        {
            const string METHOD_NAME = "GetRiskStatusesAsJson";

            Logger.LogEnteringEvent(CLASS_NAME + METHOD_NAME);

            Dictionary<string, string> statusDic = new Dictionary<string, string>();
            foreach (RiskStatus riskStatus in this.riskStatuses)
            {
                if (!statusDic.ContainsKey(riskStatus.RiskStatusId.ToString()))
                {
                    statusDic.Add(riskStatus.RiskStatusId.ToString(), riskStatus.Name);
                }
            }
